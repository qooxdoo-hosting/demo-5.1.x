...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.layout.Flow.461f802dfa2a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
;var x=new qx.ui.layout.LineSizeIterator(D,this.getSpacingX());var B=z.top;while(x.hasMoreLines()){var C=x.computeNextLine(A);this.__Eg(C,B,A,z);B+=C.height+this.getSpacingY();}
;}
,__Eg:function(L,K,I,E){var H=qx.ui.layout.Util;var F=E.left;if(this.getAlignX()!=l){F=E.left+I-L.width;if(this.getAlignX()==j){F=E.left+Math.round((I-L.width)/2);}
;}
...
;}
,_computeSizeHint:function(){return this.__Eh(Infinity);}
,hasHeightForWidth:function(){return true;}
,getHeightForWidth:function(P){return this.__Eh(P).height;}
...
;return S;}
,__Eh:function(T){var U=new qx.ui.layout.LineSizeIterator(this._getLayoutChildren(),this.getSpacingX());var V=0;var W=0;var X=0;while(U.hasMoreLines()){var Y=U.computeNextLine(T);X+=1;W=Math.max(W,Y.width);V+=Y.height;}
;return {width:W,height:V+this.getSpacingY()*(X-1)};}
}});}
)();
(function(){var a="No more lines to compute",b="qx.ui.layout.LineSizeIterator";qx.Class.define(b,{extend:Object,construct:function(d,c){this.__gP=d;this.__AD=c;this.__AE=d.length>0;this.__AF=0;}
,members:{__gP:null,__AD:null,__AE:null,__AF:null,computeNextLine:function(k){var k=k||Infinity;if(!this.__AE){throw new Error(a);}
;var p=this.__gP;var n=0;var f=0;var o=[];var j=[];for(var i=this.__AF;i<p.length;i++ ){var m=p[i];var q=m.getSizeHint();var g=this.__AG(i);var h=q.width+g;var e=i==this.__AF;if(!e&&f+h>k){this.__AF=i;break;}
;var l=q.height+m.getMarginTop()+m.getMarginBottom();o.push(m);j.push(g);f+=h;n=Math.max(n,l);if(m.getLayoutProperties().lineBreak){this.__AF=i+1;break;}
;}
;if(i>=p.length){this.__AE=false;}
;return {height:n,width:f,children:o,gapsBefore:j};}
,__AG:function(r){var s=r==this.__AF;if(s){return this.__gP[r].getMarginLeft();}
else {return Math.max(this.__gP[r-1].getMarginRight(),this.__gP[r].getMarginLeft(),this.__AD);}
;}
,hasMoreLines:function(){return this.__AE;}
}});}
)();
