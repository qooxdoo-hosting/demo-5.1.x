...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FormRenderer.1d9d672bdb90.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="change",b="qx.ui.form.Form",c="qx.event.type.Event",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__FH=[];this._buttons=[];this._buttonOptions=[];this._validationManager=this._createValidationManager();this._resetter=this._createResetter();}
,events:{"change":c},members:{__FH:null,_validationManager:null,_groupCounter:0,_buttons:null,_buttonOptions:null,_resetter:null,add:function(g,f,h,name,e,k){if(this.__FI()){this.__FH.push({title:null,items:[],labels:[],names:[],options:[],headerOptions:{}});}
;this.__FH[this._groupCounter].items.push(g);this.__FH[this._groupCounter].labels.push(f);this.__FH[this._groupCounter].options.push(k);if(name==null){name=f.replace(/\s+|&|-|\+|\*|\/|\||!|\.|,|:|\?|;|~|%|\{|\}|\(|\)|\[|\]|<|>|=|\^|@|\\/g,d);}
;this.__FH[this._groupCounter].names.push(name);this._validationManager.add(g,h,e);this._resetter.add(g);this.fireEvent(a);}
,addGroupHeader:function(m,l){if(!this.__FI()){this._groupCounter++ ;}
;this.__FH.push({title:m,items:[],labels:[],names:[],options:[],headerOptions:l});this.fireEvent(a);}
,addButton:function(n,o){this._buttons.push(n);this._buttonOptions.push(o||null);this.fireEvent(a);}
,__FI:function(){return this.__FH.length===0;}
,remove:function(p){for(var i=0;i<this.__FH.length;i++ ){var r=this.__FH[i];for(var j=0;j<r.items.length;j++ ){var q=r.items[j];if(q===p){r.items.splice(j,1);r.labels.splice(j,1);r.names.splice(j,1);r.options.splice(j,1);this._validationManager.remove(p);this._resetter.remove(p);this.fireEvent(a);return true;}
;}
;}
;return false;}
,removeGroupHeader:function(u){for(var i=0;i<this.__FH.length;i++ ){var t=this.__FH[i];if(t.title===u){var s;if(i==0){if(this.__FH.length==1){t.title=null;t.headerOptions={};this.fireEvent(a);return true;}
else {s=this.__FH[i+1];}
;}
else {s=this.__FH[i-1];}
;s.items=s.items.concat(t.items);s.labels=s.labels.concat(t.labels);s.names=s.names.concat(t.names);s.options=s.options.concat(t.options);this.__FH.splice(i,1);this._groupCounter-- ;this.fireEvent(a);return true;}
;}
;return false;}
...
;}
;return false;}
,getItems:function(){var x={};for(var i=0;i<this.__FH.length;i++ ){var y=this.__FH[i];for(var j=0;j<y.names.length;j++ ){var name=y.names[j];x[name]=y.items[j];}
;}
;return x;}
...
,getGroups:function(){return this.__FH;}
...
},destruct:function(){this.__FH=this._buttons=this._buttonOptions=null;this._validationManager.dispose();this._resetter.dispose();}
});}
)();
...
,events:{"changeValid":o,"complete":h},properties:{validator:{check:k,init:null,nullable:true},invalidMessage:{check:n,init:c},requiredFieldMessage:{check:n,init:c},context:{nullable:true}},members:{__Ao:null,__Aq:null,__Ap:null,__Ar:null,add:function(s,p,r){if(!this.__Aw(s)){throw new Error(b);}
;if(this.__Ab(s)&&!s.getValue){if(p!=null){throw new Error(a+d);}
;}
...
;return w;}
,validate:function(){var B=true;this.__Ar=true;var y=[];for(var i=0;i<this.__Ao.length;i++ ){var z=this.__Ao[i].item;var C=this.__Ao[i].validator;y.push(z);if(C==null){var x=this.__As(z);B=B&&x;this.__Ar=x&&this.__Ar;continue;}
;var x=this.__At(this.__Ao[i],z.getValue());B=x&&B;if(x!=null){this.__Ar=x&&this.__Ar;}
;}
;var A=this.__Au(y);if(qx.lang.Type.isBoolean(A)){this.__Ar=A&&this.__Ar;}
;B=A&&B;this._setValid(B);if(qx.lang.Object.isEmpty(this.__Ap)){this.fireEvent(j);}
;return B;}
,__As:function(E){if(E.getRequired()){if(this.__Ab(E)){var D=!!E.getSelection()[0];}
...
;return true;}
,__At:function(K,L){var M=K.item;var O=K.context;var N=K.validator;if(this.__Av(N)){this.__Ap[M.toHashCode()]=null;N.validate(M,M.getValue(),this,O);return null;}
...
,__Au:function(Q){var R=this.getValidator();var S=this.getContext()||this;if(R==null){return true;}
;this.setInvalidMessage(c);if(this.__Av(R)){this.__Ap[this.toHashCode()]=null;R.validateForm(Q,this,S);return null;}
...
;}
;return T;}
,__Av:function(V){var U=false;if(!qx.lang.Type.isFunction(V)){U=qx.Class.isSubClassOf(V.constructor,qx.ui.form.validation.AsyncValidator);}
;return U;}
,__Aw:function(W){var X=W.constructor;return qx.Class.hasInterface(X,qx.ui.form.IForm);}
,__Ab:function(Y){var ba=Y.constructor;return qx.Class.hasInterface(ba,qx.ui.core.ISingleSelection);}
...
,setItemValid:function(bn,bm){this.__Ap[bn.toHashCode()]=bm;bn.setValid(bm);this.__Ax();}
,setFormValid:function(bo){this.__Ap[this.toHashCode()]=bo;this.__Ax();}
,__Ax:function(){var bq=this.__Ar;for(var br in this.__Ap){var bp=this.__Ap[br];bq=bp&&bq;if(bp==null){return;}
;}
;this._setValid(bq);this.__Ap={};this.fireEvent(j);}
},destruct:function(){this.__Ao=null;}
});}
)();
(function(){var a="qx.ui.form.validation.AsyncValidator";qx.Class.define(a,{extend:qx.core.Object,construct:function(b){qx.core.Object.call(this);this.__Ay=b;}
,members:{__Ay:null,__Az:null,__gt:null,__AA:null,validate:function(c,e,d,f){this.__AA=false;this.__Az=c;this.__gt=d;this.__Ay.call(f||this,this,e);}
,validateForm:function(g,h,i){this.__AA=true;this.__gt=h;this.__Ay.call(i,g,this);}
,setValid:function(k,j){if(this.__AA){if(j!==undefined){this.__gt.setInvalidMessage(j);}
;this.__gt.setFormValid(k);}
else {if(j!==undefined){this.__Az.setInvalidMessage(j);}
;this.__gt.setItemValid(this.__Az,k);}
;}
},destruct:function(){this.__gt=this.__Az=null;}
});}
)();
...
(function(){var a="The given item has not been added.",b="qx.ui.form.Resetter",c=" not supported for reseting.",d="Item ";qx.Class.define(b,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__rV=[];}
,members:{__rV:null,add:function(e){if(this._supportsValue(e)){var f=e.getValue();}
else if(this.__Ab(e)){var f=e.getSelection();}
else if(this.__Ac(e)){var f=e.getSelection().concat();}
else {throw new Error(d+e+c);}
;this.__rV.push({item:e,init:f});}
,remove:function(g){for(var i=0;i<this.__rV.length;i++ ){var h=this.__rV[i];if(h.item===g){this.__rV.splice(i,1);return true;}
;}
;return false;}
,reset:function(){for(var i=0;i<this.__rV.length;i++ ){var j=this.__rV[i];this.__zY(j.item,j.init);}
;}
,resetItem:function(k){var l;for(var i=0;i<this.__rV.length;i++ ){var m=this.__rV[i];if(m.item===k){l=m.init;break;}
;}
;if(l===undefined){throw new Error(a);}
;this.__zY(k,l);}
,__zY:function(n,o){if(this._supportsValue(n)){n.setValue(o);}
else if(this.__Ab(n)||this.__Ac(n)){n.setSelection(o);}
;}
,redefine:function(){for(var i=0;i<this.__rV.length;i++ ){var p=this.__rV[i].item;this.__rV[i].init=this.__Aa(p);}
;}
,redefineItem:function(q){var r;for(var i=0;i<this.__rV.length;i++ ){if(this.__rV[i].item===q){r=this.__rV[i];break;}
;}
;if(r===undefined){throw new Error(a);}
;r.init=this.__Aa(r.item);}
,__Aa:function(s){if(this._supportsValue(s)){return s.getValue();}
else if(this.__Ab(s)||this.__Ac(s)){return s.getSelection();}
;}
,__Ab:function(t){var u=t.constructor;return qx.Class.hasInterface(u,qx.ui.core.ISingleSelection);}
,__Ac:function(v){var w=v.constructor;return qx.Class.hasInterface(w,qx.data.controller.ISelection);}
...
,_applyValue:function(bi,bh){var bg=this.getChildControl(t);this._updateButtons();this.__wH=bi;if(bi!==null){if(this.getNumberFormat()){bg.setValue(this.getNumberFormat().format(bi));}
...
;this._applyValue(this.__wH,undefined);}
...
;}
;}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case a:this.__wI=true;case p:this.getChildControl(y).press();break;case m:this.__wJ=true;case h:this.getChildControl(q).press();break;default:return;};e.stopPropagation();e.preventDefault();}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case a:this.getChildControl(y).release();this.__wI=false;break;case p:this.getChildControl(y).release();break;case m:this.getChildControl(q).release();this.__wJ=false;break;case h:this.getChildControl(q).release();break;};}
...
;if(bx===this.__wH){this._applyValue(this.__wH);}
else {this.setValue(bx);}
;}
else {this._applyValue(this.__wH,undefined);}
;}
...
,_countUp:function(){if(this.__wI){var bE=this.getValue()+this.getPageStep();}
...
;}
;this.gotoValue(bE);}
,_countDown:function(){if(this.__wJ){var bG=this.getValue()-this.getPageStep();}
...
,properties:{appearance:{refine:true,init:t}},members:{__px:null,_createChildControlImpl:function(B,A){var z;switch(B){case s:z=new qx.ui.core.Spacer();this._add(z,{flex:1});break;case y:z=new qx.ui.basic.Atom(v);z.setCenter(false);z.setAnonymous(true);this._add(z,{flex:1});break;case n:z=new qx.ui.basic.Image();z.setAnonymous(true);this._add(z);break;};return z||qx.ui.form.AbstractSelectBox.prototype._createChildControlImpl.call(this,B);}
...
,__og:function(e){var D=e.getData()[0];var C=this.getChildControl(l);if(C.getSelection()[0]!=D){if(D){C.setSelection([D]);}
else {C.resetSelection();}
;}
;this.__py();this.__pz();}
,__py:function(){var F=this.getChildControl(l).getSelection()[0];var E=this.getChildControl(y);var G=F?F.getIcon():q;G==null?E.resetIcon():E.setIcon(G);}
,__pz:function(){var J=this.getChildControl(l).getSelection()[0];var H=this.getChildControl(y);var I=J?J.getLabel():q;var K=this.getFormat();if(K!=null){I=K.call(this,J);}
...
,_onKeyPress:function(e){var L=e.getKeyIdentifier();if(L==b||L==i){if(this.__px){this.setSelection([this.__px]);this.__px=null;}
;this.toggle();}
...
,_onListPointerDown:function(e){if(this.__px){this.setSelection([this.__px]);this.__px=null;}
;}
,_onListChangeSelection:function(e){var Q=e.getData();var O=e.getOldData();if(O&&O.length>0){O[0].removeListener(h,this.__py,this);O[0].removeListener(g,this.__pz,this);}
;if(Q.length>0){var N=this.getChildControl(u);var P=this.getChildControl(l);var R=P.getSelectionContext();if(N.isVisible()&&(R==p||R==k)){this.__px=Q[0];}
else {this.setSelection([Q[0]]);this.__px=null;}
;Q[0].addListener(h,this.__py,this);Q[0].addListener(g,this.__pz,this);}
...
,properties:{wrap:{check:b,init:true,apply:f},appearance:{refine:true,init:j},singleStep:{check:i,init:20},minimalLineHeight:{check:i,apply:h,init:4},autoSize:{check:b,apply:w,init:false}},members:{__nQ:null,__nR:null,__nS:null,setValue:function(M){M=qx.ui.form.AbstractField.prototype.setValue.call(this,M);this.__nT();return M;}
...
;}
,__nT:function(){if(this.isAutoSize()){var R=this.__nU();if(R&&this.getBounds()){this.__nS=this.__nS||this._getAreaHeight();var T=this._getScrolledAreaHeight();if(this.getMaxHeight()){var P=this.getInsets();var Q=-P.top+this.getMaxHeight()-P.bottom;if(T>Q){this.getContentElement().setStyle(I,k);}
else {this.getContentElement().setStyle(I,H);}
;}
;var S=Math.max(T,this.__nS);this._setAreaHeight(S);}
else {this.getContentElement().addListenerOnce(x,function(){this.__nT();}
,this);}
;}
;}
,_getAreaHeight:function(){return this.getInnerSize().height;}
,_setAreaHeight:function(U){if(this._getAreaHeight()!==U){this.__nR=U;qx.ui.core.queue.Layout.add(this);qx.ui.core.queue.Manager.flush();this.__nX();}
;}
,_getScrolledAreaHeight:function(){var W=this.__nU();var V=W.getDomElement();if(V){if(!V.parentNode){qx.html.Element.flush();return this._getScrolledAreaHeight();}
...
;this.__nW(W);if(qx.core.Environment.get(C)==B&&qx.core.Environment.get(s)==8){if(!V.scrollTop){qx.html.Element.flush();}
;}
;return V.scrollTop;}
;}
,__nU:function(){this.__nQ=this.__nQ||this.__nV();return this.__nQ;}
,__nV:function(){var ba,Y,bb,X;ba=this.getContentElement();if(!ba.getDomElement()){return null;}
;bb=qx.bom.Element.clone(ba.getDomElement());X=new qx.html.Input(j);X.useElement(bb);Y=X;Y.setStyles({position:K,top:0,left:o,height:0,overflow:H},true);Y.removeAttribute(G);Y.removeAttribute(z);Y.setAttribute(y,g);Y.setValue(ba.getValue()||t);Y.insertBefore(ba);this.__nW(Y);return Y;}
,__nW:function(bc){bc=bc.getDomElement();if(bc){bc.scrollTop=10000;}
;}
...
,_applyAutoSize:function(bh,bg){if(qx.core.Environment.get(m)){this.__nY();}
;if(bh){this.__nT();this.addListener(a,this.__nT,this);if(!this.getBounds()){this.addListenerOnce(x,function(){this.getContentElement().setStyle(I,H);}
);}
else {this.getContentElement().setStyle(I,H);}
;}
else {this.removeListener(a,this.__nT);this.getContentElement().setStyle(I,k);}
;}
,_applyDimension:function(bi){qx.ui.form.AbstractField.prototype._applyDimension.call(this);if(qx.core.Environment.get(m)){this.__nY();}
;if(bi===this.getMaxHeight()){this.__nT();}
;}
,__nX:function(){var content=this.getContentElement();var bk=content.getDomElement();var bj=content.getStyle(u);content.setStyle(u,parseInt(bj,10)+1000+c,true);if(bk){qx.bom.element.Dimension.getWidth(bk);}
;content.setStyle(u,bj,true);}
,__nY:function(){if(this.isAutoSize()&&this.getHeight()){this.warn(n+J+v);}
;}
,_getContentHint:function(){var bl=qx.ui.form.AbstractField.prototype._getContentHint.call(this);bl.height=bl.height*this.getMinimalLineHeight();bl.width=this._getTextSize().width*20;if(this.isAutoSize()){bl.height=this.__nR||bl.height;}
;return bl;}
},destruct:function(){this.setAutoSize(false);if(this.__nQ){this.__nQ.dispose();}
;}
});}
)();
