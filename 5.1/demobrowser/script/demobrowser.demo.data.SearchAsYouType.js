...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.SearchAsYouType.75cb7a422b4f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var a="_applySearchString",b="demobrowser.demo.data.filter.SearchAsYouTypeFilter",c="String",d="";qx.Class.define(b,{extend:qx.core.Object,construct:function(e){qx.core.Object.call(this);this.__HL=e;this.filter=qx.lang.Function.bind(function(f){return f.search(this.getSearchString())!=-1;}
,this);this.__sr=null;}
,properties:{searchString:{check:c,apply:a,init:d}},members:{__sr:null,__HL:null,_applySearchString:function(h,g){var i=qx.util.TimerManager.getInstance();if(this.__sr!=null){i.stop(this.__sr);this.__sr=null;}
;this.__sr=i.start(function(){this.__HL.update();this.__sr=null;}
,0,this,null,200);}
,filter:null},destruct:function(){this.__HL=null;}
});}
)();
(function(){var a="qx.util.TimerManager",b="interval",c="singleton";qx.Class.define(a,{extend:qx.core.Object,type:c,statics:{__CG:[],__CH:{},__sr:0},members:{__CI:false,start:function(g,f,h,d,j){if(!j){j=f||0;}
;var e=(new Date()).getTime()+j;this.self(arguments).__CH[ ++this.self(arguments).__sr]={callback:g,userData:d||null,expireAt:e,recurTime:f,context:h||this};this.__CJ(e,this.self(arguments).__sr);return this.self(arguments).__sr;}
,stop:function(k){var l=this.self(arguments).__CG;var length=l.length;for(var i=0;i<length;i++ ){if(l[i]==k){l.splice(i,1);break;}
;}
;delete this.self(arguments).__CH[k];if(l.length==0&&this.__CI){qx.event.Idle.getInstance().removeListener(b,this.__CK,this);this.__CI=false;}
;}
,__CJ:function(m,p){var o=this.self(arguments).__CG;var n=this.self(arguments).__CH;var length=o.length;for(var i=0;i<length;i++ ){if(n[o[i]].expireAt>m){o.splice(i,0,p);break;}
;}
;if(o.length==length){o.push(p);}
;if(!this.__CI){qx.event.Idle.getInstance().addListener(b,this.__CK,this);this.__CI=true;}
;}
,__CK:function(){var r=(new Date()).getTime();var t=this.self(arguments).__CG;var s=this.self(arguments).__CH;while(t.length>0&&s[t[0]].expireAt<=r){var v=t.shift();var q=s[v];q.callback.call(q.context,q.userData,v);if(q.recurTime&&s[v]){var u=(new Date()).getTime();q.expireAt=u+q.recurTime;this.__CJ(q.expireAt,v);}
else {delete s[v];}
;}
;if(t.length==0&&this.__CI){qx.event.Idle.getInstance().removeListener(b,this.__CK,this);this.__CI=false;}
;}
}});}
)();
