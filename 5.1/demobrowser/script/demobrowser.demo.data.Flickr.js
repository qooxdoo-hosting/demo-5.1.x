...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.data.Flickr.931d8e05d31a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b=".prototype",c="function",d="Boolean",e="Error",f="Object.keys requires an object as argument.",g="constructor",h="warn",j="default",k="Null",m="hasOwnProperty",n="Undefined",o="string",p="Object",q="toLocaleString",r="error",s="toString",t="Blob",u="qx.debug",v="()",w="RegExp",x="String",y="info",z="BROKEN_IE",A="isPrototypeOf",B="Date",C="",D="qx.Bootstrap",E="Function",F="]",G="Cannot call super class. Method is not derived: ",H="Array",I="[Class ",J="valueOf",K="Number",L="FormData",M="Class",N="debug",O="ES5",P=".",Q="ArrayBuffer",R="propertyIsEnumerable",S="object";if(!window.qx){window.qx={};}
;qx.Bootstrap={genericToString:function(){return I+this.classname+F;}
,createNamespace:function(name,T){var W=name.split(P);var V=W[0];var parent=qx.$$namespaceRoot&&qx.$$namespaceRoot[V]?qx.$$namespaceRoot:window;for(var i=0,U=W.length-1;i<U;i++ ,V=W[i]){if(!parent[V]){parent=parent[V]={};}
else {parent=parent[V];}
;}
;parent[V]=T;return V;}
,setDisplayName:function(Y,X,name){Y.displayName=X+P+name+v;}
,setDisplayNames:function(bb,ba){for(var name in bb){var bc=bb[name];if(bc instanceof Function){bc.displayName=ba+P+name+v;}
;}
;}
,base:function(bd,be){if(qx.Bootstrap.DEBUG){if(!qx.Bootstrap.isFunction(bd.callee.base)){throw new Error(G+bd.callee.displayName);}
;}
...
;}
,define:function(name,bp){if(!bp){bp={statics:{}};}
;var bl;var bh=null;qx.Bootstrap.setDisplayNames(bp.statics,name);if(bp.members||bp.extend){qx.Bootstrap.setDisplayNames(bp.members,name+b);bl=bp.construct||new Function;if(bp.extend){this.extendClass(bl,bl,bp.extend,name,bj);}
;var bg=bp.statics||{};for(var i=0,bi=qx.Bootstrap.keys(bg),l=bi.length;i<l;i++ ){var bf=bi[i];bl[bf]=bg[bf];}
;bh=bl.prototype;bh.base=qx.Bootstrap.base;bh.name=bh.classname=name;var bn=bp.members||{};var bf,bm;for(var i=0,bi=qx.Bootstrap.keys(bn),l=bi.length;i<l;i++ ){bf=bi[i];bm=bn[bf];if(bm instanceof Function&&bh[bf]){bm.base=bh[bf];}
;bh[bf]=bm;}
;}
else {bl=bp.statics||{};if(qx.Bootstrap.$$registry&&qx.Bootstrap.$$registry[name]){var bo=qx.Bootstrap.$$registry[name];if(this.keys(bl).length!==0){if(bp.defer){bp.defer(bl,bh);}
;for(var bk in bl){bo[bk]=bl[bk];}
;return bo;}
;}
;}
;bl.$$type=M;if(!bl.hasOwnProperty(s)){bl.toString=this.genericToString;}
;var bj=name?this.createNamespace(name,bl):C;bl.name=bl.classname=name;bl.basename=bj;bl.$$events=bp.events;if(bp.defer){bp.defer(bl,bh);}
;if(name!=null){qx.Bootstrap.$$registry[name]=bl;}
;return bl;}
};qx.Bootstrap.define(D,{statics:{LOADSTART:qx.$$start||new Date(),DEBUG:(function(){var bq=true;if(qx.$$environment&&qx.$$environment[u]===false){bq=false;}
;return bq;}
)(),getEnvironmentSetting:function(br){if(qx.$$environment){return qx.$$environment[br];}
;}
,setEnvironmentSetting:function(bs,bt){if(!qx.$$environment){qx.$$environment={};}
;if(qx.$$environment[bs]===undefined){qx.$$environment[bs]=bt;}
;}
,createNamespace:qx.Bootstrap.createNamespace,setRoot:function(bu){qx.$$namespaceRoot=bu;}
...
,$$registry:{},objectGetLength:function(bv){return qx.Bootstrap.keys(bv).length;}
,objectMergeWith:function(bx,bw,bz){if(bz===undefined){bz=true;}
;for(var by in bw){if(bz||bx[by]===undefined){bx[by]=bw[by];}
;}
;return bx;}
,__a:[A,m,q,s,J,R,g],keys:({"ES5":Object.keys,"BROKEN_IE":function(bA){if(bA===null||(typeof bA!=S&&typeof bA!=c)){throw new TypeError(f);}
;var bB=[];var bD=Object.prototype.hasOwnProperty;for(var bE in bA){if(bD.call(bA,bE)){bB.push(bE);}
;}
;var bC=qx.Bootstrap.__a;for(var i=0,a=bC,l=a.length;i<l;i++ ){if(bD.call(bA,a[i])){bB.push(a[i]);}
;}
;return bB;}
,"default":function(bF){if(bF===null||(typeof bF!=S&&typeof bF!=c)){throw new TypeError(f);}
;var bG=[];var bH=Object.prototype.hasOwnProperty;for(var bI in bF){if(bH.call(bF,bI)){bG.push(bI);}
;}
;return bG;}
})[typeof (Object.keys)==c?O:(function(){for(var bJ in {toString:1}){return bJ;}
;}
)()!==s?z:j],__b:{"[object String]":x,"[object Array]":H,"[object Object]":p,"[object RegExp]":w,"[object Number]":K,"[object Boolean]":d,"[object Date]":B,"[object Function]":E,"[object Error]":e,"[object Blob]":t,"[object ArrayBuffer]":Q,"[object FormData]":L},bind:function(bL,self,bM){var bK=Array.prototype.slice.call(arguments,2,arguments.length);return function(){var bN=Array.prototype.slice.call(arguments,0,arguments.length);return bL.apply(self,bK.concat(bN));}
;}
...
,getClass:function(bR){if(bR===undefined){return n;}
else if(bR===null){return k;}
;var bQ=Object.prototype.toString.call(bR);return (qx.Bootstrap.__b[bQ]||bQ.slice(8,-1));}
,isString:function(bS){return (bS!==null&&(typeof bS===o||qx.Bootstrap.getClass(bS)==x||bS instanceof String||(!!bS&&!!bS.$$isString)));}
,isArray:function(bT){return (bT!==null&&(bT instanceof Array||(bT&&qx.data&&qx.data.IListData&&qx.util.OOUtil.hasInterface(bT.constructor,qx.data.IListData))||qx.Bootstrap.getClass(bT)==H||(!!bT&&!!bT.$$isArray)));}
,isObject:function(bU){return (bU!==undefined&&bU!==null&&qx.Bootstrap.getClass(bU)==p);}
,isFunction:function(bV){return qx.Bootstrap.getClass(bV)==E;}
,$$logs:[],debug:function(bX,bW){qx.Bootstrap.$$logs.push([N,arguments]);}
,info:function(ca,bY){qx.Bootstrap.$$logs.push([y,arguments]);}
,warn:function(cc,cb){qx.Bootstrap.$$logs.push([h,arguments]);}
,error:function(ce,cd){qx.Bootstrap.$$logs.push([r,arguments]);}
,trace:function(cf){}
}});}
)();
...
;}
,__V:function(fz){var name=fz.classname;var fy=this.__ba(fz,name,fz.$$classtype);for(var i=0,a=Object.keys(fz),l=a.length;i<l;i++ ){fA=a[i];fy[fA]=fz[fA];}
...
;}
;return cn;}
,assert:function(cq,cp){cq==true||this.__bQ(cp||Q,v);}
,fail:function(cr,cs){var ct=cs?Q:bS;this.__bQ(cr||Q,ct);}
,assertTrue:function(cv,cu){(cv===true)||this.__bQ(cu||Q,m,cv,x);}
,assertFalse:function(cx,cw){(cx===false)||this.__bQ(cw||Q,W,cx,x);}
,assertEquals:function(cy,cz,cA){cy==cz||this.__bQ(cA||Q,D,cy,z,cz,O);}
,assertNotEquals:function(cB,cC,cD){cB!=cC||this.__bQ(cD||Q,D,cB,h,cC,O);}
,assertIdentical:function(cE,cF,cG){cE===cF||this.__bQ(cG||Q,D,cE,T,cF,O);}
,assertNotIdentical:function(cH,cI,cJ){cH!==cI||this.__bQ(cJ||Q,D,cH,bH,cI,O);}
,assertNotUndefined:function(cL,cK){cL!==undefined||this.__bQ(cK||Q,J);}
,assertUndefined:function(cN,cM){cN===undefined||this.__bQ(cM||Q,p,cN,I);}
,assertNotNull:function(cP,cO){cP!==null||this.__bQ(cO||Q,bN);}
,assertNull:function(cR,cQ){cR===null||this.__bQ(cQ||Q,t,cR,I);}
,assertJsonEquals:function(cS,cT,cU){this.assertEquals(qx.lang.Json.stringify(cS),qx.lang.Json.stringify(cT),cU);}
,assertMatch:function(cX,cW,cV){this.assertString(cX);this.assert(qx.lang.Type.isRegExp(cW)||qx.lang.Type.isString(cW),cf);cX.search(cW)>=0||this.__bQ(cV||Q,E,cX,bG,cW.toString(),O);}
,assertArgumentsCount:function(db,dc,dd,cY){var da=db.length;(da>=dc&&da<=dd)||this.__bQ(cY||Q,cd,dc,j,dd,bP,da,bK);}
...
;}
;df===true||this.__bQ(dj||Q,G,event,N);}
,assertEventNotFired:function(dn,event,dr,ds){var dp=false;var dq=function(e){dp=true;}
;var dt=dn.addListener(event,dq,dn);dr.call();dp===false||this.__bQ(ds||Q,G,event,f);dn.removeListenerById(dt);}
,assertException:function(dx,dw,dv,du){var dw=dw||Error;var dy;try{this.__bP=false;dx();}
catch(dz){dy=dz;}
finally{this.__bP=true;}
;if(dy==null){this.__bQ(du||Q,o);}
;dy instanceof dw||this.__bQ(du||Q,M,dw,a,dy);if(dv){this.assertMatch(dy.toString(),dv,du);}
;}
,assertInArray:function(dC,dB,dA){dB.indexOf(dC)!==-1||this.__bQ(dA||Q,bU,dC,U,dB,x);}
...
;}
;}
,assertKeyInMap:function(dI,dH,dG){dH[dI]!==undefined||this.__bQ(dG||Q,bU,dI,C,dH,x);}
,assertFunction:function(dK,dJ){qx.lang.Type.isFunction(dK)||this.__bQ(dJ||Q,bW,dK,I);}
,assertString:function(dM,dL){qx.lang.Type.isString(dM)||this.__bQ(dL||Q,F,dM,I);}
,assertBoolean:function(dO,dN){qx.lang.Type.isBoolean(dO)||this.__bQ(dN||Q,bM,dO,I);}
,assertNumber:function(dQ,dP){(qx.lang.Type.isNumber(dQ)&&isFinite(dQ))||this.__bQ(dP||Q,V,dQ,I);}
,assertPositiveNumber:function(dS,dR){(qx.lang.Type.isNumber(dS)&&isFinite(dS)&&dS>=0)||this.__bQ(dR||Q,bY,dS,I);}
,assertInteger:function(dU,dT){(qx.lang.Type.isNumber(dU)&&isFinite(dU)&&dU%1===0)||this.__bQ(dT||Q,cb,dU,I);}
,assertPositiveInteger:function(dX,dV){var dW=(qx.lang.Type.isNumber(dX)&&isFinite(dX)&&dX%1===0&&dX>=0);dW||this.__bQ(dV||Q,g,dX,I);}
,assertInRange:function(eb,ec,ea,dY){(eb>=ec&&eb<=ea)||this.__bQ(dY||Q,qx.lang.String.format(bL,[eb,ec,ea]));}
,assertObject:function(ee,ed){var ef=ee!==null&&(qx.lang.Type.isObject(ee)||typeof ee===ce);ef||this.__bQ(ed||Q,R,(ee),I);}
,assertArray:function(eh,eg){qx.lang.Type.isArray(eh)||this.__bQ(eg||Q,c,eh,I);}
,assertMap:function(ej,ei){qx.lang.Type.isObject(ej)||this.__bQ(ei||Q,n,ej,I);}
,assertRegExp:function(el,ek){qx.lang.Type.isRegExp(el)||this.__bQ(ek||Q,r,el,I);}
,assertType:function(eo,en,em){this.assertString(en,u);typeof (eo)===en||this.__bQ(em||Q,bE,en,S,eo,I);}
,assertInstance:function(er,es,ep){var eq=es.classname||es+Q;er instanceof es||this.__bQ(ep||Q,ca,eq,S,er,I);}
,assertInterface:function(ev,eu,et){qx.Class&&qx.Class.implementsInterface(ev,eu)||this.__bQ(et||Q,k,ev,s,eu,O);}
...
catch(eE){this.__bQ(eB||Q,H,eC,d,ey.join(B),bT,ez,bQ);}
;var eD=ey[0]==eA[0]&&ey[1]==eA[1]&&ey[2]==eA[2];eD||this.__bQ(eB||Q,H,ey,d,ey.join(B),bT,ez,d,eA.join(B),bO);}
,assertElement:function(eG,eF){!!(eG&&eG.nodeType===1)||this.__bQ(eF||Q,q,eG,O);}
,assertQxObject:function(eI,eH){this.__bS(eI,b)||this.__bQ(eH||Q,bJ,eI,I);}
,assertQxWidget:function(eK,eJ){this.__bS(eK,X)||this.__bQ(eJ||Q,bD,eK,I);}
,__bS:function(eM,eL){if(!eM){return false;}
...
;this.__bt=e||c;this.message=f||qx.type.BaseError.DEFAULTMESSAGE;}
,statics:{DEFAULTMESSAGE:d},members:{__bu:null,__bt:null,message:null,getComment:function(){return this.__bt;}
,toString:function(){return this.__bt+(this.message?a+this.message:c);}
}});}
)();
(function(){var a="qx.core.AssertionError";qx.Bootstrap.define(a,{extend:qx.type.BaseError,construct:function(b,c){qx.type.BaseError.call(this,b,c);this.__bv=qx.dev.StackTrace.getStackTrace();}
...
;U=/@(.+):(\d+)$/gm;while((V=U.exec(bg.stack))!=null){be=V[1];T=V[2];bd=this.__bw(be);W.push(bd+p+T);}
;if(W.length>0){return this.__by(W);}
;U=/at (.*)/gm;var bh=/\((.*?)(:[^\/].*)\)/;var bc=/(.*?)(:[^\/].*)/;while((V=U.exec(bg.stack))!=null){var bb=bh.exec(V[1]);if(!bb){bb=bc.exec(V[1]);}
;if(bb){bd=this.__bw(bb[1]);W.push(bd+bb[2]);}
else {W.push(V[1]);}
;}
;}
else if(bf===t){var X=bg.stacktrace;if(!X){return W;}
;if(X.indexOf(j)>=0){X=X.split(j)[0];}
;U=/line\ (\d+?),\ column\ (\d+?)\ in\ (?:.*?)\ in\ (.*?):[^\/]/gm;while((V=U.exec(X))!=null){T=V[1];S=V[2];be=V[3];bd=this.__bw(be);W.push(bd+p+T+p+S);}
;if(W.length>0){return this.__by(W);}
;U=/Line\ (\d+?)\ of\ linked\ script\ (.*?)$/gm;while((V=U.exec(X))!=null){T=V[1];be=V[2];bd=this.__bw(be);W.push(bd+p+T);}
;}
else if(bg.message&&bg.message.indexOf(m)>=0){var ba=bg.message.split(m)[1].trim();var Y=ba.split(f);for(var i=0;i<Y.length;i++ ){var R=Y[i].match(/\s*Line ([0-9]+) of.* (\S.*)/);if(R&&R.length>=2){T=R[1];bi=this.__bw(R[2]);W.push(bi+p+T);}
;}
;}
else if(bg.sourceURL&&bg.line){W.push(this.__bw(bg.sourceURL)+p+bg.line);}
;return this.__by(W);}
,__bw:function(bk){if(typeof qx.dev.StackTrace.FILENAME_TO_CLASSNAME==r){var bj=qx.dev.StackTrace.FILENAME_TO_CLASSNAME(bk);if(qx.core.Environment.get(g)&&!qx.lang.Type.isString(bj)){throw new Error(k);}
;return bj;}
;return qx.dev.StackTrace.__bx(bk);}
,__bx:function(bn){var bo=h;var bl=bn.indexOf(bo);var bp=bn.indexOf(a);if(bp>=0){bn=bn.substring(0,bp);}
;var bm=(bl==-1)?bn:bn.substring(bl+bo.length).replace(/\//g,q).replace(/\.js$/,e);return bm;}
,__by:function(bq){if(typeof qx.dev.StackTrace.FORMAT_STACKTRACE==r){bq=qx.dev.StackTrace.FORMAT_STACKTRACE(bq);if(qx.core.Environment.get(g)&&!qx.lang.Type.isArray(bq)){throw new Error(o);}
;}
;return bq;}
...
;}
;}
else if(qx.bom.client.Engine.__bz()){if(/Opera[\s\/]([0-9]+)\.([0-9])([0-9]*)/.test(B)){if(B.indexOf(g)!=-1){var E=B.match(/Version\/(\d+)\.(\d+)/);C=E[1]+x+E[2].charAt(0)+x+E[2].substring(1,E[2].length);}
else {C=RegExp.$1+x+RegExp.$2;if(RegExp.$3!=p){C+=x+RegExp.$3;}
;}
;}
;}
else if(qx.bom.client.Engine.__bA()){if(/AppleWebKit\/([^ ]+)/.test(B)){C=RegExp.$1;var D=RegExp(c).exec(C);if(D){C=C.slice(0,D.index);}
;}
;}
else if(qx.bom.client.Engine.__bC()||qx.bom.client.Engine.__bB()){if(/rv\:([^\);]+)(\)|;)/.test(B)){C=RegExp.$1;}
;}
else {var z=window.qxFail;if(z&&typeof z===b){C=z().FULLVERSION;}
else {C=f;qx.Bootstrap.warn(o+B+s);}
;}
;return C;}
,getName:function(){var name;if(qx.bom.client.Engine.__bD()){name=t;}
else if(qx.bom.client.Engine.__bz()){name=q;}
else if(qx.bom.client.Engine.__bA()){name=v;}
else if(qx.bom.client.Engine.__bC()||qx.bom.client.Engine.__bB()){name=e;}
else {var F=window.qxFail;if(F&&typeof F===b){name=F().NAME;}
else {name=e;qx.Bootstrap.warn(o+window.navigator.userAgent+s);}
;}
;return name;}
,__bz:function(){return window.opera&&Object.prototype.toString.call(window.opera)==a;}
,__bA:function(){return window.navigator.userAgent.indexOf(m)!=-1;}
,__bB:function(){return window.navigator.userAgent.indexOf(l)!=-1;}
,__bC:function(){return window.navigator.mozApps&&window.navigator.product===k&&window.navigator.userAgent.indexOf(l)==-1&&window.navigator.userAgent.indexOf(n)==-1;}
,__bD:function(){if(window.navigator.cpuClass&&(/MSIE\s+([^\);]+)(\)|;)/.test(window.navigator.userAgent)||/Trident\/\d+?\.\d+?/.test(window.navigator.userAgent))){return true;}
;if(qx.bom.client.Engine.__bE()){return true;}
;return false;}
,__bE:function(){return window.navigator.userAgent.indexOf(r)>-1;}
...
(function(){var a="qx.globalErrorHandling",b="qx.event.GlobalError";qx.Bootstrap.define(b,{statics:{__bF:null,__bG:null,__bH:null,__bI:function(){if(qx.core&&qx.core.Environment){return qx.core.Environment.get(a);}
...
(function(){var c="-",d="qx.debug.dispose",e="",f="qx.core.ObjectRegistry",g="qx.debug",h="$$hash",j="-0",k="Invalid object: ",m="Could not dispose object ",n=" objects",o=": ",p="Disposed ";qx.Bootstrap.define(f,{statics:{inShutDown:false,__j:{},__bX:0,__bY:[],__ca:e,__cb:{},register:function(q){var t=this.__j;if(!t){return;}
...
;var w=this.__j;if(w&&w[v]){delete w[v];this.__bY.push(v);}
;try{delete u.$$hash;}
...
;}
;if(!(cG={}.hasOwnProperty)){cG=function(dp){var dq={},dr;if((dq.__bT=null,dq.__bT={"toString":1},dq).toString!=cr){cG=function(ds){var dt=this.__bT,du=ds in (this.__bT=null,this);this.__bT=dt;return du;}
;}
...
;}
,_getRequest:function(){return this.__uy;}
,_setRequest:function(C){this.__uy=C;}
,_createRequest:function(D){if(this.__uy){this.__uy.dispose();this.__uy=null;}
...
;this.fireDataEvent(e,this.getModel());if(this.__uy){this.__uy.dispose();this.__uy=null;}
;}
,reload:function(){var R=this.getUrl();if(R!=null){this._createRequest(R);}
;}
},destruct:function(){if(this.__uy!=null){this._disposeObjects(g);}
...
},members:{__yL:null,__yM:function(z){return Object.keys(z).sort().join(a);}
,toClass:function(A,B){this.__yN(A,B,null,0);}
,__yN:function(N,I,C,K){if(!qx.lang.Type.isObject(N)||!!N.$$isString||N instanceof qx.core.Object){if(N instanceof Array||qx.Bootstrap.getClass(N)==h){for(var i=0;i<N.length;i++ ){this.__yN(N[i],I,C+o+i+m,K+1);}
;}
;return;}
;var E=this.__yM(N);if(this.__yR(E,C,K)){return;}
;for(var M in N){this.__yN(N[M],I,M,K+1);}
;if(qx.Class.isDefined(s+E)){return;}
;if(this.__yL&&this.__yL.getModelClass&&this.__yL.getModelClass(E,N,C,K)!=null){return;}
;var O={};var F={__yO:this.__yO};for(var M in N){if(this.__yL&&this.__yL.getPropertyMapping){M=this.__yL.getPropertyMapping(M,E);}
...
;if(this.__yL&&this.__yL.getValidationRule){var H=this.__yL.getValidationRule(E,M);if(H){O[M].validate=k+M;F[k+M]=H;}
;}
;}
;if(this.__yL&&this.__yL.getModelSuperClass){var L=this.__yL.getModelSuperClass(E,C,K)||qx.core.Object;}
else {var L=qx.core.Object;}
;var G=[];if(this.__yL&&this.__yL.getModelMixins){var J=this.__yL.getModelMixins(E,C,K);if(!qx.lang.Type.isArray(J)){if(J!=null){G=[J];}
;}
else {G=J;}
;}
;if(I){G.push(qx.data.marshal.MEventBubbling);}
;var D={extend:L,include:G,properties:O,members:F,destruct:this.__yP};qx.Class.define(s+E,D);}
,__yP:function(){var P=qx.util.PropertyUtil.getAllProperties(this.constructor);for(var Q in P){this.__yO(this.get(P[Q].name));}
;}
,__yO:function(R){if(!(R instanceof qx.core.Object)){return;}
;if(R.isDisposed()){return;}
;R.dispose();}
,__yQ:function(S,W,X,V){var Y;if(this.__yL&&this.__yL.getModelClass){Y=this.__yL.getModelClass(S,W,X,V);}
;if(Y!=null){return (new Y());}
else {var U=s+S;var T=qx.Class.getByName(U);if(!T){throw new Error(b+U+l);}
;return (new T());}
;}
,__yR:function(ba,bd,bb){var bc=this.__yL;return bc&&bc.ignore&&bc.ignore(ba,bd,bb);}
,toModel:function(be){return this.__yS(be,null,0);}
,__yS:function(bq,bg,bm){var bk=qx.lang.Type.isObject(bq);var bf=bq instanceof Array||qx.Bootstrap.getClass(bq)==h;if((!bk&&!bf)||!!bq.$$isString||bq instanceof qx.core.Object){return bq;}
else if(this.__yR(this.__yM(bq),bg,bm)){return bq;}
else if(bf){var bo=qx.data.Array;if(this.__yL&&this.__yL.getArrayClass){var bl=this.__yL.getArrayClass(bg,bm);bo=bl||bo;}
;var br=new bo();br.setAutoDisposeItems(true);for(var i=0;i<bq.length;i++ ){br.push(this.__yS(bq[i],bg+o+i+m,bm+1));}
;return br;}
else if(bk){var bh=this.__yM(bq);var bp=this.__yQ(bh,bq,bg,bm);for(var bn in bq){var bi=bn;if(this.__yL&&this.__yL.getPropertyMapping){bi=this.__yL.getPropertyMapping(bn,bh);}
;var bs=bi.replace(/-|\.|\s+/g,d);if((qx.core.Environment.get(t))&&qx.core.Environment.get(e)){if(bs!=bi){this.warn(c+bn+n+bi+v);}
;}
;bi=bs;var bj=p+qx.lang.String.firstUp(bi);if(bp[bj]){bp[bj](this.__yS(bq[bn],bn,bm+1));}
;}
;return bp;}
;throw new Error(j);}
...
;if((n instanceof qx.core.Object)&&qx.Class.hasMixin(n.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__oS,this,name);var q=n.addListener(g,o,this);var p=n.getUserData(d+this.$$hash);if(p==null){p=[];n.setUserData(d+this.$$hash,p);}
;p.push(q);}
;}
,__oS:function(name,e){var y=e.getData();var u=y.value;var s=y.old;if(qx.Class.hasInterface(e.getTarget().constructor,qx.data.IListData)){if(y.name.indexOf){var x=y.name.indexOf(h)!=-1?y.name.indexOf(h):y.name.length;var v=y.name.indexOf(f)!=-1?y.name.indexOf(f):y.name.length;if(v==0){var t=name+y.name;}
...
else if(arguments.length>1){this.__oT=[];for(var i=0;i<arguments.length;i++ ){this.__oT.push(arguments[i]);}
;}
else if(typeof u==q){this.__oT=new Array(u);}
else if(u instanceof Array){this.__oT=qx.lang.Array.clone(u);}
else {this.__oT=[];this.dispose();throw new Error(l);}
;for(var i=0;i<this.__oT.length;i++ ){this._applyEventPropagation(this.__oT[i],null,i);}
;this.__oU();if(qx.core.Environment.get(k)){this[0]=h;}
;}
,properties:{autoDisposeItems:{check:e,init:false}},events:{"change":t,"changeLength":t},members:{__oT:null,concat:function(v){if(v){var w=this.__oT.concat(v);}
else {var w=this.__oT.concat();}
;return new qx.data.Array(w);}
,join:function(x){return this.__oT.join(x);}
,pop:function(){var y=this.__oT.pop();this.__oU();this._registerEventChaining(null,y,this.length-1);this.fireDataEvent(r,{value:[],name:this.length+f,old:[y],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:g,removed:[y],added:[]},null);return y;}
,push:function(z){for(var i=0;i<arguments.length;i++ ){this.__oT.push(arguments[i]);this.__oU();this._registerEventChaining(arguments[i],null,this.length-1);this.fireDataEvent(r,{value:[arguments[i]],name:(this.length-1)+f,old:[],item:this});this.fireDataEvent(o,{start:this.length-1,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,reverse:function(){if(this.length==0){return;}
;var A=this.__oT.concat();this.__oT.reverse();this.__oV(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__oT,name:n+(this.__oT.length-1),old:A,item:this});}
,shift:function(){if(this.length==0){return;}
;var B=this.__oT.shift();this.__oU();this._registerEventChaining(null,B,this.length-1);this.__oV(0,this.length);this.fireDataEvent(r,{value:[],name:p,old:[B],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:g,removed:[B],added:[]},null);return B;}
,slice:function(D,C){return new qx.data.Array(this.__oT.slice(D,C));}
,splice:function(I,K,M){var Q=this.__oT.length;var L=this.__oT.splice.apply(this.__oT,arguments);if(this.__oT.length!=Q){this.__oU();}
...
;this.__oV(I+(arguments.length-2)-K,this.length);if(O||G){var J=[];for(var i=2;i<arguments.length;i++ ){J[i-2]=arguments[i];}
...
;var R=this.__oT.concat();this.__oT.sort.apply(this.__oT,arguments);if(qx.lang.Array.equals(this.__oT,R)===true){return;}
;this.__oV(0,this.length);this.fireDataEvent(o,{start:0,end:this.length-1,type:c,added:[],removed:[]},null);this.fireDataEvent(r,{value:this.__oT,name:n+(this.length-1),old:R,item:this});}
,unshift:function(T){for(var i=arguments.length-1;i>=0;i-- ){this.__oT.unshift(arguments[i]);this.__oU();this.__oV(0,this.length);this.fireDataEvent(r,{value:[this.__oT[0]],name:p,old:[this.__oT[1]],item:this});this.fireDataEvent(o,{start:0,end:this.length-1,type:b,added:[arguments[i]],removed:[]},null);}
;return this.length;}
,toArray:function(){return this.__oT;}
,getItem:function(U){return this.__oT[U];}
,setItem:function(V,X){var W=this.__oT[V];if(W===X){return;}
;this.__oT[V]=X;this._registerEventChaining(X,W,V);if(this.length!=this.__oT.length){this.__oU();}
...
,indexOf:function(Y){return this.__oT.indexOf(Y);}
,lastIndexOf:function(ba){return this.__oT.lastIndexOf(ba);}
,toString:function(){if(this.__oT!=null){return this.__oT.toString();}
;return f;}
,contains:function(bb){return this.__oT.indexOf(bb)!==-1;}
...
,removeAll:function(){for(var i=0;i<this.__oT.length;i++ ){this._registerEventChaining(null,this.__oT[i],i);}
;if(this.getLength()==0){return [];}
;var bo=this.getLength();var bn=this.__oT.concat();this.__oT.length=0;this.__oU();this.fireDataEvent(r,{value:[],name:n+(bo-1),old:bn,item:this});this.fireDataEvent(o,{start:0,end:bo-1,type:g,removed:bn,added:[]},null);return bn;}
...
;var bq=this.__oT.length;Array.prototype.push.apply(this.__oT,bp);for(var i=0;i<bp.length;i++ ){this._registerEventChaining(bp[i],null,bq+i);}
;var bq=this.length;this.__oU();var name=bq==(this.length-1)?bq:bq+a+(this.length-1);this.fireDataEvent(r,{value:bp,name:name+f,old:[],item:this});this.fireDataEvent(o,{start:bq,end:this.length-1,type:b,added:bp,removed:[]},null);}
...
,forEach:function(bx,by){for(var i=0;i<this.__oT.length;i++ ){bx.call(by,this.__oT[i],i,this);}
;}
,filter:function(bz,self){return new qx.data.Array(this.__oT.filter(bz,self));}
,map:function(bA,self){return new qx.data.Array(this.__oT.map(bA,self));}
,some:function(bB,self){return this.__oT.some(bB,self);}
,every:function(bC,self){return this.__oT.every(bC,self);}
,reduce:function(bE,bD){return this.__oT.reduce(bE,bD);}
,reduceRight:function(bG,bF){return this.__oT.reduceRight(bG,bF);}
,__oU:function(){var bH=this.length;this.length=this.__oT.length;this.fireDataEvent(s,this.length,bH);}
,__oV:function(bJ,bI){for(var i=bJ;i<bI;i++ ){this._registerEventChaining(this.__oT[i],this.__oT[i],i);}
;}
},destruct:function(){for(var i=0;i<this.__oT.length;i++ ){var bK=this.__oT[i];this._applyEventPropagation(null,bK,i);if(this.isAutoDisposeItems()&&bK&&bK instanceof qx.core.Object){bK.dispose();}
;}
;this.__oT=null;}
});}
)();
...
;this.__rW={};var S=this._transport=this._createTransport();this._setPhase(H);this.__rX=qx.lang.Function.bind(this._onReadyStateChange,this);this.__rY=qx.lang.Function.bind(this._onLoad,this);this.__sa=qx.lang.Function.bind(this._onLoadEnd,this);this.__sb=qx.lang.Function.bind(this._onAbort,this);this.__np=qx.lang.Function.bind(this._onTimeout,this);this.__sc=qx.lang.Function.bind(this._onError,this);S.onreadystatechange=this.__rX;S.onload=this.__rY;S.onloadend=this.__sa;S.onabort=this.__sb;S.ontimeout=this.__np;S.onerror=this.__sc;}
...
,nullable:true},authentication:{check:k,nullable:true}},members:{__rX:null,__rY:null,__sa:null,__sb:null,__np:null,__sc:null,__sd:null,__ns:null,__se:null,__rW:null,__sf:null,_transport:null,_createTransport:function(){throw new Error(F);}
...
;this.__ns=true;this.__se=t;this._transport.abort();}
...
;}
,_getAllRequestHeaders:function(){var bd={};qx.lang.Object.mergeWith(bd,this._getConfiguredRequestHeaders());qx.lang.Object.mergeWith(bd,this.__sg());qx.lang.Object.mergeWith(bd,this.__sf);qx.lang.Object.mergeWith(bd,this.__rW);return bd;}
,__sg:function(){var bf=this.getAuthentication(),be={};if(bf){bf.getAuthHeaders().forEach(function(bg){be[bg.key]=bg.value;}
);return be;}
;}
,setRequestHeader:function(bh,bi){this.__rW[bh]=bi;}
,getRequestHeader:function(bj){return this.__rW[bj];}
,removeRequestHeader:function(bk){if(this.__rW[bk]){delete this.__rW[bk];}
;}
...
;this.fireEvent(E);if(this.__ns){return;}
...
;return C;}
;return B;}
,equals:function(F,G){return qx.lang.Object.__dt(F,G,[],[]);}
,__dt:function(N,J,H,I){if(N===J){return N!==0||1/N==1/J;}
...
;H.push(N);I.push(J);var Q=0,O=true;if(M==j){Q=N.length;O=Q==J.length;if(O){while(Q-- ){if(!(O=qx.lang.Object.__dt(N[Q],J[Q],H,I))){break;}
;}
;}
;}
else {for(var P in N){if(Object.prototype.hasOwnProperty.call(N,P)){Q++ ;if(!(O=Object.prototype.hasOwnProperty.call(J,P)&&qx.lang.Object.__dt(N[P],J[P],H,I))){break;}
;}
;}
...
(function(){var a="Retry open native request with method: ",b="activex",c="No XHR support available.",d="If-None-Match",f="xhr",g="If-Modified-Since",h="engine.version",i="onunload",j="GET",k="-1",l="qx.debug.io",m="Undefined",n="Open native request with method: ",o="error",p="loadend",q="Blob",r="HTMLDocument",s="progress",t="load",u="Received native readyState: ",v="abort",w=", url: ",x="String",y="browser.documentmode",z="",A="readystatechange",B="Microsoft.XMLHTTP",C="Already disposed",D="browser.version",E="opera",F="qx.bom.request.Xhr",G="on",H="timeout",I="gecko",J="If-Match",K="mshtml",L="engine.name",M="Microsoft.XMLDOM",N="file:",O="Send native request",P="FormData",Q="If-Range",R="Content-Type",S="io.xhr",T="Not enough arguments",U="ArrayBuffer",V="undefined",W=", async: ",X="Native XHR object doesn't support overrideMimeType.";qx.Bootstrap.define(F,{extend:Object,construct:function(){var Y=qx.Bootstrap.bind(this.__sv,this);if(qx.event&&qx.event.GlobalError&&qx.event.GlobalError.observeMethod){this.__sj=qx.event.GlobalError.observeMethod(Y);}
else {this.__sj=Y;}
;this.__sk=qx.Bootstrap.bind(this.__su,this);this.__VL=qx.Bootstrap.bind(this.__VM,this);this.__np=qx.Bootstrap.bind(this.__sz,this);this.__st();this._emitter=new qx.event.Emitter();if(window.attachEvent){this.__sl=qx.Bootstrap.bind(this.__sC,this);window.attachEvent(i,this.__sl);}
;}
,statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},events:{"readystatechange":F,"error":F,"loadend":F,"timeout":F,"abort":F,"load":F,"progress":F},members:{readyState:0,responseText:z,responseXML:null,status:0,statusText:z,timeout:0,progress:null,open:function(be,ba,bb,bd,bc){this.__sE();if(typeof ba===V){throw new Error(T);}
else if(typeof be===V){be=j;}
;this.__ns=false;this.__sm=false;this.__sn=false;this.__nt=ba;if(typeof bb==V){bb=true;}
;this.__so=bb;if(!this.__sD()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();this.__st();}
;this.__sq.onreadystatechange=this.__sj;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,n+be+w+ba+W+bb);}
;this.__sq.open(be,ba,bb,bd,bc);}
catch(bf){if(!qx.util.Request.isCrossDomain(ba)){throw bf;}
;if(!this.__so){this.__sp=bf;}
;if(this.__so){if(window.XDomainRequest){this.readyState=4;this.__sq=new XDomainRequest();this.__sq.onerror=qx.Bootstrap.bind(function(){this._emit(A);this._emit(o);this._emit(p);}
,this);if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,a+be+w+ba+W+bb);}
;this.__sq.open(be,ba,bb,bd,bc);return;}
;window.setTimeout(qx.Bootstrap.bind(function(){if(this.__nu){return;}
;this.readyState=4;this._emit(A);this._emit(o);this._emit(p);}
,this));}
;}
;if(qx.core.Environment.get(L)===K&&qx.core.Environment.get(y)<9&&this.__sq.readyState>0){this.__sq.setRequestHeader(g,k);}
;if(qx.core.Environment.get(L)===I&&parseInt(qx.core.Environment.get(h),10)<2&&!this.__so){this.readyState=qx.bom.request.Xhr.OPENED;this._emit(A);}
;}
,setRequestHeader:function(bg,bh){this.__sE();if(bg==J||bg==g||bg==d||bg==Q){this.__sn=true;}
;this.__sq.setRequestHeader(bg,bh);return this;}
,send:function(bj){this.__sE();if(!this.__so&&this.__sp){throw this.__sp;}
;if(qx.core.Environment.get(L)===E&&this.timeout===0){this.timeout=10000;}
;if(this.timeout>0){this.__sr=window.setTimeout(this.__np,this.timeout);}
;bj=typeof bj==V?null:bj;var bi=qx.Bootstrap.getClass(bj);bj=(bj!==null&&this.__ss.indexOf(bi)===-1)?bj.toString():bj;try{if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,O);}
;this.__sq.send(bj);}
catch(bl){if(!this.__so){throw bl;}
;if(this._getProtocol()===N){this.readyState=2;this.__sw();var bk=this;window.setTimeout(function(){if(bk.__nu){return;}
;bk.readyState=3;bk.__sw();bk.readyState=4;bk.__sw();}
);}
;}
;if(qx.core.Environment.get(L)===I&&!this.__so){this.__sv();}
;this.__sm=true;return this;}
,abort:function(){this.__sE();this.__ns=true;this.__sq.abort();if(this.__sq){this.readyState=this.__sq.readyState;}
;return this;}
...
,getResponseHeader:function(bo){this.__sE();if(qx.core.Environment.get(y)===9&&this.__sq.aborted){return z;}
;return this.__sq.getResponseHeader(bo);}
,getAllResponseHeaders:function(){this.__sE();if(qx.core.Environment.get(y)===9&&this.__sq.aborted){return z;}
;return this.__sq.getAllResponseHeaders();}
,overrideMimeType:function(bp){this.__sE();if(this.__sq.overrideMimeType){this.__sq.overrideMimeType(bp);}
else {throw new Error(X);}
;return this;}
,getRequest:function(){return this.__sq;}
,dispose:function(){if(this.__nu){return false;}
;window.clearTimeout(this.__sr);if(window.detachEvent){window.detachEvent(i,this.__sl);}
;try{this.__sq.onreadystatechange;}
catch(br){return false;}
;var bq=function(){}
;this.__sq.onreadystatechange=bq;this.__sq.onload=bq;this.__sq.onerror=bq;this.__sq.onprogress=bq;this.abort();this.__sq=null;this.__nu=true;return true;}
,isDisposed:function(){return !!this.__nu;}
...
,_getProtocol:function(){var bt=this.__nt;var bu=/^(\w+:)\/\//;if(bt!==null&&bt.match){var bv=bt.match(bu);if(bv&&bv[1]){return bv[1];}
;}
;return window.location.protocol;}
,__sq:null,__so:null,__sj:null,__sk:null,__VL:null,__sl:null,__np:null,__sm:null,__nt:null,__ns:null,__gs:null,__nu:null,__sr:null,__sp:null,__sn:null,__ss:null,__st:function(){this.__sq=this._createNativeXhr();this.__sq.onreadystatechange=this.__sj;if(qx.Bootstrap.getClass(this.__sq.onabort)!==m){this.__sq.onabort=this.__sk;}
;if(qx.Bootstrap.getClass(this.__sq.onprogress)!==m){this.__sq.onprogress=this.__VL;this.progress={lengthComputable:false,loaded:0,total:0};}
;this.__nu=this.__sm=this.__ns=false;this.__ss=[U,q,r,x,P];}
,__su:function(){if(!this.__ns){this.abort();}
;}
,__VM:function(e){this.progress.lengthComputable=e.lengthComputable;this.progress.loaded=e.loaded;this.progress.total=e.total;this._emit(s);}
,__sv:function(){var bw=this.__sq,bx=true;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Xhr,u+bw.readyState);}
;if(this.readyState==bw.readyState){return;}
;this.readyState=bw.readyState;if(this.readyState===qx.bom.request.Xhr.DONE&&this.__ns&&!this.__sm){return;}
;if(!this.__so&&(bw.readyState==2||bw.readyState==3)){return;}
...
;if(bx){this.__sA();this.__sB();}
;}
;this.__sw();if(this.readyState==qx.bom.request.Xhr.DONE){if(bw){bw.onreadystatechange=function(){}
;}
;}
;}
,__sw:function(){if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__sr);}
;this._emit(A);if(this.readyState===qx.bom.request.Xhr.DONE){this.__sx();}
;}
,__sx:function(){if(this.__gs){this._emit(H);if(qx.core.Environment.get(L)===E){this._emit(o);}
;this.__gs=false;}
else {if(this.__ns){this._emit(v);}
else {if(this.__sy()){this._emit(o);}
else {this._emit(t);}
;}
;}
;this._emit(p);}
,__sy:function(){var bz;if(this._getProtocol()===N){bz=!this.responseText;}
else {bz=!this.statusText;}
;return bz;}
,__sz:function(){var bA=this.__sq;this.readyState=qx.bom.request.Xhr.DONE;this.__gs=true;bA.aborted=true;bA.abort();this.responseText=z;this.responseXML=null;this.__sw();}
,__sA:function(){var bB=this.readyState===qx.bom.request.Xhr.DONE;if(this._getProtocol()===N&&this.status===0&&bB){if(!this.__sy()){this.status=200;}
;}
;if(this.status===1223){this.status=204;}
;if(qx.core.Environment.get(L)===E){if(bB&&this.__sn&&!this.__ns&&this.status===0){this.status=304;}
;}
;}
,__sB:function(){if(qx.core.Environment.get(L)==K&&(this.getResponseHeader(R)||z).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var bC=new window.ActiveXObject(M);bC.async=false;bC.validateOnParse=false;bC.loadXML(this.responseText);this.responseXML=bC;}
;}
,__sC:function(){try{if(this){this.dispose();}
;}
catch(e){}
;}
,__sD:function(){var name=qx.core.Environment.get(L);var bD=qx.core.Environment.get(D);return !(name==K&&bD<9||name==I&&bD<3.5);}
,__sE:function(){if(this.__nu){throw new Error(C);}
;}
...
;}
,statics:{PARSER:{json:qx.lang.Json.parse,xml:qx.xml.Document.fromString}},members:{__sF:null,parse:function(h,g){var f=this._getParser(g);if(typeof f===a){if(h!==c){return f.call(this,h);}
;}
;return h;}
,setParser:function(i){if(typeof qx.util.ResponseParser.PARSER[i]===a){return this.__sF=qx.util.ResponseParser.PARSER[i];}
;if(qx.core.Environment.get(d)){qx.core.Assert.assertFunction(i);}
;return this.__sF=i;}
,_getParser:function(k){var j=this.__sF,m=c,l=c;if(j){return j;}
...
(function(){var a="url: ",b="qx.debug.io",c="qx.bom.request.Script",d="Invalid state",e="head",f="error",g="loadend",h="qx.debug",i="script",j="load",k="Unknown response headers",l="browser.documentmode",m="abort",n="",o="Received native readyState: loaded",p="readystatechange",q="Response header cannot be determined for ",r="requests made with script transport.",s="opera",t="unknown",u="Open native request with ",v="Response headers cannot be determined for",w="mshtml",x="engine.name",y="Detected error",z="Send native request",A="on",B="timeout",C="Unknown environment key at this phase",D="Received native load";qx.Bootstrap.define(c,{construct:function(){this.__nw();this.__nn=qx.Bootstrap.bind(this._onNativeLoad,this);this.__no=qx.Bootstrap.bind(this._onNativeError,this);this.__np=qx.Bootstrap.bind(this._onTimeout,this);this.__nq=document.head||document.getElementsByTagName(e)[0]||document.documentElement;this._emitter=new qx.event.Emitter();this.timeout=this.__ny()?0:15000;}
,events:{"readystatechange":c,"error":c,"loadend":c,"timeout":c,"abort":c,"load":c},members:{readyState:null,status:null,statusText:null,timeout:null,__nr:null,on:function(name,E,F){this._emitter.on(name,E,F);return this;}
,open:function(H,G){if(this.__nu){return;}
;this.__nw();this.__ns=null;this.__nt=G;if(this.__nB(b)){qx.Bootstrap.debug(qx.bom.request.Script,u+a+G);}
;this._readyStateChange(1);}
,setRequestHeader:function(I,J){if(this.__nu){return null;}
;var K={};if(this.readyState!==1){throw new Error(d);}
;K[I]=J;this.__nt=qx.util.Uri.appendParamsToUrl(this.__nt,K);return this;}
,send:function(){if(this.__nu){return null;}
;var M=this.__nz(),L=this.__nq,N=this;if(this.timeout>0){this.__gq=window.setTimeout(this.__np,this.timeout);}
;if(this.__nB(b)){qx.Bootstrap.debug(qx.bom.request.Script,z);}
;L.insertBefore(M,L.firstChild);window.setTimeout(function(){N._readyStateChange(2);N._readyStateChange(3);}
);return this;}
,abort:function(){if(this.__nu){return null;}
;this.__ns=true;this.__nA();this._emit(m);return this;}
...
,onabort:function(){}
,ontimeout:function(){}
,getResponseHeader:function(O){if(this.__nu){return null;}
;if(this.__nB(h)){qx.Bootstrap.debug(q+r);}
;return t;}
,getAllResponseHeaders:function(){if(this.__nu){return null;}
;if(this.__nB(h)){qx.Bootstrap.debug(v+r);}
;return k;}
,setDetermineSuccess:function(P){this.__nr=P;}
,dispose:function(){var Q=this.__nv;if(!this.__nu){if(Q){Q.onload=Q.onreadystatechange=null;this.__nA();}
;if(this.__gq){window.clearTimeout(this.__gq);}
;this.__nu=true;}
;}
,isDisposed:function(){return !!this.__nu;}
,_getUrl:function(){return this.__nt;}
,_getScriptElement:function(){return this.__nv;}
,_onTimeout:function(){this.__nx();if(!this.__ny()){this._emit(f);}
;this._emit(B);if(!this.__ny()){this._emit(g);}
;}
,_onNativeLoad:function(){var S=this.__nv,R=this.__nr,T=this;if(this.__ns){return;}
;if(this.__nB(x)===w&&this.__nB(l)<9){if(!(/loaded|complete/).test(S.readyState)){return;}
else {if(this.__nB(b)){qx.Bootstrap.debug(qx.bom.request.Script,o);}
;}
;}
;if(this.__nB(b)){qx.Bootstrap.debug(qx.bom.request.Script,D);}
;if(R){if(!this.status){this.status=R()?200:500;}
;}
;if(this.status===500){if(this.__nB(b)){qx.Bootstrap.debug(qx.bom.request.Script,y);}
;}
;if(this.__gq){window.clearTimeout(this.__gq);}
;window.setTimeout(function(){T._success();T._readyStateChange(4);T._emit(j);T._emit(g);}
);}
,_onNativeError:function(){this.__nx();this._emit(f);this._emit(g);}
,__nv:null,__nq:null,__nt:n,__nn:null,__no:null,__np:null,__gq:null,__ns:null,__nu:null,__nw:function(){this.readyState=0;this.status=0;this.statusText=n;}
,_readyStateChange:function(U){this.readyState=U;this._emit(p);}
,_success:function(){this.__nA();this.readyState=4;if(!this.status){this.status=200;}
;this.statusText=n+this.status;}
,__nx:function(){this.__nA();this.readyState=4;this.status=0;this.statusText=null;}
,__ny:function(){var W=this.__nB(x)===w&&this.__nB(l)<9;var V=this.__nB(x)===s;return !(W||V);}
,__nz:function(){var X=this.__nv=document.createElement(i);X.src=this.__nt;X.onerror=this.__no;X.onload=this.__nn;if(this.__nB(x)===w&&this.__nB(l)<9){X.onreadystatechange=this.__nn;}
;return X;}
,__nA:function(){var Y=this.__nv;if(Y&&Y.parentNode){this.__nq.removeChild(Y);}
;}
,__nB:function(ba){if(qx&&qx.core&&qx.core.Environment){return qx.core.Environment.get(ba);}
...
(function(){var a="qx.bom.request.Jsonp",b="callback",c="open",d="dispose",e="",f="qx",g="qx.debug",h="Expecting JavaScript response to call: ",i="_onNativeLoad",j=" already exists",k="Callback ",l="qx.debug.io",m=".callback",n="qx.bom.request.Jsonp.";qx.Bootstrap.define(a,{extend:qx.bom.request.Script,construct:function(){qx.bom.request.Script.apply(this);this.__zj();}
,members:{responseJson:null,__cl:null,__zc:null,__zd:null,__ze:null,__zf:null,__zg:null,__nu:null,__zh:e,open:function(t,p){if(this.__nu){return;}
;var r={},q,s,o=this;this.responseJson=null;this.__ze=false;q=this.__zc||b;s=this.__zd||this.__zh+n+this.__cl+m;if(!this.__zd){this.constructor[this.__cl]=this;}
else {if(!window[this.__zd]){this.__zf=true;window[this.__zd]=function(u){o.callback(u);}
;}
else {if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,k+this.__zd+j);}
;}
;}
;if(qx.core.Environment.get(l)){qx.Bootstrap.debug(qx.bom.request.Jsonp,h+s);}
;r[q]=s;this.__zg=p=qx.util.Uri.appendParamsToUrl(p,r);this.__hh(c,[t,p]);}
,callback:function(v){if(this.__nu){return;}
;this.__ze=true;if(qx.core.Environment.get(g)){v=qx.lang.Json.stringify(v);v=qx.lang.Json.parse(v);}
;this.responseJson=v;this.constructor[this.__cl]=undefined;this.__zi();}
,setCallbackParam:function(w){this.__zc=w;return this;}
,setCallbackName:function(name){this.__zd=name;return this;}
,setPrefix:function(x){this.__zh=x;}
,getGeneratedUrl:function(){return this.__zg;}
,dispose:function(){this.__zi();this.__hh(d);}
,_onNativeLoad:function(){this.status=this.__ze?200:500;this.__hh(i);}
,__zi:function(){if(this.__zf&&window[this.__zd]){window[this.__zd]=undefined;this.__zf=false;}
;}
,__hh:function(z,y){qx.bom.request.Script.prototype[z].apply(this,y||[]);}
,__zj:function(){this.__cl=f+(new Date().valueOf())+(e+Math.random()).substring(2,5);}
}});}
)();
...
;qx.log.Logger.debug(this,j+(new Date-qx.Bootstrap.LOADSTART)+k);var q=qx.core.Environment.get(f);var r=qx.Class.getByName(q);if(r){this.__cR=new r;var p=new Date;this.__cR.main();qx.log.Logger.debug(this,c+(new Date-p)+k);var p=new Date;this.__cR.finalize();qx.log.Logger.debug(this,m+(new Date-p)+k);}
...
;}
,_onNative:function(){var q=qx.core.Environment.select(a,{"true":qx.event.GlobalError.observeMethod(this.__cU),"false":this.__cU});q.apply(this,arguments);}
...
(function(){var a="ready",b="mshtml",c="engine.name",d="qx.event.handler.Application",f="complete",g="webkit",h="gecko",i="qx.globalErrorHandling",j="load",k="unload",l="opera",m="left",n="DOMContentLoaded",o="shutdown",p="browser.documentmode";qx.Class.define(d,{extend:qx.core.Object,implement:qx.event.IEventHandler,construct:function(q){qx.core.Object.call(this);this._window=q.getWindow();this.__cV=false;this.__cW=false;this.__cX=false;this.__cY=false;this._initObserver();qx.event.handler.Application.$$instance=this;}
...
,__dc:function(){if(!this.__cY){this.__cY=true;try{qx.event.Registration.fireEvent(this._window,o);}
catch(e){throw e;}
...
;var bc=this.getInnerOpacity();if(bc<1){this.__dz(bd,bc);}
...
;}
,__dz:function(bh,be){if(!qx.core.Environment.get(F)){if(qx.core.Environment.get(m)){qx.log.Logger.warn(A);}
;return;}
...
else if(qx.core.Environment.get(N)&&!qx.core.Environment.get(J)&&qx.core.Environment.get(d)){if(!this.__dE){this.__dE=document.createElement(H);}
;var bd=this.getOrientation()==B;var bo=this.__dF();var bl=bd?200:1;var bf=bd?1:200;var bj=Math.max(100,this.getEndColorPosition()-this.getStartColorPosition());if(bv===c){if(bd){bl=Math.max(bl,this.getEndColorPosition()-this.getStartColorPosition());}
...
;}
;this.__dE.width=bf;this.__dE.height=bl;var bi=this.__dE.getContext(T);if(bd){var bu=bi.createLinearGradient(0,0,0,bl);}
...
;bi.fillStyle=bu;bi.fillRect(0,0,bf,bl);var bk=s+this.__dE.toDataURL()+f;bc[g]=bk;if(bv===U){if(bd){bc[j]=S+bj+U;}
else {bc[j]=bj+F;}
;}
else {bc[j]=bd?bl+z:S+bf+c;}
;}
else if(qx.core.Environment.get(N)&&!qx.core.Environment.get(J)){var bo=this.__dF();var bt=this.getOrientation()==O?1:0;var bs=bo.start;var bm=bo.end;if(!qx.util.ColorUtil.isHex6String(bs)){bs=qx.util.ColorUtil.stringToRgb(bs);bs=qx.util.ColorUtil.rgbToHexString(bs);}
...
;}
;}
,__dF:function(){if(qx.core.Environment.get(x)){var bx=qx.theme.manager.Color.getInstance();var bz=bx.resolve(this.getStartColor());var by=bx.resolve(this.getEndColor());}
...
;var j=this.getTheme();if(j!==null&&j.fonts[l]){var m=this.__dI(j.fonts[l]);return k[l]=(new m).set(j.fonts[l]);}
;return l;}
...
(function(){var a="m",b="os.name",c=")",d="os.version",e="qx.bom.webfonts.Manager",f="svg",g="chrome",h="browser.name",k="singleton",n=",\n",o="src: ",p="mobileSafari",q="'eot)",r="');",s="qx.debug",t="interval",u="#",v="firefox",w="!",y="eot",z="ios",A="changeStatus",B="\.(",C="}\n",D="font-family: ",E="browser.documentmode",F="mobile safari",G="safari",H="@font-face.*?",I="",J="ttf",K=";\n",L="') format('svg')",M="') format('woff')",N="('embedded-opentype')",O="browser.version",P="opera",Q="engine.version",R="Couldn't create @font-face rule for WebFont ",S="mshtml",T="engine.name",U="url('",V="src: url('",W="('embedded-opentype)",X="\nfont-style: normal;\nfont-weight: normal;",Y="?#iefix') format('embedded-opentype')",bl="woff",bm="ie",bn=";",bh="'eot')",bi="Error while adding @font-face rule:",bj="@font-face {",bk="') format('truetype')";qx.Class.define(e,{extend:qx.core.Object,type:k,construct:function(){qx.core.Object.call(this);this.__dM=[];this.__dN={};this.__dO=[];this.__dP=this.getPreferredFormats();}
,statics:{FONT_FORMATS:[y,bl,J,f],VALIDATION_TIMEOUT:5000},members:{__dM:null,__dQ:null,__dN:null,__dP:null,__dO:null,__dR:null,require:function(bq,br,bs,bu){var bp=[];for(var i=0,l=br.length;i<l;i++ ){var bt=br[i].split(u);var bo=qx.util.ResourceManager.getInstance().toUri(bt[0]);if(bt.length>1){bo=bo+u+bt[1];}
;bp.push(bo);}
...
;this.__dO.push([bq,bp,bs,bu]);}
else {this.__dS(bq,bp,bs,bu);}
;}
,remove:function(bw){var bv=null;for(var i=0,l=this.__dM.length;i<l;i++ ){if(this.__dM[i]==bw){bv=i;this.__ea(bw);break;}
;}
;if(bv){qx.lang.Array.removeAt(this.__dM,bv);}
;if(bw in this.__dN){this.__dN[bw].dispose();delete this.__dN[bw];}
;}
...
;this.__dQ=null;}
,__dS:function(bE,bG,bD,bH){if(!qx.lang.Array.contains(this.__dM,bE)){var bI=this.__dV(bG);var bF=this.__dW(bE,bI);if(!bF){throw new Error(R+bE+w);}
...
;}
;this.__dM.push(bE);}
;if(!this.__dN[bE]){this.__dN[bE]=new qx.bom.webfonts.Validator(bE);this.__dN[bE].setTimeout(qx.bom.webfonts.Manager.VALIDATION_TIMEOUT);this.__dN[bE].addListenerOnce(A,this.__dU,this);}
;if(bD){var bC=bH||window;this.__dN[bE].addListenerOnce(A,bD,bC);}
;this.__dN[bE].validate();}
,__dT:function(){if(this.__dO.length==0){this.__dR.stop();return;}
;var bK=this.__dO.shift();this.__dS.apply(this,bK);}
,__dU:function(bL){var bM=bL.getData();if(bM.valid===false){qx.event.Timer.once(function(){this.remove(bM.family);}
,this,250);}
;}
,__dV:function(bN){var bP=qx.bom.webfonts.Manager.FONT_FORMATS;var bO={};for(var i=0,l=bN.length;i<l;i++ ){var bQ=null;for(var x=0;x<bP.length;x++ ){var bR=new RegExp(B+bP[x]+c);var bS=bR.exec(bN[i]);if(bS){bQ=bS[1];}
;}
;if(bQ){bO[bQ]=bN[i];}
;}
;return bO;}
,__dW:function(bV,bY){var bX=[];var bT=this.__dP.length>0?this.__dP:qx.bom.webfonts.Manager.FONT_FORMATS;for(var i=0,l=bT.length;i<l;i++ ){var bU=bT[i];if(bY[bU]){bX.push(this.__dX(bU,bY[bU]));}
;}
;var bW=o+bX.join(n)+bn;bW=D+bV+K+bW;bW=bW+X;return bW;}
,__dX:function(cb,ca){switch(cb){case y:return U+ca+r+V+ca+Y;case bl:return U+ca+M;case J:return U+ca+bk;case f:return U+ca+L;default:return null;};}
,__dY:function(cd){var cc=bj+cd+C;if(qx.core.Environment.get(h)==bm&&qx.core.Environment.get(E)<9){var ce=this.__eb(this.__dQ.cssText);ce+=cc;this.__dQ.cssText=ce;}
else {this.__dQ.insertRule(cc,this.__dQ.cssRules.length);}
;}
,__ea:function(cf){var ci=new RegExp(H+cf,a);for(var i=0,l=document.styleSheets.length;i<l;i++ ){var cg=document.styleSheets[i];if(cg.cssText){var ch=cg.cssText.replace(/\n/g,I).replace(/\r/g,I);ch=this.__eb(ch);if(ci.exec(ch)){ch=ch.replace(ci,I);}
;cg.cssText=ch;}
else if(cg.cssRules){for(var j=0,m=cg.cssRules.length;j<m;j++ ){var ch=cg.cssRules[j].cssText.replace(/\n/g,I).replace(/\r/g,I);if(ci.exec(ch)){this.__dQ.deleteRule(j);return;}
;}
;}
;}
;}
,__eb:function(cj){return cj.replace(q,bh).replace(W,N);}
},destruct:function(){if(this.__dR){this.__dR.stop();this.__dR.dispose();}
;delete this.__dM;this.removeStyleSheet();for(var ck in this.__dN){this.__dN[ck].dispose();}
...
;var p=new qx.event.Timer(o);p.__ed=m;p.addListener(c,function(e){p.stop();m.call(n,e);p.dispose();n=null;}
,n);p.start();return p;}
},properties:{enabled:{init:true,check:f,apply:k},interval:{check:i,init:1000,apply:b}},members:{__ee:null,__ec:null,_applyInterval:function(r,q){if(this.getEnabled()){this.restart();}
;}
,_applyEnabled:function(t,s){if(s){window.clearInterval(this.__ee);this.__ee=null;}
else if(t){this.__ee=window.setInterval(this.__ec,this.getInterval());}
;}
...
;if(qx.bom.Document.isStandardMode(qx.dom.Node.getWindow(p))){if(!this.__ep(p)){return i;}
;}
;return b;}
...
(function(){var a="css.float",b='cssFloat',c="px",d="Cascaded styles are not supported in this browser!",e="css.appearance",f="pixelRight",g="css.userselect",h="css.boxsizing",i="css.textoverflow",j="qx.debug",k="pixelHeight",l=":",m="pixelTop",n="css.borderimage",o="Invalid argument 'name'",p="pixelLeft",q="css.usermodify",r="qx.bom.element.Style",s="Invalid argument 'smart'",t="",u="pixelBottom",v="Invalid argument 'styles'",w="pixelWidth",x='float',y=";",z="\"\"",A="Invalid argument 'element'",B="style";qx.Bootstrap.define(r,{statics:{__er:null,__es:null,__et:function(){var D={"appearance":qx.core.Environment.get(e),"userSelect":qx.core.Environment.get(g),"textOverflow":qx.core.Environment.get(i),"borderImage":qx.core.Environment.get(n),"float":qx.core.Environment.get(a),"userModify":qx.core.Environment.get(q),"boxSizing":qx.core.Environment.get(h)};this.__es={};for(var C in qx.lang.Object.clone(D)){if(!D[C]){delete D[C];}
else {if(C===x){this.__es[b]=C;}
else {this.__es[C]=qx.bom.Style.getCssName(D[C]);}
;}
;}
;this.__er=D;}
,__eu:function(name){var E=qx.bom.Style.getPropertyName(name);if(E){this.__er[name]=E;}
;return E;}
,__ev:{width:w,height:k,left:p,right:f,top:m,bottom:u},__ew:{clip:qx.bom.element.Clip,cursor:qx.bom.element.Cursor,opacity:qx.bom.element.Opacity,boxSizing:qx.bom.element.BoxSizing},compile:function(F){var I=[];var J=this.__ew;var H=this.__es;var name,G;for(name in F){G=F[name];if(G==null){continue;}
;name=this.__es[name]||name;if(J[name]){I.push(J[name].compile(G));}
...
,isPropertySupported:function(N){return (this.__ew[N]||this.__er[N]||N in document.documentElement.style);}
...
;}
;name=this.__er[name]||this.__eu(name)||name;if(O!==false&&this.__ew[name]){this.__ew[name].set(Q,P);}
...
;}
;var U=this.__er;var Y=this.__ew;var S=X.style;for(var W in R){var T=R[W];var name=U[W]||this.__eu(W)||W;if(T===undefined){if(V!==false&&Y[name]){Y[name].reset(X);}
else {S[name]=t;}
;}
...
;}
;}
,reset:function(bb,name,ba){name=this.__er[name]||this.__eu(name)||name;if(ba!==false&&this.__ew[name]){this.__ew[name].reset(bb);}
else {bb.style[name]=t;}
;}
,get:function(bf,name,bh,bj){name=this.__er[name]||this.__eu(name)||name;if(bj!==false&&this.__ew[name]){return this.__ew[name].get(bf,bh);}
...
;var bk=this.__ev[name];if(bk&&(bk in bf.style)){var bi=bf.style[name];bf.style[name]=bl||0;var be=bf.style[bk]+c;bf.style[name]=bi;return be;}
;return bl;}
;return bf.style[name]||t;};}
},defer:function(bm){bm.__et();}
});}
)();
...
(function(){var a="'.",b="Missing appearance: ",c="_applyTheme",d="qx.debug",e="string",f="Cannot find a matching appearance for '",g="Hint: This may be an issue with nested child controls and a missing alias definition in the appearance theme.",h="qx.theme.manager.Appearance",j=":",k="Theme",l="undefined",m="changeTheme",n="/",o="singleton";qx.Class.define(h,{type:o,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ez={};this.__eA={};}
,properties:{theme:{check:k,nullable:true,event:m,apply:c}},members:{__eB:{},__ez:null,__eA:null,_applyTheme:function(){this.__eA={};this.__ez={};}
,__eC:function(D,z,q,u){var w=z.appearances;var r=w[D];if(!r){var C=n;var s=[];var v=D.split(C);var B=qx.lang.Array.clone(v);var y;while(!r&&v.length>0){s.unshift(v.pop());var t=v.join(C);r=w[t];if(r){y=r.alias||r;if(typeof y===e){var A=y+C+s.join(C);return this.__eC(A,z,q,B);}
;}
;}
;for(var i=0;i<s.length-1;i++ ){s.shift();var x=s.join(C);var p=this.__eC(x,z,null,B);if(p){return p;}
;}
;if(q!=null){return this.__eC(q,z,null,B);}
;if(qx.core.Environment.get(d)){if(typeof u!==l){this.debug(f+u.join(n)+a);if(u.length>1){this.info(g);}
;}
;}
;return null;}
else if(typeof r===e){return this.__eC(r,z,q,B);}
else if(r.include&&!r.style){return this.__eC(r.include,z,q,B);}
;return D;}
,styleFrom:function(W,O,P,F){if(!P){P=this.getTheme();}
;var M=this.__eA;var E=M[W];if(!E){E=M[W]=this.__eC(W,P,F);}
...
;if(J>0){U+=j+J;}
;}
;var K=this.__ez;if(K[U]!==undefined){return K[U];}
;if(!O){O=this.__eB;}
...
;W.include=this.__eD(W.include);W.patch=this.__eD(W.patch);if(qx.core.Environment.get(q)){this.__i(name,W);}
...
;}
,__eE:function(Y,ba){var bb=ba.aliases||{};if(ba.extend&&ba.extend.aliases){qx.Bootstrap.objectMergeWith(bb,ba.extend.aliases,false);}
;Y.aliases=bb;}
...
,__eF:function(bd){for(var i=0,bc=this.__eH,l=bc.length;i<l;i++ ){if(bd[bc[i]]){return bc[i];}
;}
;}
,__eG:function(bi,bj){var bf=this.__eF(bj);if(bj.extend&&!bf){bf=bj.extend.type;}
...
,$$registry:{},__eH:[N,u,n,d,j,s,e,R],__h:qx.core.Environment.select(q,{"true":{"title":m,"aliases":U,"type":m,"extend":U,"colors":U,"borders":U,"decorations":U,"fonts":U,"icons":U,"widgets":U,"appearances":U,"meta":U,"include":U,"patch":U},"default":null}),__eI:qx.core.Environment.select(q,{"true":{"color":U,"border":U,"decoration":U,"font":U,"icon":U,"appearance":U,"widget":U},"default":null}),__i:qx.core.Environment.select(q,{"true":function(name,bq){var bp=this.__h;for(var bo in bq){if(bp[bo]===undefined){throw new Error(f+bo+M+name+h);}
...
;}
;if(bq.meta){var bm;for(var bo in bq.meta){bm=bq.meta[bo];if(this.__eI[bo]===undefined){throw new Error(C+bo+S);}
;if(typeof bm!==this.__eI[bo]){throw new Error(I+bo+L);}
...
;}
,"default":function(){}
}),patch:function(bu,bs){this.__eJ(bs);var bw=this.__eF(bs);if(bw!==this.__eF(bu)){throw new Error(w+bu.name+v+bs.name+b);}
;var bt=bs[bw];var br=bu.$$clazz.prototype;for(var bv in bt){br[bv]=bt[bv];}
;}
,include:function(bA,by){this.__eJ(by);var bC=by.type;if(bC!==bA.type){throw new Error(w+bA.name+v+by.name+b);}
;var bz=by[bC];var bx=bA.$$clazz.prototype;for(var bB in bz){if(bx[bB]!==undefined){continue;}
;bx[bB]=bz[bB];}
;}
,__eJ:function(bD){if(typeof bD===H||bD==null){var bF=new Error(g);if(qx.core.Environment.get(q)){var bE=qx.dev.StackTrace.getStackTraceFromError(bF);qx.Bootstrap.error(this,bE);}
;throw bF;}
;}
}});}
)();
...
;var z=qx.lang.Array.cast(x,w);for(var i=0;i<qxWeb.__ek.length;i++ ){qxWeb.__ek[i].call(z);}
;return z;}
...
;}
,$attachInit:function(H){this.__ek.push(H);}
...
;var v=function(){qx.module.Event.__cX=true;u();}
...
else {var z=function(){if(qx.module.Event.__cX){return;}
...
;var name=this.__fK+this.__cl++ ;var bw=this.__fM[O]+K+name;qx.bom.Stylesheet.addRule(this.__dn,bw,bv);this.__dk[bv]=name;return name;}
...
(function(){var a="qx.util.DeferredCall",b="The context object '",c="qx.debug",d="'is already disposed.",e="' of the defered call '";qx.Class.define(a,{extend:qx.core.Object,construct:function(f,g){qx.core.Object.call(this);this.__bF=f;this.__bH=g||null;this.__gt=qx.util.DeferredCallManager.getInstance();}
,members:{__bF:null,__bH:null,__gt:null,cancel:function(){this.__gt.cancel(this);}
,schedule:function(){this.__gt.schedule(this);}
,call:function(){if(qx.core.Environment.get(c)){var h=this.__bH;if(h&&h.isDisposed&&h.isDisposed()){this.warn(b+h+e+this+d);}
;}
;this.__bH?this.__bF.apply(this.__bH):this.__bF();}
},destruct:function(){this.cancel();this.__bH=this.__bF=this.__gt=null;}
});}
)();
(function(){var a="Child is already in: ",b="text",c="|bubble|",d="qx.html.Element",f="': ",g="|capture|",h="scroll",j="Invalid capture flag.",k="focus",m="Failed to add event listener for type '",n="Flushing elements...",o="class",p="blur",q="div",r="Flush: ",s="deactivate",t="'",u="css.userselect",v="animationEnd",w="Synced DOM with ",z=" from the target '",A="qx.debug",B="capture",C="visible",D="Root elements could not be inserted into other ones.",E="Has no children!",F="off",G="Invalid callback function",H="releaseCapture",I="Could not move to same index!",J="Invalid context for callback.",K="element",L="",M="__gP",N="Flush invisible element",O="Failed to remove event listener for type '",P="qxSelectable",Q="tabIndex",R="Has no child at this position!",S="Invalid event type.",T="qx.html.Iframe",U="activate",V="Has no parent to remove from.",W=" to the target '",X="mshtml",Y="engine.name",bp=" operations",bq="Flush rendered element",br="none",bl="Has no child: ",bm="css.userselect.none",bn=" ",bo="hidden",bs="on",bt="Switching visibility to: ",bu="id",bv="Could not overwrite existing element!";qx.Class.define(d,{extend:qx.core.Object,construct:function(by,bw,bx){qx.core.Object.call(this);this.__gu=by||q;this.__gv=bw||null;this.__gw=bx||null;}
,statics:{DEBUG:false,_modified:{},_visibility:{},_scroll:{},_actions:[],__gx:{},__gy:null,__gz:null,_scheduleFlush:function(bz){qx.html.Element.__hb.schedule();}
,flush:function(){var bL;if(qx.core.Environment.get(A)){if(this.DEBUG){qx.log.Logger.debug(this,n);}
;}
;var bD=this.__gA();var bB=bD.getFocus();if(bB&&this.__gC(bB)){bD.blur(bB);}
;var bS=bD.getActive();if(bS&&this.__gC(bS)){qx.bom.Element.deactivate(bS);}
;var bG=this.__gB();if(bG&&this.__gC(bG)){qx.bom.Element.releaseCapture(bG);}
;var bM=[];var bN=this._modified;for(var bK in bN){bL=bN[bK];if(bL.__gT()||bL.classname==T){if(bL.__gD&&qx.dom.Hierarchy.isRendered(bL.__gD)){bM.push(bL);}
...
;}
;bL.__gS();}
;var bI=this._visibility;for(var bK in bI){bL=bI[bK];var bO=bL.__gD;if(!bO){delete bI[bK];continue;}
;if(qx.core.Environment.get(A)){if(this.DEBUG){qx.log.Logger.debug(this,bt+bL.__gF);}
;}
;if(!bL.$$disposed){bO.style.display=bL.__gF?L:br;if((qx.core.Environment.get(Y)==X)){if(!(document.documentMode>=8)){bO.style.visibility=bL.__gF?C:bo;}
;}
;}
;delete bI[bK];}
;var scroll=this._scroll;for(var bK in scroll){bL=scroll[bK];var bC=bL.__gD;if(bC&&bC.offsetWidth){var bF=true;if(bL.__gI!=null){bL.__gD.scrollLeft=bL.__gI;delete bL.__gI;}
;if(bL.__gJ!=null){bL.__gD.scrollTop=bL.__gJ;delete bL.__gJ;}
;var bP=bL.__gG;if(bP!=null){var bJ=bP.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewX(bJ,bC,bP.align);delete bL.__gG;}
else {bF=false;}
;}
;var bQ=bL.__gH;if(bQ!=null){var bJ=bQ.element.getDomElement();if(bJ&&bJ.offsetWidth){qx.bom.element.Scroll.intoViewY(bJ,bC,bQ.align);delete bL.__gH;}
else {bF=false;}
;}
;if(bF){delete scroll[bK];}
;}
;}
;var bE={"releaseCapture":1,"blur":1,"deactivate":1};for(var i=0;i<this._actions.length;i++ ){var bR=this._actions[i];var bO=bR.element.__gD;if(!bO||!bE[bR.type]&&!bR.element.__gT()){continue;}
;var bH=bR.args;bH.unshift(bO);qx.bom.Element[bR.type].apply(qx.bom.Element,bH);}
;this._actions=[];for(var bK in this.__gx){var bA=this.__gx[bK];var bC=bA.element.__gD;if(bC){qx.bom.Selection.set(bC,bA.start,bA.end);delete this.__gx[bK];}
;}
;qx.event.handler.Appear.refresh();}
,__gA:function(){if(!this.__gy){var bT=qx.event.Registration.getManager(window);this.__gy=bT.getHandler(qx.event.handler.Focus);}
;return this.__gy;}
,__gB:function(){if(!this.__gz){var bU=qx.event.Registration.getManager(window);this.__gz=bU.getDispatcher(qx.event.dispatch.MouseCapture);}
;return this.__gz.getCaptureElement();}
,__gC:function(bV){var bW=qx.core.ObjectRegistry.fromHashCode(bV.$$element);return bW&&!bW.__gT();}
...
;this.__gQ=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
,_createDomElement:function(){return qx.dom.Element.create(this.__gu);}
,__gS:function(){if(qx.core.Environment.get(A)){if(this.DEBUG){this.debug(r+this.getAttribute(bu));}
;}
;var length;var bX=this.__gP;if(bX){length=bX.length;var bY;for(var i=0;i<length;i++ ){bY=bX[i];if(bY.__gF&&bY.__gE&&!bY.__gD){bY.__gS();}
;}
;}
;if(!this.__gD){this.__gD=this._createDomElement();this.__gD.$$element=this.$$hash;this._copyData(false);if(bX&&length>0){this._insertChildren();}
;}
else {this._syncData();if(this.__gQ){this._syncChildren();}
;}
;delete this.__gQ;}
,_insertChildren:function(){var ca=this.__gP;var length=ca.length;var cc;if(length>2){var cb=document.createDocumentFragment();for(var i=0;i<length;i++ ){cc=ca[i];if(cc.__gD&&cc.__gE){cb.appendChild(cc.__gD);}
;}
;this.__gD.appendChild(cb);}
else {var cb=this.__gD;for(var i=0;i<length;i++ ){cc=ca[i];if(cc.__gD&&cc.__gE){cb.appendChild(cc.__gD);}
;}
;}
;}
,_syncChildren:function(){var cm=qx.core.ObjectRegistry;var cd=this.__gP;var ck=cd.length;var ce;var ci;var cg=this.__gD;var cl=cg.childNodes;var cf=0;var cj;if(qx.core.Environment.get(A)){var ch=0;}
;for(var i=cl.length-1;i>=0;i-- ){cj=cl[i];ci=cm.fromHashCode(cj.$$element);if(!ci||!ci.__gE||ci.__gR!==this){cg.removeChild(cj);if(qx.core.Environment.get(A)){ch++ ;}
;}
;}
;for(var i=0;i<ck;i++ ){ce=cd[i];if(ce.__gE){ci=ce.__gD;cj=cl[cf];if(!ci){continue;}
...
;}
;}
,_copyData:function(co){var cq=this.__gD;var cs=this.__gw;if(cs){var cp=qx.bom.element.Attribute;for(var cr in cs){cp.set(cq,cr,cs[cr]);}
;}
;var cs=this.__gv;if(cs){var cn=qx.bom.element.Style;if(co){cn.setStyles(cq,cs);}
else {cn.setCss(cq,cn.compile(cs));}
;}
;var cs=this.__gN;if(cs){for(var cr in cs){this._applyProperty(cr,cs[cr]);}
;}
;var cs=this.__gO;if(cs){qx.event.Registration.getManager(cq).importListeners(cq,cs);delete this.__gO;}
;}
,_syncData:function(){var cx=this.__gD;var cw=qx.bom.element.Attribute;var cu=qx.bom.element.Style;var cv=this.__gL;if(cv){var cA=this.__gw;if(cA){var cy;for(var cz in cv){cy=cA[cz];if(cy!==undefined){cw.set(cx,cz,cy);}
else {cw.reset(cx,cz);}
;}
;}
;this.__gL=null;}
;var cv=this.__gK;if(cv){var cA=this.__gv;if(cA){var ct={};for(var cz in cv){ct[cz]=cA[cz];}
;cu.setStyles(cx,ct);}
;this.__gK=null;}
;var cv=this.__gM;if(cv){var cA=this.__gN;if(cA){var cy;for(var cz in cv){this._applyProperty(cz,cA[cz]);}
;}
;this.__gM=null;}
;}
,__gT:function(){var cB=this;while(cB){if(cB.__dd){return true;}
;if(!cB.__gE||!cB.__gF){return false;}
;cB=cB.__gR;}
;return false;}
,__gU:function(cC){if(cC.__gR===this){throw new Error(a+cC);}
;if(cC.__dd){throw new Error(D);}
;if(cC.__gR){cC.__gR.remove(cC);}
;cC.__gR=this;if(!this.__gP){this.__gP=[];}
;if(this.__gD){this._scheduleChildrenUpdate();}
;}
,__gV:function(cD){if(cD.__gR!==this){throw new Error(bl+cD);}
;if(this.__gD){this._scheduleChildrenUpdate();}
;delete cD.__gR;}
,__gW:function(cE){if(cE.__gR!==this){throw new Error(bl+cE);}
;if(this.__gD){this._scheduleChildrenUpdate();}
;}
,getChildren:function(){return this.__gP||null;}
,getChild:function(cF){var cG=this.__gP;return cG&&cG[cF]||null;}
,hasChildren:function(){var cH=this.__gP;return cH&&cH[0]!==undefined;}
,indexOf:function(cJ){var cI=this.__gP;return cI?cI.indexOf(cJ):-1;}
,hasChild:function(cL){var cK=this.__gP;return cK&&cK.indexOf(cL)!==-1;}
,add:function(cM){if(arguments[1]){for(var i=0,l=arguments.length;i<l;i++ ){this.__gU(arguments[i]);}
;this.__gP.push.apply(this.__gP,arguments);}
else {this.__gU(cM);this.__gP.push(cM);}
;return this;}
,addAt:function(cO,cN){this.__gU(cO);qx.lang.Array.insertAt(this.__gP,cO,cN);return this;}
,remove:function(cP){var cQ=this.__gP;if(!cQ){return this;}
;if(arguments[1]){var cR;for(var i=0,l=arguments.length;i<l;i++ ){cR=arguments[i];this.__gV(cR);qx.lang.Array.remove(cQ,cR);}
;}
else {this.__gV(cP);qx.lang.Array.remove(cQ,cP);}
;return this;}
,removeAt:function(cS){var cT=this.__gP;if(!cT){throw new Error(E);}
;var cU=cT[cS];if(!cU){throw new Error(R);}
;this.__gV(cU);qx.lang.Array.removeAt(this.__gP,cS);return this;}
,removeAll:function(){var cV=this.__gP;if(cV){for(var i=0,l=cV.length;i<l;i++ ){this.__gV(cV[i]);}
;cV.length=0;}
;return this;}
,getParent:function(){return this.__gR||null;}
,insertInto:function(parent,cW){parent.__gU(this);if(cW==null){parent.__gP.push(this);}
else {qx.lang.Array.insertAt(this.__gP,this,cW);}
;return this;}
,insertBefore:function(cX){var parent=cX.__gR;parent.__gU(this);qx.lang.Array.insertBefore(parent.__gP,this,cX);return this;}
,insertAfter:function(cY){var parent=cY.__gR;parent.__gU(this);qx.lang.Array.insertAfter(parent.__gP,this,cY);return this;}
,moveTo:function(da){var parent=this.__gR;parent.__gW(this);var dc=parent.__gP.indexOf(this);if(dc===da){throw new Error(I);}
else if(dc<da){da-- ;}
;qx.lang.Array.removeAt(parent.__gP,dc);qx.lang.Array.insertAt(parent.__gP,this,da);return this;}
,moveBefore:function(dd){var parent=this.__gR;return this.moveTo(parent.__gP.indexOf(dd));}
,moveAfter:function(de){var parent=this.__gR;return this.moveTo(parent.__gP.indexOf(de)+1);}
,free:function(){var parent=this.__gR;if(!parent){throw new Error(V);}
...
,useMarkup:function(dg){if(this.__gD){throw new Error(bv);}
;if(qx.core.Environment.get(Y)==X){var dh=document.createElement(q);}
else {var dh=qx.dom.Element.getHelperElement();}
;dh.innerHTML=dg;this.useElement(dh.firstChild);return this.__gD;}
,useElement:function(di){if(this.__gD){throw new Error(bv);}
;this.__gD=di;this.__gD.$$element=this.$$hash;this._copyData(true);}
,isFocusable:function(){var dk=this.getAttribute(Q);if(dk>=1){return true;}
;var dj=qx.event.handler.Focus.FOCUSABLE_ELEMENTS;if(dk>=0&&dj[this.__gu]){return true;}
;return false;}
...
,fadeIn:function(dn){var dp=qxWeb(this.__gD);if(dp.isPlaying()){dp.stop();}
;if(!this.__gD){this.__gS();dp.push(this.__gD);}
;if(this.__gD){dp.fadeIn(dn);return dp.getAnimationHandles()[0];}
;}
,fadeOut:function(dq){var dr=qxWeb(this.__gD);if(dr.isPlaying()){dr.stop();}
;if(this.__gD){dr.fadeOut(dq).once(v,function(){this.hide();qx.html.Element.flush();}
,this);return dr.getAnimationHandles()[0];}
;}
,show:function(){if(this.__gF){return this;}
;if(this.__gD){qx.html.Element._visibility[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
...
,scrollChildIntoViewX:function(dv,dt,dw){var ds=this.__gD;var du=dv.getDomElement();if(dw!==false&&ds&&ds.offsetWidth&&du&&du.offsetWidth){qx.bom.element.Scroll.intoViewX(du,ds,dt);}
else {this.__gG={element:dv,align:dt};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__gI;}
,scrollChildIntoViewY:function(dA,dy,dB){var dx=this.__gD;var dz=dA.getDomElement();if(dB!==false&&dx&&dx.offsetWidth&&dz&&dz.offsetWidth){qx.bom.element.Scroll.intoViewY(dz,dx,dy);}
else {this.__gH={element:dA,align:dy};qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__gJ;}
,scrollToX:function(x,dC){var dD=this.__gD;if(dC!==true&&dD&&dD.offsetWidth){dD.scrollLeft=x;delete this.__gI;}
else {this.__gI=x;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__gG;}
,getScrollX:function(){var dE=this.__gD;if(dE){return dE.scrollLeft;}
;return this.__gI||0;}
,scrollToY:function(y,dG){var dF=this.__gD;if(dG!==true&&dF&&dF.offsetWidth){dF.scrollTop=y;delete this.__gJ;}
else {this.__gJ=y;qx.html.Element._scroll[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;delete this.__gH;}
,getScrollY:function(){var dH=this.__gD;if(dH){return dH.scrollTop;}
;return this.__gJ||0;}
,disableScrolling:function(){this.enableScrolling();this.scrollToX(0);this.scrollToY(0);this.addListener(h,this.__gY,this);}
,enableScrolling:function(){this.removeListener(h,this.__gY,this);}
,__gX:null,__gY:function(e){if(!this.__gX){this.__gX=true;this.__gD.scrollTop=0;this.__gD.scrollLeft=0;delete this.__gX;}
;}
,getTextSelection:function(){var dI=this.__gD;if(dI){return qx.bom.Selection.get(dI);}
;return null;}
,getTextSelectionLength:function(){var dJ=this.__gD;if(dJ){return qx.bom.Selection.getLength(dJ);}
;return null;}
,getTextSelectionStart:function(){var dK=this.__gD;if(dK){return qx.bom.Selection.getStart(dK);}
;return null;}
,getTextSelectionEnd:function(){var dL=this.__gD;if(dL){return qx.bom.Selection.getEnd(dL);}
;return null;}
,setTextSelection:function(dM,dN){var dO=this.__gD;if(dO){qx.bom.Selection.set(dO,dM,dN);return;}
;qx.html.Element.__gx[this.toHashCode()]={element:this,start:dM,end:dN};qx.html.Element._scheduleFlush(K);}
,clearTextSelection:function(){var dP=this.__gD;if(dP){qx.bom.Selection.clear(dP);}
;delete qx.html.Element.__gx[this.toHashCode()];}
,__ha:function(dQ,dR){var dS=qx.html.Element._actions;dS.push({type:dQ,element:this,args:dR||[]});qx.html.Element._scheduleFlush(K);}
,focus:function(){this.__ha(k);}
,blur:function(){this.__ha(p);}
,activate:function(){this.__ha(U);}
,deactivate:function(){this.__ha(s);}
,capture:function(dT){this.__ha(B,[dT!==false]);}
,releaseCapture:function(){this.__ha(H);}
,setStyle:function(dU,dV,dW){if(!this.__gv){this.__gv={};}
;if(this.__gv[dU]==dV){return this;}
;if(dV==null){delete this.__gv[dU];}
else {this.__gv[dU]=dV;}
;if(this.__gD){if(dW){qx.bom.element.Style.set(this.__gD,dU,dV);return this;}
;if(!this.__gK){this.__gK={};}
;this.__gK[dU]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
,setStyles:function(dY,eb){var ea=qx.bom.element.Style;if(!this.__gv){this.__gv={};}
;if(this.__gD){if(!this.__gK){this.__gK={};}
;for(var dX in dY){var ec=dY[dX];if(this.__gv[dX]==ec){continue;}
;if(ec==null){delete this.__gv[dX];}
else {this.__gv[dX]=ec;}
;if(eb){ea.set(this.__gD,dX,ec);continue;}
;this.__gK[dX]=true;}
;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
else {for(var dX in dY){var ec=dY[dX];if(this.__gv[dX]==ec){continue;}
;if(ec==null){delete this.__gv[dX];}
else {this.__gv[dX]=ec;}
;}
;}
...
,getStyle:function(ef){return this.__gv?this.__gv[ef]:null;}
,getAllStyles:function(){return this.__gv||null;}
,setAttribute:function(eg,eh,ei){if(!this.__gw){this.__gw={};}
;if(this.__gw[eg]==eh){return this;}
;if(eh==null){delete this.__gw[eg];}
else {this.__gw[eg]=eh;}
;if(this.__gD){if(ei){qx.bom.element.Attribute.set(this.__gD,eg,eh);return this;}
;if(!this.__gL){this.__gL={};}
;this.__gL[eg]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
...
,getAttribute:function(ep){return this.__gw?this.__gw[ep]:null;}
...
,_setProperty:function(et,eu,ev){if(!this.__gN){this.__gN={};}
;if(this.__gN[et]==eu){return this;}
;if(eu==null){delete this.__gN[et];}
else {this.__gN[et]=eu;}
;if(this.__gD){if(ev){this._applyProperty(et,eu);return this;}
;if(!this.__gM){this.__gM={};}
;this.__gM[et]=true;qx.html.Element._modified[this.$$hash]=this;qx.html.Element._scheduleFlush(K);}
;return this;}
,_removeProperty:function(ex,ew){return this._setProperty(ex,null,ew);}
,_getProperty:function(ez){var ey=this.__gN;if(!ey){return null;}
...
;}
;if(this.__gD){return qx.event.Registration.addListener(this.__gD,eF,eC,self,eB);}
;if(!this.__gO){this.__gO={};}
;if(eB==null){eB=false;}
;var eE=qx.event.Manager.getNextUniqueId();var eG=eF+(eB?g:c)+eE;this.__gO[eG]={type:eF,listener:eC,self:self,capture:eB,unique:eE};return eG;}
,removeListener:function(eO,eI,self,eH){if(this.$$disposed){return null;}
;if(qx.core.Environment.get(A)){var eM=O+eO+t+z+this+f;this.assertString(eO,eM+S);this.assertFunction(eI,eM+G);if(self!==undefined){this.assertObject(self,J);}
;if(eH!==undefined){this.assertBoolean(eH,j);}
;}
;if(this.__gD){if(eI.$$wrapped_callback&&eI.$$wrapped_callback[eO+this.$$hash]){var eJ=eI.$$wrapped_callback[eO+this.$$hash];delete eI.$$wrapped_callback[eO+this.$$hash];eI=eJ;}
;qx.event.Registration.removeListener(this.__gD,eO,eI,self,eH);}
else {var eK=this.__gO;var eN;if(eH==null){eH=false;}
...
;if(this.__gD){qx.event.Registration.removeListenerById(this.__gD,eP);}
else {delete this.__gO[eP];}
;return this;}
,hasListener:function(eR,eQ){if(this.$$disposed){return false;}
;if(this.__gD){return qx.event.Registration.hasListener(this.__gD,eR,eQ);}
;var eS=this.__gO;var eU;if(eQ==null){eQ=false;}
...
;var eV=[];for(var eX in this.__gO){var eW=this.__gO[eX];eV.push({type:eW.type,handler:eW.listener,self:eW.self,capture:eW.capture});}
;return eV;}
},defer:function(eY){eY.__hb=new qx.util.DeferredCall(eY.flush,eY);}
,destruct:function(){if(this.$$hash){delete qx.html.Element._modified[this.$$hash];delete qx.html.Element._scroll[this.$$hash];}
;var fa=this.__gD;if(fa){qx.event.Registration.getManager(fa).removeAllListeners(fa);fa.$$element=L;}
;if(!qx.core.ObjectRegistry.inShutDown){var parent=this.__gR;if(parent&&!parent.$$disposed){parent.remove(this);}
;}
;this._disposeArray(M);this.__gw=this.__gv=this.__gO=this.__gN=this.__gL=this.__gK=this.__gM=this.__gD=this.__gR=this.__gG=this.__gH=null;}
});}
)();
...
(function(){var a=" due to exceptions in user code. The application has to be reloaded!",b="Error in the 'Appearance' queue:",c="Error in the 'Widget' queue:",d="\n",f="Error in the 'Layout' queue:",g="Error in the 'Visibility' queue:",h="qx.debug",i="useraction",j="qx.debug.ui.queue",k="Error while layout flush: ",l="Error in the 'Dispose' queue:",m="Stack trace: \n",n="event.touch",o="qx.ui.core.queue.Manager",p=" times in a row",q="Fatal Error: Flush terminated ";qx.Class.define(o,{statics:{__ji:false,__gl:false,__jj:{},__jk:0,MAX_RETRIES:10,scheduleFlush:function(r){var self=qx.ui.core.queue.Manager;self.__jj[r]=true;if(!self.__ji){self.__gl=false;qx.bom.AnimationFrame.request(function(){if(self.__gl){self.__gl=false;return;}
;self.flush();}
,self);self.__ji=true;}
;}
,flush:function(){var self=qx.ui.core.queue.Manager;if(self.__jl){return;}
;self.__jl=true;self.__gl=true;var s=self.__jj;self.__jm(function(){while(s.visibility||s.widget||s.appearance||s.layout||s.element){if(s.widget){delete s.widget;if(qx.core.Environment.get(j)){try{qx.ui.core.queue.Widget.flush();}
...
;this.__jt=fl;qx.ui.core.queue.Layout.add(this);}
,setLayoutParent:function(parent){if(this.$$parent===parent){return;}
...
;if(fw||ft.local||ft.margin){if(this.__jt&&this.hasLayoutChildren()){var fr=this.getInsets();var innerWidth=fp-fr.left-fr.right;var innerHeight=fn-fr.top-fr.bottom;var fv=this.getDecorator();var fq={left:0,right:0,top:0,bottom:0};if(fv){fv=qx.theme.manager.Decoration.getInstance().resolve(fv);fq=fv.getPadding();}
;var fo={top:this.getPaddingTop()+fq.top,right:this.getPaddingRight()+fq.right,bottom:this.getPaddingBottom()+fq.bottom,left:this.getPaddingLeft()+fq.left};this.__jt.renderLayout(innerWidth,innerHeight,fo);}
...
,__ju:null,clearSeparators:function(){var fy=this.__ju;if(!fy){return;}
;var fz=qx.ui.core.Widget.__jq;var content=this.getContentElement();var fx;for(var i=0,l=fy.length;i<l;i++ ){fx=fy[i];fz.poolObject(fx);content.remove(fx.getContentElement());}
;fy.length=0;}
,renderSeparator:function(fB,fC){var fA=qx.ui.core.Widget.__jq.getObject(qx.ui.core.Widget);fA.set({decorator:fB});var fE=fA.getContentElement();this.getContentElement().add(fE);var fD=fE.getDomElement();if(fD){fD.style.top=fC.top+dI;fD.style.left=fC.left+dI;fD.style.width=fC.width+dI;fD.style.height=fC.height+dI;}
...
;}
,_getContentHint:function(){var fQ=this.__jt;if(fQ){if(this.hasLayoutChildren()){var fR=fQ.getSizeHint();if(qx.core.Environment.get(eq)){var fP=eb+this.toString()+dA;this.assertInteger(fR.width,T+fP);this.assertInteger(fR.height,ev+fP);}
;return fR;}
...
;return false;}
,__jv:function(){var go=this._createContentElement();go.setAttribute(dt,this.toHashCode());go.setStyles({"touch-action":du,"-ms-touch-action":du});if(qx.core.Environment.get(eq)){go.setAttribute(eu,this.classname);}
...
,__jw:null,getLayoutChildren:function(){var gq=this.__jw;if(!gq){return this.__jx;}
...
;return gr||gq;}
,scheduleLayoutUpdate:function(){qx.ui.core.queue.Layout.add(this);}
,invalidateLayoutChildren:function(){var gs=this.__jt;if(gs){gs.invalidateChildrenCache();}
;qx.ui.core.queue.Layout.add(this);}
,hasLayoutChildren:function(){var gu=this.__jw;if(!gu){return false;}
...
,_indexOf:function(gw){var gv=this.__jw;if(!gv){return -1;}
;return gv.indexOf(gw);}
,_hasChildren:function(){var gx=this.__jw;return gx!=null&&(!!gx[0]);}
,addChildrenToQueue:function(gy){var gz=this.__jw;if(!gz){return;}
...
;if(gC.getLayoutParent()==this){qx.lang.Array.remove(this.__jw,gC);}
;if(this.__jw){this.__jw.push(gC);}
else {this.__jw=[gC];}
;this.__jy(gC,gB);}
,_addAt:function(gG,gD,gF){if(!this.__jw){this.__jw=[];}
;if(gG.getLayoutParent()==this){qx.lang.Array.remove(this.__jw,gG);}
;var gE=this.__jw[gD];if(gE===gG){gG.setLayoutProperties(gF);}
;if(gE){qx.lang.Array.insertBefore(this.__jw,gG,gE);}
else {this.__jw.push(gG);}
;this.__jy(gG,gF);}
...
;if(gH.getLayoutParent()==this){qx.lang.Array.remove(this.__jw,gH);}
;qx.lang.Array.insertBefore(this.__jw,gH,gJ);this.__jy(gH,gI);}
,_addAfter:function(gM,gK,gL){if(qx.core.Environment.get(eq)){this.assertInArray(gK,this._getChildren(),dw);}
;if(gM==gK){return;}
;if(!this.__jw){this.__jw=[];}
;if(gM.getLayoutParent()==this){qx.lang.Array.remove(this.__jw,gM);}
;qx.lang.Array.insertAfter(this.__jw,gM,gK);this.__jy(gM,gL);}
,_remove:function(gN){if(!this.__jw){throw new Error(dx);}
;qx.lang.Array.remove(this.__jw,gN);this.__jz(gN);}
,_removeAt:function(gO){if(!this.__jw){throw new Error(dx);}
;var gP=this.__jw[gO];qx.lang.Array.removeAt(this.__jw,gO);this.__jz(gP);return gP;}
,_removeAll:function(){if(!this.__jw){return [];}
;var gQ=this.__jw.concat();this.__jw.length=0;for(var i=gQ.length-1;i>=0;i-- ){this.__jz(gQ[i]);}
;qx.ui.core.queue.Layout.add(this);return gQ;}
,_afterAddChild:null,_afterRemoveChild:null,__jy:function(gS,gR){if(qx.core.Environment.get(eq)){this.assertInstance(gS,qx.ui.core.LayoutItem,eo+gS);this.assertNotIdentical(gS,this,B+gS);if(gR!=null){this.assertType(gR,k,A+gR);}
;}
...
;}
,__jz:function(gT){if(qx.core.Environment.get(eq)){this.assertNotUndefined(gT);}
;if(gT.getLayoutParent()!==this){throw new Error(X+gT+ef);}
;gT.setLayoutParent(null);if(this.__jt){this.__jt.invalidateChildrenCache();}
...
,_applyPadding:function(ha,gY,name){this._updateInsets=true;qx.ui.core.queue.Layout.add(this);this.__jA(name,ha);}
,__jA:function(hb,he){var content=this.getContentElement();var hc=this.getDecorator();hc=qx.theme.manager.Decoration.getInstance().resolve(hc);if(hc){var hd=qx.Bootstrap.firstLow(hb.replace(eD,eN));he+=hc.getPadding()[hd]||0;}
...
;}
,_applyToolTipText:function(hj,hi){if(qx.core.Environment.get(y)){if(this.__js){return;}
;var hh=qx.locale.Manager.getInstance();this.__js=hh.addListener(eY,function(){var hk=this.getToolTipText();if(hk&&hk.translate){this.setToolTipText(hk.translate());}
;}
,this);}
;}
...
;}
,__jB:null,$$stateChanges:null,_forwardStates:null,hasState:function(hD){var hC=this.__jB;return !!hC&&!!hC[hD];}
,addState:function(hH){var hG=this.__jB;if(!hG){hG=this.__jB={};}
;if(hG[hH]){return;}
;this.__jB[hH]=true;if(hH===dB){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hF=this.__jE;if(forward&&forward[hH]&&hF){var hE;for(var hI in hF){hE=hF[hI];if(hE instanceof qx.ui.core.Widget){hF[hI].addState(hH);}
;}
;}
;}
,removeState:function(hM){var hL=this.__jB;if(!hL||!hL[hM]){return;}
;delete this.__jB[hM];if(hM===dB){this.syncAppearance();}
else if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hK=this.__jE;if(forward&&forward[hM]&&hK){for(var hN in hK){var hJ=hK[hN];if(hJ instanceof qx.ui.core.Widget){hJ.removeState(hM);}
;}
;}
;}
,replaceState:function(hP,hS){var hR=this.__jB;if(!hR){hR=this.__jB={};}
;if(!hR[hS]){hR[hS]=true;}
;if(hR[hP]){delete hR[hP];}
;if(!qx.ui.core.queue.Visibility.isVisible(this)){this.$$stateChanges=true;}
else {qx.ui.core.queue.Appearance.add(this);}
;var forward=this._forwardStates;var hQ=this.__jE;if(forward&&forward[hS]&&hQ){for(var hT in hQ){var hO=hQ[hT];if(hO instanceof qx.ui.core.Widget){hO.replaceState(hP,hS);}
;}
;}
;}
,__jC:null,__jD:null,syncAppearance:function(){var hY=this.__jB;var hX=this.__jC;var ia=qx.theme.manager.Appearance.getInstance();var hV=qx.core.Property.$$method.setThemed;var ig=qx.core.Property.$$method.resetThemed;if(this.__jD){delete this.__jD;if(hX){var hU=ia.styleFrom(hX,hY,null,this.getAppearance());hX=null;}
;}
;if(!hX){var hW=this;var ib=[];do {ib.push(hW.$$subcontrol||hW.getAppearance());}
while(hW=hW.$$subparent);hX=ib.reverse().join(eJ).replace(/#[0-9]+/g,eN);this.__jC=hX;}
...
;}
;this.fireDataEvent(N,this.__jB);}
...
;}
,updateAppearance:function(){this.__jD=true;qx.ui.core.queue.Appearance.add(this);var il=this.__jE;if(il){var ij;for(var ik in il){ij=il[ik];if(ij instanceof qx.ui.core.Widget){ij.updateAppearance();}
;}
;}
;}
...
,_applyEnabled:function(iC,iB){if(iC===false){this.addState(q);this.removeState(dB);if(this.isFocusable()){this.removeState(p);this._applyFocusable(false,true);}
...
;if(!iG){this.removeListener(eU,this._onContextMenuOpen);this.removeListener(dD,this._onContextMenuOpen);iF.removeListener(de,this._onBeforeContextMenuOpen,this);}
;}
;if(iG){iG.setOpener(this);iG.addState(eU);if(!iF){this.addListener(eU,this._onContextMenuOpen);this.addListener(dD,this._onContextMenuOpen);iG.addListener(de,this._onBeforeContextMenuOpen,this);}
;}
;}
...
,hasChildControl:function(ja){if(!this.__jE){return false;}
;return !!this.__jE[ja];}
,__jE:null,_getCreatedChildControls:function(){return this.__jE;}
,getChildControl:function(jd,jc){if(!this.__jE){if(jc){return null;}
;this.__jE={};}
;var jb=this.__jE[jd];if(jb){return jb;}
...
;delete jk.$$subcontrol;delete jk.$$subparent;var jl=this.__jB;var forward=this._forwardStates;if(jl&&forward&&jk instanceof qx.ui.core.Widget){for(var jm in jl){if(forward[jm]){jk.removeState(jm);}
;}
;}
;delete this.__jE[jn];return jk;}
,_createChildControl:function(js){if(!this.__jE){this.__jE={};}
else if(this.__jE[js]){throw new Error(K+js+dJ);}
...
;jo.$$subcontrol=js;jo.$$subparent=this;var jq=this.__jB;var forward=this._forwardStates;if(jq&&forward&&jo instanceof qx.ui.core.Widget){for(var jr in jq){if(forward[jr]){jo.addState(jr);}
;}
;}
;this.fireDataEvent(s,jo);return this.__jE[js]=jo;}
,_createChildControlImpl:function(jv,ju){return null;}
,_disposeChildControls:function(){var jz=this.__jE;if(!jz){return;}
...
;this._disposeArray(dS);this.__jB=this.__jE=null;this._disposeObjects(eV,Q);}
});}
)();
...
(function(){var a="-",b="best-fit",c="size",d="edge-start",e="target.bottom",f="offsets",g="size.width",h='__ke',i="bottom",j="offsets.bottom",k="qx.util.placement.Placement",l="Please use '",m="qx.debug",n="keep-align",o="center",p="target.right",q="direct",r="offsets.right",s="middle",t="target",u="align-start",v="Invalid 'mode' argument!'",w="left",x="align-end",y="Class",z="offsets.left",A="top",B="area",C="right",D="edge-end",E="target.top",F="area.height",G="target.left",H="align-center",I="area.width",J="' instead!",K="size.height",L="offsets.top";qx.Class.define(k,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__ke=qx.util.placement.DirectAxis;}
,properties:{axisX:{check:y},axisY:{check:y},edge:{check:[A,C,i,w],init:A},align:{check:[A,C,i,w,o,s],init:C}},statics:{__kf:null,compute:function(W,P,M,N,V,Q,R){this.__kf=this.__kf||new qx.util.placement.Placement();var T=V.split(a);var S=T[0];var O=T[1];if(qx.core.Environment.get(m)){if(O===o||O===s){var U=s;if(S===A||S===i){U=o;}
...
,__kg:null,__kh:null,__ki:null,__kj:function(X){switch(X){case q:this.__kg=this.__kg||qx.util.placement.DirectAxis;return this.__kg;case n:this.__kh=this.__kh||qx.util.placement.KeepAlignAxis;return this.__kh;case b:this.__ki=this.__ki||qx.util.placement.BestFitAxis;return this.__ki;default:throw new Error(v);};}
},members:{__ke:null,compute:function(be,bb,Y,ba){if(qx.core.Environment.get(m)){this.assertObject(be,c);this.assertNumber(be.width,g);this.assertNumber(be.height,K);this.assertObject(bb,B);this.assertNumber(bb.width,I);this.assertNumber(bb.height,F);this.assertObject(Y,t);this.assertNumber(Y.top,E);this.assertNumber(Y.right,p);this.assertNumber(Y.bottom,e);this.assertNumber(Y.left,G);this.assertObject(ba,f);this.assertNumber(ba.top,L);this.assertNumber(ba.right,r);this.assertNumber(ba.bottom,j);this.assertNumber(ba.left,z);}
...
,__kk:function(){var bh=this.getEdge();var bg=this.getAlign();if(bh==w){return d;}
else if(bh==C){return D;}
else if(bg==w){return u;}
else if(bg==o){return H;}
else if(bg==C){return x;}
;}
,__kl:function(){var bj=this.getEdge();var bi=this.getAlign();if(bj==A){return d;}
...
(function(){var a="Image could not be loaded: ",b="Boolean",c="px",d="http",e=".png",f="background-image",g="engine.version",h="scale",j="changeSource",l="x",m="div",n="aborted",o="background-size",p="nonScaled",q="3",r="qx.ui.basic.Image",s="top",t="0 0",u=", no-repeat",v="qx.debug",w="loaded",x="backgroundImage",y="backgroundRepeat",z="-disabled.$1",A="class",B="qx.event.type.Event",C="image",D="loadingFailed",E="css.alphaimageloaderneeded",F="1.5",G="String",H="browser.documentmode",I="backgroundPosition",J="border-box",K="left",L="_applySource",M="$$widget",N="@",O="data:image/",P="px, ",Q='.',R="scaled",S=", ",T="2",U="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7",V="mshtml",W="engine.name",X=", 0 0",Y="_applyScale",bp="try to load an unmanaged relative image: ",bq="position",br="replacement",bl="img",bm="no-repeat",bn="background-position",bo="hidden",bs="alphaScaled",bt=",",bu="__km",bv="absolute";qx.Class.define(r,{extend:qx.ui.core.Widget,construct:function(bw){this.__km={};qx.ui.core.Widget.call(this);if(bw){this.setSource(bw);}
;}
,properties:{source:{check:G,init:null,nullable:true,event:j,apply:L,themeable:true},scale:{check:b,init:false,themeable:true,apply:Y},appearance:{refine:true,init:C},allowShrinkX:{refine:true,init:false},allowShrinkY:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false}},events:{loadingFailed:B,loaded:B,aborted:B},statics:{PLACEHOLDER_IMAGE:U},members:{__kn:null,__ko:null,__iP:null,__km:null,__kp:null,__kq:null,__kr:0,_onChangeTheme:function(){qx.ui.core.Widget.prototype._onChangeTheme.call(this);this._styleSource();}
...
,_applyDecorator:function(bz,by){qx.ui.core.Widget.prototype._applyDecorator.call(this,bz,by);var bA=this.getSource();bA=qx.util.AliasManager.getInstance().resolve(bA);var bx=this.getContentElement();if(this.__kq){bx=bx.getChild(0);}
;this.__kD(bx,bA);}
,_applyPadding:function(bC,bB,name){qx.ui.core.Widget.prototype._applyPadding.call(this,bC,bB,name);var bD=this.getContentElement();if(this.__kq){bD.getChild(0).setStyles({top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
else {bD.setPadding(this.getPaddingLeft()||0,this.getPaddingTop()||0);}
;}
,renderLayout:function(bG,top,bE,bH){qx.ui.core.Widget.prototype.renderLayout.call(this,bG,top,bE,bH);var bF=this.getContentElement();if(this.__kq){bF.getChild(0).setStyles({width:bE-(this.getPaddingLeft()||0)-(this.getPaddingRight()||0),height:bH-(this.getPaddingTop()||0)-(this.getPaddingBottom()||0),top:this.getPaddingTop()||0,left:this.getPaddingLeft()||0});}
;}
...
,__ks:function(bN){this.__iP=bN;}
,__kt:function(){if(this.__iP==null){var bP=this.getSource();var bO=false;if(bP!=null){bO=qx.lang.String.endsWith(bP,e);}
;if(this.getScale()&&bO&&qx.core.Environment.get(E)){this.__iP=bs;}
else if(this.getScale()){this.__iP=R;}
else {this.__iP=p;}
;}
;return this.__iP;}
,__ku:function(bS){var bR;var bQ;if(bS==bs){bR=true;bQ=m;}
else if(bS==p){bR=false;bQ=m;}
else {bR=true;bQ=bl;}
;var bU=new qx.html.Image(bQ);bU.setAttribute(M,this.toHashCode());bU.setScale(bR);bU.setStyles({"overflowX":bo,"overflowY":bo,"boxSizing":J});if(qx.core.Environment.get(E)){var bT=this.__kq=new qx.html.Element(m);bT.setAttribute(M,this.toHashCode());bT.setStyle(bq,bv);bT.add(bU);return bT;}
;return bU;}
,__kv:function(){if(this.$$disposed){return null;}
;var bV=this.__kt();if(this.__km[bV]==null){this.__km[bV]=this.__ku(bV);}
;var bW=this.__km[bV];if(!this.__kp){this.__kp=bW;}
;return bW;}
,_styleSource:function(){var cc=qx.util.AliasManager.getInstance();var cb=qx.util.ResourceManager.getInstance();var ce=cc.resolve(this.getSource());var cd=this.getContentElement();if(this.__kq){cd=cd.getChild(0);}
;if(!ce){cd.resetSource();return;}
;this.__ky(ce);if((qx.core.Environment.get(W)==V)&&(parseInt(qx.core.Environment.get(g),10)<9||qx.core.Environment.get(H)<9)){var bY=this.getScale()?h:bm;cd.tagNameHint=qx.bom.element.Decoration.getTagName(bY,ce);}
;var ca=this.__kx();if(qx.util.ResourceManager.getInstance().has(ce)){var cf=this._findHighResolutionSource(ce);if(cf){var ch=cb.getImageHeight(ce);var cg=cb.getImageWidth(ce);this.setWidth(ch);this.setHeight(cg);var bX=ch+P+cg+c;this.__kp.setStyle(o,bX);this.setSource(cf);ce=cf;}
;this.__kA(ca,ce);this.__kw();}
else if(qx.io.ImageLoader.isLoaded(ce)){this.__kB(ca,ce);this.__kw();}
else {this.__kC(ca,ce);}
;}
,__kw:function(){this.__kr++ ;qx.bom.AnimationFrame.request(function(ci){if(ci===this.__kr){this.fireEvent(w);}
else {this.fireEvent(n);}
;}
.bind(this,this.__kr));}
,__kx:function(){var cj=this.__kp;if(this.__kq){cj=cj.getChild(0);}
;return cj;}
,__ky:qx.core.Environment.select(W,{"mshtml":function(cl){var cm=qx.core.Environment.get(E);var ck=qx.lang.String.endsWith(cl,e);if(cm&&ck){if(this.getScale()&&this.__kt()!=bs){this.__ks(bs);}
else if(!this.getScale()&&this.__kt()!=p){this.__ks(p);}
;}
else {if(this.getScale()&&this.__kt()!=R){this.__ks(R);}
else if(!this.getScale()&&this.__kt()!=p){this.__ks(p);}
;}
;this.__kz(this.__kv());}
,"default":function(cn){if(this.getScale()&&this.__kt()!=R){this.__ks(R);}
else if(!this.getScale()&&this.__kt(p)){this.__ks(p);}
;this.__kz(this.__kv());}
}),__kz:function(cr){var cq=this.__kp;if(cq!=cr){if(cq!=null){var cC=c;var co={};var cw=this.getBounds();if(cw!=null){co.width=cw.width+cC;co.height=cw.height+cC;}
;var cx=this.getInsets();co.left=parseInt(cq.getStyle(K)||cx.left)+cC;co.top=parseInt(cq.getStyle(s)||cx.top)+cC;co.zIndex=10;var cu=this.__kq?cr.getChild(0):cr;cu.setStyles(co,true);cu.setSelectable(this.getSelectable());if(!cq.isVisible()){cr.hide();}
;if(!cq.isIncluded()){cr.exclude();}
;var cz=cq.getParent();if(cz){var cp=cz.getChildren().indexOf(cq);cz.removeAt(cp);cz.addAt(cr,cp);}
;var ct=cu.getNodeName();cu.setSource(null);var cs=this.__kx();cu.tagNameHint=ct;cu.setAttribute(A,cs.getAttribute(A));qx.html.Element.flush();var cB=cs.getDomElement();var cA=cr.getDomElement();var cy=cq.getListeners()||[];cy.forEach(function(cD){cr.addListener(cD.type,cD.handler,cD.self,cD.capture);}
);if(cB&&cA){var cv=cB.$$hash;cB.$$hash=cA.$$hash;cA.$$hash=cv;}
;this.__kp=cr;}
;}
;}
,__kA:function(cF,cH){var cG=qx.util.ResourceManager.getInstance();if(!this.getEnabled()){var cE=cH.replace(/\.([a-z]+)$/,z);if(cG.has(cE)){cH=cE;this.addState(br);}
else {this.removeState(br);}
;}
;if(cF.getSource()===cH){return;}
;this.__kD(cF,cH);this.__kF(cG.getImageWidth(cH),cG.getImageHeight(cH));}
,__kB:function(cI,cM){var cK=qx.io.ImageLoader;this.__kD(cI,cM);var cL=cK.getWidth(cM);var cJ=cK.getHeight(cM);this.__kF(cL,cJ);}
,__kC:function(cN,cQ){var cR=qx.io.ImageLoader;if(qx.core.Environment.get(v)){var cP=cQ.toLowerCase();var cO=qx.lang.String.startsWith;if(!cO(cP,d)&&!cO(cP,O)){var self=this.self(arguments);if(!self.__Kp){self.__Kp={};}
;if(!self.__Kp[cQ]){this.debug(bp+cQ);self.__Kp[cQ]=true;}
;}
;}
;if(!cR.isFailed(cQ)){cR.load(cQ,this.__kE,this);}
else {if(cN!=null){cN.resetSource();}
;}
;}
,__kD:function(cS,cW){if(cS.getNodeName()==m){var db=qx.theme.manager.Decoration.getInstance().resolve(this.getDecorator());if(db){var cX=(db.getStartColor()&&db.getEndColor());var cV=db.getBackgroundImage();if(cX||cV){var cT=this.getScale()?h:bm;var cU=qx.bom.element.Decoration.getAttributes(cW,cT);var da=db.getStyles(true);var cY={"backgroundImage":cU.style.backgroundImage,"backgroundPosition":(cU.style.backgroundPosition||t),"backgroundRepeat":(cU.style.backgroundRepeat||bm)};if(cV){cY[I]+=bt+da[bn]||t;cY[y]+=S+db.getBackgroundRepeat();}
;if(cX){cY[I]+=X;cY[y]+=u;}
;cY[x]+=bt+da[f];cS.setStyles(cY);return;}
;}
else {cS.setSource(null);}
;}
;cS.setSource(cW);}
,_findHighResolutionSource:function(dc){var df=[q,T,F];var de=parseFloat(qx.bom.client.Device.getDevicePixelRatio().toFixed(2));if(de<=1){return false;}
...
;}
;return null;}
,_getHighResolutionSource:function(dg,dh){var dj=dg.lastIndexOf(Q);if(dj>-1){var di=N+dh+l;var dk=dg.slice(0,dj)+di+dg.slice(dj);if(qx.util.ResourceManager.getInstance().has(dk)){return dk;}
;}
;return null;}
,__kE:function(dl,dm){if(this.$$disposed===true){return;}
...
;this.__kB(this.__kx(),dl);}
,__kF:function(dn,dp){if(dn!==this.__kn||dp!==this.__ko){this.__kn=dn;this.__ko=dp;qx.ui.core.queue.Layout.add(this);}
;}
},destruct:function(){for(var dq in this.__km){if(this.__km.hasOwnProperty(dq)){this.__km[dq].setAttribute(M,null,true);}
;}
;delete this.__kp;if(this.__kq){delete this.__kq;}
;this._disposeMap(bu);}
});}
)();
(function(){var a="mshtml",b='img',c="",d="qx.globalErrorHandling",e="load",f="qx.io.ImageLoader";qx.Bootstrap.define(f,{statics:{__cP:{},__kG:{width:null,height:null},__kH:/\.(png|gif|jpg|jpeg|bmp)\b/i,__kI:/^data:image\/(png|gif|jpg|jpeg|bmp)\b/i,isLoaded:function(g){var h=this.__cP[g];return !!(h&&h.loaded);}
,isFailed:function(j){var k=this.__cP[j];return !!(k&&k.failed);}
,isLoading:function(m){var n=this.__cP[m];return !!(n&&n.loading);}
,getFormat:function(r){var q=this.__cP[r];if(!q||!q.format){var o=this.__kI.exec(r);if(o!=null){var p=(q&&qx.lang.Type.isNumber(q.width)?q.width:this.__kG.width);var s=(q&&qx.lang.Type.isNumber(q.height)?q.height:this.__kG.height);q={loaded:true,format:o[1],width:p,height:s};}
;}
;return q?q.format:null;}
,getSize:function(t){var u=this.__cP[t];return u?{width:u.width,height:u.height}:this.__kG;}
...
;var z=document.createElement(b);var E=qx.lang.Function.listener(this.__kJ,this,z,B);z.onload=E;z.onerror=E;z.src=B;D.element=z;}
;}
...
;}
;this.__cP[F]=null;}
,__kJ:function(){var J=qx.core.Environment.select(d,{"true":qx.event.GlobalError.observeMethod(this.__kK),"false":this.__kK});J.apply(this,arguments);}
,__kK:function(event,L,K){var P=this.__cP[K];if(qx.bom.client.Engine.getName()==a&&parseFloat(qx.bom.client.Engine.getVersion())===11){document.body.appendChild(L);}
;var M=function(Q){return (Q&&Q.height!==0);}
;if(event.type===e&&M(L)){P.loaded=true;P.width=L.width;P.height=L.height;var N=this.__kH.exec(K);if(N!=null){P.format=N[1];}
;}
else {P.failed=true;}
...
(function(){var a="source",b="engine.name",c="",d="mshtml",e="px",f="px ",g="no-repeat",h="backgroundImage",i="scale",j="webkit",k="div",l="qx.html.Image",m="qx/static/blank.gif",n="backgroundPosition";qx.Class.define(l,{extend:qx.html.Element,members:{__kN:null,__kO:null,tagNameHint:null,setPadding:function(o,p){this.__kO=o;this.__kN=p;if(this.getNodeName()==k){this.setStyle(n,o+f+p+e);}
;}
...
;var u=this._getProperty(a);var r=this._getProperty(i);var v=r?i:g;if(u!=null){u=u||null;q.paddingTop=this.__kN;q.paddingLeft=this.__kO;qx.bom.element.Decoration.update(s,u,v,q);}
;}
;}
...
;}
,getTagName:function(bf,be){if(be&&qx.core.Environment.get(q)&&this.__kQ[bf]&&qx.lang.String.endsWith(be,d)){return k;}
;return this.__kR[bf];}
...
;}
;var bl;if(qx.core.Environment.get(q)&&this.__kQ[bi]&&bj===z){var bm=this.__kT(bk);this.__kS(bh,bm.width,bm.height);bl=this.processAlphaFix(bh,bi,bk);}
else {delete bh.clip;if(bi===i){bl=this.__kU(bh,bi,bk);}
else if(bi===p||bi===P){bl=this.__kV(bh,bi,bk);}
else {bl=this.__kY(bh,bi,bk);}
;}
;return bl;}
,__kS:function(bo,bn,bp){if(bo.width==null&&bn!=null){bo.width=bn+c;}
;if(bo.height==null&&bp!=null){bo.height=bp+c;}
;}
,__kT:function(bq){var br=qx.util.ResourceManager.getInstance().getImageWidth(bq)||qx.io.ImageLoader.getWidth(bq);var bs=qx.util.ResourceManager.getInstance().getImageHeight(bq)||qx.io.ImageLoader.getHeight(bq);return {width:br,height:bs};}
...
,__kU:function(bz,bA,by){var bB=qx.util.ResourceManager.getInstance().toUri(by);var bC=this.__kT(by);this.__kS(bz,bC.width,bC.height);return {src:bB,style:bz};}
,__kV:function(bD,bE,bG){var bF=qx.util.ResourceManager.getInstance();var bJ=bF.getCombinedFormat(bG);var bL=this.__kT(bG);var bH;if(bJ){var bK=bF.getData(bG);var bI=bK[4];if(bJ==H){bH=bF.toDataUri(bG);}
else {bH=bF.toUri(bI);}
;if(bE===p){bD=this.__kW(bD,bK,bL.height);}
else {bD=this.__kX(bD,bK,bL.width);}
...
;}
,__kW:function(bM,bN,bP){var bO=qx.util.ResourceManager.getInstance().getImageHeight(bN[4]);bM.clip={top:-bN[6],height:bP};bM.height=bO+c;if(bM.top!=null){bM.top=(parseInt(bM.top,10)+bN[6])+c;}
else if(bM.bottom!=null){bM.bottom=(parseInt(bM.bottom,10)+bP-bO-bN[6])+c;}
;return bM;}
,__kX:function(bR,bS,bQ){var bT=qx.util.ResourceManager.getInstance().getImageWidth(bS[4]);bR.clip={left:-bS[5],width:bQ};bR.width=bT+c;if(bR.left!=null){bR.left=(parseInt(bR.left,10)+bS[5])+c;}
else if(bR.right!=null){bR.right=(parseInt(bR.right,10)+bQ-bT-bS[5])+c;}
;return bR;}
,__kY:function(bU,bV,bY){var bX=qx.util.ResourceManager.getInstance();var bW=bX.getCombinedFormat(bY);var ch=this.__kT(bY);if(bW&&bV!==b){var cg=bX.getData(bY);var ce=cg[4];if(bW==H){var cb=bX.toDataUri(bY);var ca=0;var cc=0;}
...
;}
;this.__kS(bU,ch.width,ch.height);this.__la(bU,bY,bV);return {style:bU};}
;}
...
;}
,__lb:function(cq){if(this.DEBUG&&qx.util.ResourceManager.getInstance().has(cq)&&cq.indexOf(a)==-1){if(!this.__kP[cq]){qx.log.Logger.debug(l+cq);this.__kP[cq]=true;}
;}
;}
}});}
)();
...
;return y+m+w;}
,__lf:function(A){var String=qx.lang.String;var B=A.substr(0,50);return String.startsWith(B,l)&&String.contains(B,r);}
...
},destruct:function(){this.__lv=this.__eY=null;}
});}
)();
...
(function(){var a="locale",b="_applyLocale",c="",d="changeLocale",e="_",f="Locale: ",g="C",h="locale.variant",j="qx.dynlocale",k=" not available.",l="qx.locale.Manager",m="String",n="singleton",o="qx.debug";qx.Class.define(l,{type:n,extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__lj=qx.$$translations||{};this.__lk=qx.$$locales||{};var p=qx.core.Environment.get(a);var q=qx.core.Environment.get(h);if(q!==c){p+=e+q;}
;this.__ll=p;this.setLocale(p||this.__lm);}
...
},properties:{locale:{check:m,nullable:true,apply:b,event:d}},members:{__lm:g,__ln:null,__lo:null,__lj:null,__lk:null,__ll:null,getLanguage:function(){return this.__lo;}
...
(function(){var a="qx.bom.client.Locale",b="-",c="locale",d="",e="android",f="locale.variant";qx.Bootstrap.define(a,{statics:{getLocale:function(){var g=qx.bom.client.Locale.__lr();var h=g.indexOf(b);if(h!=-1){g=g.substr(0,h);}
;return g;}
,getVariant:function(){var i=qx.bom.client.Locale.__lr();var k=d;var j=i.indexOf(b);if(j!=-1){k=i.substr(j+1);}
;return k;}
,__lr:function(){var l=(navigator.userLanguage||navigator.language||d);if(qx.bom.client.OperatingSystem.getName()==e){var m=/(\w{2})-(\w{2})/i.exec(navigator.userAgent);if(m){l=m[0];}
;}
...
;}
,properties:{alignX:{check:[o,j,p],init:o,apply:c},alignY:{check:[n,b,g],init:n,apply:c},spacing:{check:q,init:0,apply:c},separator:{check:a,nullable:true,apply:c},reversed:{check:k,init:false,apply:e}},members:{__lw:null,__lx:null,__ly:null,__gP:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__lz:function(){var B=this._getLayoutChildren();var length=B.length;var y=false;var w=this.__lw&&this.__lw.length!=length&&this.__lx&&this.__lw;var z;var x=w?this.__lw:new Array(length);var A=w?this.__lx:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;this.__ly=y;this.__gP=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(U,O,T){if(this._invalidChildrenCache){this.__lz();}
;var K=this.__gP;var length=K.length;var W=qx.ui.layout.Util;var S=this.getSpacing();var Y=this.getSeparator();if(Y){var H=W.computeHorizontalSeparatorGaps(K,S,Y);}
...
;var bs=qx.ui.layout.Util;var bf=this.__gP;var bk=0,bl=0,be=0;var bi=0,bj=0;var bp,bg,br;for(var i=0,l=bf.length;i<l;i+=1){bp=bf[i];bg=bp.getSizeHint();bl+=bg.width;var bo=this.__lx[i];var bh=this.__lw[i];if(bo){bk+=bg.minWidth;}
...
},destruct:function(){this.__lw=this.__lx=this.__gP=null;}
});}
)();
...
;}
,properties:{rich:{check:d,init:false,event:J,apply:L},wrap:{check:d,init:true,apply:j},value:{check:y,apply:t,event:k,nullable:true},buddy:{check:f,apply:w,nullable:true,init:null,dereference:true},textAlign:{check:[P,u,G,I],nullable:true,themeable:true,apply:c,event:i},appearance:{refine:true,init:N},selectable:{refine:true,init:false},allowGrowX:{refine:true,init:false},allowGrowY:{refine:true,init:false},allowShrinkY:{refine:true,init:false}},members:{__lA:null,__lB:null,__lC:null,__lD:null,_getContentHint:function(){if(this.__lB){this.__lE=this.__lF();delete this.__lB;}
...
;}
,__lE:{width:0,height:0},_applyFont:function(bc,bb){if(bb&&this.__lA&&this.__lD){this.__lA.removeListenerById(this.__lD);this.__lD=null;}
;var ba;if(bc){this.__lA=qx.theme.manager.Font.getInstance().resolve(bc);if(this.__lA instanceof qx.bom.webfonts.WebFont){this.__lD=this.__lA.addListener(h,this._onWebFontStatusChange,this);}
;ba=this.__lA.getStyles();}
else {this.__lA=null;ba=qx.bom.Font.getDefaultStyles();}
;if(this.getTextColor()!=null){delete ba[m];}
;this.getContentElement().setStyles(ba);this.__lB=true;qx.ui.core.queue.Layout.add(this);}
,__lF:function(bf){var be=qx.bom.Label;var bh=this.getFont();var bd=bh?this.__lA.getStyles():qx.bom.Font.getDefaultStyles();var content=this.getValue()||R;var bg=this.getRich();if(this.__lD){this.__lG();}
;return bg?be.getHtmlSize(content,bd,bf):be.getTextSize(content,bd);}
,__lG:function(){if(!this.getContentElement()){return;}
...
;}
;}
,_applyBuddy:function(bk,bj){if(bj!=null){this.removeRelatedBindings(bj);this.removeListenerById(this.__lC);this.__lC=null;}
;if(bk!=null){bk.bind(x,this,x);this.__lC=this.addListener(Q,function(){if(bk.isFocusable()){bk.focus.apply(bk);}
;if(z in bk&&typeof bk.toggleValue===C){bk.toggleValue();}
;}
,this);}
;}
,_applyRich:function(bl){this.getContentElement().setRich(bl);this.__lB=true;qx.ui.core.queue.Layout.add(this);}
...
;}
,"false":null}),_onWebFontStatusChange:function(bp){if(bp.getData().valid===true){if(qx.core.Environment.get(l)==a&&parseFloat(qx.core.Environment.get(D))>=8){window.setTimeout(function(){this.__lB=true;qx.ui.core.queue.Layout.add(this);}
.bind(this),0);}
;this.__lB=true;qx.ui.core.queue.Layout.add(this);}
;}
,_applyValue:function(br,bq){this.getContentElement().setValue(br);this.__lB=true;qx.ui.core.queue.Layout.add(this);}
},destruct:function(){if(qx.core.Environment.get(E)){qx.locale.Manager.getInstance().removeListener(O,this._onChangeLocale,this);}
;if(this.__lA&&this.__lD){this.__lA.removeListenerById(this.__lD);}
;this.__lA=null;}
});}
)();
...
;}
;return C;}
,__lM:function(I){var H={};if(I){H.whiteSpace=t;}
...
else if(!qx.core.Environment.get(l)&&qx.core.Environment.get(n)){var N=L.document.createElementNS(d,w);var K=N.style;K.cursor=u;K.color=u;K.overflow=y;K.maxWidth=i;K.padding=k;K.margin=k;K.width=j;for(var O in this.__lI){N.style[O]=u;}
...
(function(){var a="_applyActiveWindow",b="changeModal",c="windowAdded",d="changeVisibility",f="__gt",g="windowRemoved",h="qx.ui.window.Window",i="changeActive",j="qx.ui.window.MDesktop",k="__lP",l="changeActiveWindow",m="qx.event.type.Data";qx.Mixin.define(j,{properties:{activeWindow:{check:h,apply:a,event:l,init:null,nullable:true}},events:{windowAdded:m,windowRemoved:m},members:{__lP:null,__gt:null,getWindowManager:function(){if(!this.__gt){this.setWindowManager(new qx.ui.window.Window.DEFAULT_MANAGER_CLASS());}
;return this.__gt;}
...
;}
,getWindows:function(){if(!this.__lP){this.__lP=[];}
;return this.__lP;}
},destruct:function(){this._disposeArray(k);this._disposeObjects(f);}
});}
)();
(function(){var a="_applyBlockerColor",b="__lQ",c="Number",d="qx.ui.core.MBlocker",e="_applyBlockerOpacity",f="Color";qx.Mixin.define(d,{properties:{blockerColor:{check:f,init:null,nullable:true,apply:a,themeable:true},blockerOpacity:{check:c,init:1,apply:e,themeable:true}},members:{__lQ:null,_createBlocker:function(){return new qx.ui.core.Blocker(this);}
...
,isBlocked:function(){return this.__lQ&&this.__lQ.isBlocked();}
,unblock:function(){if(this.__lQ){this.__lQ.unblock();}
;}
,forceUnblock:function(){if(this.__lQ){this.__lQ.forceUnblock();}
;}
,blockContent:function(k){this.getBlocker().blockContent(k);}
,getBlocker:function(){if(!this.__lQ){this.__lQ=this._createBlocker();}
;return this.__lQ;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a="qx.dyntheme",b="backgroundColor",c="_applyOpacity",d="Boolean",f="px",g="keydown",h="deactivate",j="changeTheme",k="opacity",l="Tab",m="qx.event.type.Event",n="move",o="Color",p="resize",q="__de",r="zIndex",s="appear",t="qx.ui.root.Abstract",u="keyup",v="keypress",w="__lQ",x="Number",y="unblocked",z="qx.ui.core.Blocker",A="disappear",B="blocked",C="_applyColor";qx.Class.define(z,{extend:qx.core.Object,events:{blocked:m,unblocked:m},construct:function(D){qx.core.Object.call(this);this._widget=D;D.addListener(p,this.__lV,this);D.addListener(n,this.__lV,this);D.addListener(A,this.__lX,this);if(qx.Class.isDefined(t)&&D instanceof qx.ui.root.Abstract){this._isRoot=true;this.setKeepBlockerActive(true);}
;if(qx.core.Environment.get(a)){qx.theme.manager.Meta.getInstance().addListener(j,this._onChangeTheme,this);}
;this.__lR=[];this.__lS=[];}
,properties:{color:{check:o,init:null,nullable:true,apply:C,themeable:true},opacity:{check:x,init:1,apply:c,themeable:true},keepBlockerActive:{check:d,init:false}},members:{__lQ:null,__lT:0,__lR:null,__lS:null,__de:null,_widget:null,_isRoot:false,__lU:null,__lV:function(e){var E=e.getData();if(this.isBlocked()){this._updateBlockerBounds(E);}
;}
,__lW:function(){this._updateBlockerBounds(this._widget.getBounds());if(this._widget.isRootWidget()){this._widget.getContentElement().add(this.getBlockerElement());}
else {this._widget.getLayoutParent().getContentElement().add(this.getBlockerElement());}
;}
,__lX:function(){if(this.isBlocked()){this.getBlockerElement().getParent().remove(this.getBlockerElement());this._widget.addListenerOnce(s,this.__lW,this);}
;}
...
,_applyColor:function(I,H){var G=qx.theme.manager.Color.getInstance().resolve(I);this.__lY(b,G);}
,_applyOpacity:function(K,J){this.__lY(k,K);}
,_onChangeTheme:qx.core.Environment.select(a,{"true":function(){this._applyColor(this.getColor());}
,"false":null}),__lY:function(M,N){var L=[];this.__lQ&&L.push(this.__lQ);for(var i=0;i<L.length;i++ ){L[i].setStyle(M,N);}
;}
,_backupActiveWidget:function(){var O=qx.event.Registration.getManager(window).getHandler(qx.event.handler.Focus);this.__lR.push(qx.ui.core.Widget.getWidgetByElement(O.getActive()));this.__lS.push(qx.ui.core.Widget.getWidgetByElement(O.getFocus()));if(this._widget.isFocusable()){this._widget.focus();}
;}
,_restoreActiveWidget:function(){var Q;var P=this.__lS.length;if(P>0){Q=this.__lS.pop();if(Q&&!Q.isDisposed()&&Q.isFocusable()){Q.focus();}
;}
;var R=this.__lR.length;if(R>0){Q=this.__lR.pop();if(Q&&!Q.isDisposed()){Q.activate();}
;}
;}
,__ma:function(){return new qx.html.Blocker(this.getColor(),this.getOpacity());}
,getBlockerElement:function(S){if(!this.__lQ){this.__lQ=this.__ma();this.__lQ.setStyle(r,15);if(!S){if(this._isRoot){S=this._widget;}
else {S=this._widget.getLayoutParent();}
;}
;S.getContentElement().add(this.__lQ);this.__lQ.exclude();}
;return this.__lQ;}
,block:function(){this._block();}
,_block:function(T,V){if(!this._isRoot&&!this._widget.getLayoutParent()){this.__lU=this._widget.addListenerOnce(s,this._block.bind(this,T));return;}
...
;this.__lT++ ;if(this.__lT<2){this._backupActiveWidget();var W=this._widget.getBounds();if(W){this._updateBlockerBounds(W);}
;U.include();if(!V){U.activate();}
;U.addListener(h,this.__md,this);U.addListener(v,this.__mc,this);U.addListener(g,this.__mc,this);U.addListener(u,this.__mc,this);this.fireEvent(B,qx.event.type.Event);}
;}
,isBlocked:function(){return this.__lT>0;}
,unblock:function(){if(this.__lU){this._widget.removeListenerById(this.__lU);}
;if(!this.isBlocked()){return;}
;this.__lT-- ;if(this.__lT<1){this.__mb();this.__lT=0;}
;}
,forceUnblock:function(){if(!this.isBlocked()){return;}
;this.__lT=0;this.__mb();}
,__mb:function(){this._restoreActiveWidget();var X=this.getBlockerElement();X.removeListener(h,this.__md,this);X.removeListener(v,this.__mc,this);X.removeListener(g,this.__mc,this);X.removeListener(u,this.__mc,this);X.exclude();this.fireEvent(y,qx.event.type.Event);}
,blockContent:function(Y){this._block(Y,true);}
,__mc:function(e){if(e.getKeyIdentifier()==l){e.stop();}
;}
...
;this._widget.removeListener(p,this.__lV,this);this._widget.removeListener(n,this.__lV,this);this._widget.removeListener(s,this.__lW,this);this._widget.removeListener(A,this.__lX,this);if(this.__lU){this._widget.removeListenerById(this.__lU);}
;this._disposeObjects(w,q);this.__lR=this.__lS=this._widget=null;}
});}
)();
...
(function(){var a="Boolean",b="event.help",c="Space",d="textarea",f="changeGlobalCursor",g="",h="input",i="select",j="_applyNativeHelp",k=" !important",l="help",m="*",n="root",o="_applyGlobalCursor",p="qx.ui.root.Abstract",q="abstract",r="engine.name",s="keypress",t="contextmenu",u="String",v="a",w=";";qx.Class.define(p,{type:q,extend:qx.ui.core.Widget,include:[qx.ui.core.MChildrenHandling,qx.ui.core.MBlocker,qx.ui.window.MDesktop],construct:function(){qx.ui.core.Widget.call(this);qx.ui.core.FocusHandler.getInstance().addRoot(this);qx.ui.core.queue.Visibility.add(this);this.initNativeHelp();this.addListener(s,this.__mg,this);}
,properties:{appearance:{refine:true,init:n},enabled:{refine:true,init:true},focusable:{refine:true,init:true},globalCursor:{check:u,nullable:true,themeable:true,apply:o,event:f},nativeContextMenu:{refine:true,init:false},nativeHelp:{check:a,init:false,apply:j}},members:{__mf:null,isRootWidget:function(){return true;}
,getLayout:function(){return this._getLayout();}
,_applyGlobalCursor:qx.core.Environment.select(r,{"mshtml":function(y,x){}
,"default":function(C,B){var A=qx.bom.Stylesheet;var z=this.__mf;if(!z){this.__mf=z=A.createElement();}
;A.removeAllRules(z);if(C){A.addRule(z,m,qx.bom.element.Cursor.compile(C).replace(w,g)+k);}
;}
}),_applyNativeContextMenu:function(E,D){if(E){this.removeListener(t,this._onNativeContextMenu,this,true);}
else {this.addListener(t,this._onNativeContextMenu,this,true);}
;}
,_onNativeContextMenu:function(e){if(e.getTarget().getNativeContextMenu()){return;}
;e.preventDefault();}
,__mg:function(e){if(e.getKeyIdentifier()!==c){return;}
;var G=e.getTarget();var F=qx.ui.core.FocusHandler.getInstance();if(!F.isFocused(G)){return;}
;var H=G.getContentElement().getNodeName();if(H===h||H===d){return;}
;H=qx.dom.Node.getName(e.getOriginalTarget());if(H&&[h,d,i,v].indexOf(H)>-1){return;}
;e.preventDefault();}
,_applyNativeHelp:function(J,I){if(qx.core.Environment.get(b)){if(I===false){qx.bom.Event.removeNativeListener(document,l,(function(){return false;}
));}
;if(J===false){qx.bom.Event.addNativeListener(document,l,(function(){return false;}
));}
;}
;}
},destruct:function(){this.__mf=null;}
,defer:function(K,L){qx.ui.core.MChildrenHandling.remap(L);}
});}
)();
(function(){var a="keypress",b="focusout",c="activate",d="Tab",f="singleton",g="deactivate",h="__mh",j="focusin",k="qx.ui.core.FocusHandler";qx.Class.define(k,{extend:qx.core.Object,type:f,construct:function(){qx.core.Object.call(this);this.__mh={};}
,members:{__mh:null,__mi:null,__mj:null,__mk:null,connectTo:function(m){m.addListener(a,this.__hV,this);m.addListener(j,this._onFocusIn,this,true);m.addListener(b,this._onFocusOut,this,true);m.addListener(c,this._onActivate,this,true);m.addListener(g,this._onDeactivate,this,true);}
...
;}
,__hV:function(e){if(e.getKeyIdentifier()!=d){return;}
;if(!this.__mk){return;}
...
,__mw:function(e){var M=e.getOriginalTarget();while(M&&M.style){var Q=qx.bom.element.Style.get(M,v)!==s&&qx.bom.element.Style.get(M,v)!==o;var P=qx.bom.element.Style.get(M,c)===d;var O=qx.bom.element.Style.get(M,z)!=x;var N=qx.bom.element.Style.get(M,m)!=x;if(Q||P||N||O){return;}
;M=M.parentNode;}
;e.preventDefault();}
},destruct:function(){this.__cz=this.__mv=null;}
});}
)();
...
}});qx.Class.define(n,{extend:qx.data.store.Jsonp,construct:function(R){this.setCallbackName(z);qx.data.store.Jsonp.call(this,this.__NH(R));}
,members:{searchForTag:function(S){if(S!=t){this.setUrl(this.__NH(S));}
;}
,__NH:function(U){if(!U){return;}
;var T=r+U;T+=C;return T;}
}});}
)();
(function(){var a="",b="qx.ui.form.MForm",c="Boolean",d="_applyValid",f="changeLocale",g="changeRequired",h="changeValid",i="qx.dynlocale",j="changeInvalidMessage",k="String",l="invalid";qx.Mixin.define(b,{construct:function(){if(qx.core.Environment.get(i)){qx.locale.Manager.getInstance().addListener(f,this.__nH,this);}
;}
...
;var cK=this.getContentElement();if(cF&&this.__nJ){if(this.__nJ){var cH=this.getInsets();this._getPlaceholderElement().setStyles({paddingTop:cH.top+cM,paddingRight:cH.right+cM,paddingBottom:cH.bottom+cM,paddingLeft:cH.left+cM});}
;}
;if(cJ||cL.margin){if(this.__nJ){var cH=this.getInsets();this._getPlaceholderElement().setStyles({"width":(innerWidth-cH.left-cH.right)+cM,"height":(innerHeight-cH.top-cH.bottom)+cM});}
;cK.setStyles({"width":innerWidth+cM,"height":innerHeight+cM});this._renderContentElement(innerHeight,cK);}
;if(cL.position){if(this.__nJ){this._getPlaceholderElement().setStyles({"left":cN+cM,"top":top+cM});}
;}
;}
...
;return cP;}
,_applyEnabled:function(cS,cR){qx.ui.core.Widget.prototype._applyEnabled.call(this,cS,cR);this.getContentElement().setEnabled(cS);if(this.__nJ){if(cS){this._showPlaceholder();}
...
,_applyFont:function(cV,cU){if(cU&&this.__lA&&this.__lD){this.__lA.removeListenerById(this.__lD);this.__lD=null;}
;var cT;if(cV){this.__lA=qx.theme.manager.Font.getInstance().resolve(cV);if(this.__lA instanceof qx.bom.webfonts.WebFont){this.__lD=this.__lA.addListener(C,this._onWebFontStatusChange,this);}
;cT=this.__lA.getStyles();}
...
;if(this.__nJ){delete cT[F];this._getPlaceholderElement().setStyles(cT);}
;if(cV){this.__nL=qx.bom.Label.getTextSize(D,cT);}
...
,_onHtmlInput:function(e){var dd=e.getData();var dc=true;this.__nK=false;if(this.__iW&&this.__iW===dd){dc=false;}
...
;}
;if(dc){this.fireDataEvent(r,dd,this.__iW);this.__iW=dd;if(this.getLiveUpdate()){this.__nM(dd);}
;}
;}
,_onWebFontStatusChange:function(dh){if(dh.getData().valid===true){var dg=this.__lA.getStyles();this.__nL=qx.bom.Label.getTextSize(D,dg);qx.ui.core.queue.Layout.add(this);}
;}
,__nM:function(dj){var di=this.__iV;this.__iV=dj;if(di!=dj){this.fireNonBubblingEvent(c,qx.event.type.Data,[dj,di]);}
;}
,setValue:function(dn){if(dn===null){if(this.__nK){return dn;}
;dn=cm;this.__nK=true;}
else {this.__nK=false;if(this.__nJ){this._removePlaceholder();}
;}
;if(qx.lang.Type.isString(dn)){var dm=this.getContentElement();if(dm.getValue()!=dn){var dk=dm.getValue();dm.setValue(dn);var dl=this.__nK?null:dn;this.__iV=dk;this.__nM(dl);this.__iW=this.__iV;}
;if(this.__nJ){this._showPlaceholder();}
;return dn;}
;throw new Error(J+dn);}
,getValue:function(){var dp=this.getContentElement().getValue();return this.__nK?null:dp;}
,resetValue:function(){this.setValue(null);}
,_onChangeContent:function(e){this.__nK=e.getData()===null;this.__nM(e.getData());}
...
,setLayoutParent:function(parent){qx.ui.core.Widget.prototype.setLayoutParent.call(this,parent);if(this.__nJ){if(parent){this.getLayoutParent().getContentElement().add(this._getPlaceholderElement());}
...
.bind(this),0);}
,_removePlaceholder:function(){if(this.hasState(j)){if(this.__nJ){this._getPlaceholderElement().setStyle(cv,Y);}
;this.removeState(j);}
;}
,_syncPlaceholder:function(){if(this.hasState(j)&&this.__nJ){this._getPlaceholderElement().setStyle(cv,M);}
;}
...
;if(!this.__nJ&&qx.ui.form.AbstractField.__ds){qx.bom.Stylesheet.removeSheet(qx.ui.form.AbstractField.__ds);qx.ui.form.AbstractField.__ds=null;qx.ui.form.AbstractField.__nI();}
;}
,_applyPlaceholder:function(dx,dw){if(this.__nJ){this._getPlaceholderElement().setValue(dx);if(dx!=null){this.addListener(V,this._removePlaceholder,this);this.addListener(ct,this._showPlaceholder,this);this._showPlaceholder();}
...
;this._placeholder=this.__lA=null;if(qx.core.Environment.get(s)){qx.locale.Manager.getInstance().removeListener(cg,this._onChangeLocale,this);}
;if(this.__lA&&this.__lD){this.__lA.removeListenerById(this.__lD);}
...
;qx.html.Element.call(this,t,q,r);this.__nN=s;}
,members:{__nN:null,__nO:null,__nP:null,_createDomElement:function(){return qx.bom.Input.create(this.__nN);}
...
;}
,setEnabled:function(w){this.__nP=w;this.setAttribute(l,w===false);if(qx.core.Environment.get(a)==g){if(!w){this.setStyles({"userModify":m,"userSelect":d});}
else {this.setStyles({"userModify":null,"userSelect":this.__nO?null:d});}
;}
;}
,setSelectable:qx.core.Environment.select(a,{"webkit":function(x){this.__nO=x;qx.html.Element.prototype.setSelectable.call(this,this.__nP&&x);}
...
(function(){var a="toolTipText",b="icon",c="label",d="qx.ui.core.MExecutable",f="enabled",g="value",h="qx.event.type.Event",j="execute",k="_applyCommand",l="qx.ui.command.Command",m="menu",n="changeCommand";qx.Mixin.define(d,{events:{"execute":h},properties:{command:{check:l,apply:k,event:n,nullable:true}},members:{__ng:null,__nh:false,__ni:null,_bindableProperties:[f,c,b,a,g,m],execute:function(){var o=this.getCommand();if(o){if(this.__nh){this.__nh=false;}
else {this.__nh=true;o.execute(this);}
;}
;this.fireEvent(j);}
,__nj:function(e){if(this.__nh){this.__nh=false;return;}
;if(this.isEnabled()){this.__nh=true;this.execute();}
;}
,_applyCommand:function(r,p){if(p!=null){p.removeListenerById(this.__ni);}
;if(r!=null){this.__ni=r.addListener(j,this.__nj,this);}
;var q=this.__ng;if(q==null){this.__ng=q={};}
...
;}
;}
},destruct:function(){this._applyCommand(null,this.getCommand());this.__ng=null;}
});}
)();
...
;throw new Error(n);}
,getSelection:function(){return qx.lang.Object.getValues(this.__gx);}
,getSortedSelection:function(){var bU=this.getSelectables();var bT=qx.lang.Object.getValues(this.__gx);bT.sort(function(a,b){return bU.indexOf(a)-bU.indexOf(b);}
);return bT;}
,isItemSelected:function(bV){var bW=this._selectableToHashCode(bV);return this.__gx[bW]!==undefined;}
,isSelectionEmpty:function(){return qx.lang.Object.isEmpty(this.__gx);}
...
;this._fireChange();}
,_setLeadItem:function(cb){var ca=this.__po;if(ca!==null){this._styleSelectable(ca,u,false);}
;if(cb!==null){this._styleSelectable(cb,u,true);}
;this.__po=cb;}
,getLeadItem:function(){return this.__po;}
,_setAnchorItem:function(cd){var cc=this.__pp;if(cc!=null){this._styleSelectable(cc,I,false);}
;if(cd!=null){this._styleSelectable(cd,I,true);}
;this.__pp=cd;}
,_getAnchorItem:function(){return this.__pp!==null?this.__pp:null;}
...
;this._fireChange();}
,handlePointerOver:function(event){if(this.__pr!=null&&this.__pr!=this._getScroll().top){this.__pr=null;return;}
...
;var cD=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cA=event.isShiftPressed();if(this.isItemSelected(cB)&&!cA&&!cD&&!this.getDrag()){this.__pq=cB;this._userInteraction=false;return;}
else {this.__pq=null;}
;this._scrollItemIntoView(cB);var cC=this.getMode();if(this.getDrag()&&cC!==bu&&cC!==h&&!cA&&!cD&&event.getPointerType()==F){this._setAnchorItem(cB);this._setLeadItem(cB);this.__pe=this._getLocation();this.__pb=this._getScroll();this.__pf=event.getDocumentLeft()+this.__pb.left;this.__pg=event.getDocumentTop()+this.__pb.top;this.__ph=true;this._capture();}
;this._fireChange(bD);this._userInteraction=false;}
,handleTap:function(event){this._userInteraction=true;var cH=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var cE=event.isShiftPressed();if(!cH&&!cE&&this.__pq!=null){this._userInteraction=false;var cF=this._getSelectableFromPointerEvent(event);if(cF===null||!this.isItemSelected(cF)){return;}
;}
...
,handlePointerMove:function(event){if(!this.__ph){return;}
;this.__pi=event.getDocumentLeft();this.__pj=event.getDocumentTop();this._userInteraction=true;var cI=this.__pi+this.__pb.left;if(cI>this.__pf){this.__pk=1;}
else if(cI<this.__pf){this.__pk=-1;}
else {this.__pk=0;}
;var cJ=this.__pj+this.__pb.top;if(cJ>this.__pg){this.__pl=1;}
else if(cJ<this.__pg){this.__pl=-1;}
else {this.__pl=0;}
;var location=this.__pe;if(this.__pi<location.left){this.__oX=this.__pi-location.left;}
else if(this.__pi>location.right){this.__oX=this.__pi-location.right;}
else {this.__oX=0;}
;if(this.__pj<location.top){this.__oY=this.__pj-location.top;}
else if(this.__pj>location.bottom){this.__oY=this.__pj-location.bottom;}
else {this.__oY=0;}
;if(!this.__pa){this.__pa=new qx.event.Timer(100);this.__pa.addListener(G,this._onInterval,this);}
;this.__pa.start();this._autoSelect();event.stopPropagation();this._userInteraction=false;}
,handleAddItem:function(e){var cK=e.getData();if(this.getMode()===h&&this.isSelectionEmpty()){this.addItem(cK);}
;}
,handleRemoveItem:function(e){this.removeItem(e.getData());}
,_cleanup:function(){if(!this.getDrag()&&this.__ph){return;}
;if(this.__pm){this._fireChange(bD);}
;delete this.__ph;delete this.__pc;delete this.__pd;this._releaseCapture();if(this.__pa){this.__pa.stop();}
;}
,_onInterval:function(e){this._scrollBy(this.__oX,this.__oY);this.__pb=this._getScroll();this._autoSelect();}
,_autoSelect:function(){var cT=this._getDimension();var cM=Math.max(0,Math.min(this.__pi-this.__pe.left,cT.width))+this.__pb.left;var cL=Math.max(0,Math.min(this.__pj-this.__pe.top,cT.height))+this.__pb.top;if(this.__pc===cM&&this.__pd===cL){return;}
;this.__pc=cM;this.__pd=cL;var cQ=this._getAnchorItem();var cO=cQ;var cR=this.__pk;var cU,cN;while(cR!==0){cU=cR>0?this._getRelatedSelectable(cO,t):this._getRelatedSelectable(cO,W);if(cU!==null){cN=this._getSelectableLocationX(cU);if((cR>0&&cN.left<=cM)||(cR<0&&cN.right>=cM)){cO=cU;continue;}
;}
;break;}
;var cS=this.__pl;var cV,cP;while(cS!==0){cV=cS>0?this._getRelatedSelectable(cO,z):this._getRelatedSelectable(cO,bB);if(cV!==null){cP=this._getSelectableLocationY(cV);if((cS>0&&cP.top<=cL)||(cS<0&&cP.bottom>=cL)){cO=cV;continue;}
;}
;break;}
...
;this._fireChange(bw);}
,__ps:{Home:1,Down:1,Right:1,PageDown:1,End:1,Up:1,Left:1,PageUp:1},handleKeyPress:function(event){this._userInteraction=true;var dd,dc;var df=event.getKeyIdentifier();var de=this.getMode();var cY=event.isCtrlPressed()||(qx.core.Environment.get(c)==E&&event.isMetaPressed());var da=event.isShiftPressed();var db=false;if(df===A&&cY){if(de!==bu&&de!==h){this._selectAllItems();db=true;}
;}
...
;db=true;}
;}
else if(this.__ps[df]){db=true;if(de===bu||de==h){dd=this._getSelectedItem();}
...
;}
;break;};this.__pr=this._getScroll().top;this._scrollItemIntoView(dc);}
;}
...
;}
,_clearSelection:function(){var di=this.__gx;for(var dj in di){this._removeFromSelection(di[dj]);}
;this.__gx={};}
,_selectItemRange:function(dk,dp,dm){var dq=this._getSelectableRange(dk,dp);if(!dm){var dn=this.__gx;var dr=this.__pt(dq);for(var dl in dn){if(!dr[dl]){this._removeFromSelection(dn[dl]);}
;}
;}
...
;}
,__pt:function(dw){var dx={};var dv;for(var i=0,l=dw.length;i<l;i++ ){dv=dw[i];dx[this._selectableToHashCode(dv)]=dv;}
;return dx;}
,_getSelectedItem:function(){for(var dy in this.__gx){return this.__gx[dy];}
;return null;}
,_setSelectedItem:function(dA){if(this._isSelectable(dA)){var dz=this.__gx;var dB=this._selectableToHashCode(dA);if(!dz[dB]||(dz.length>=2)){this._clearSelection();this._addToSelection(dA);}
;}
;}
,_addToSelection:function(dC){var dD=this._selectableToHashCode(dC);if(this.__gx[dD]==null&&this._isSelectable(dC)){this.__gx[dD]=dC;this._styleSelectable(dC,R,true);this.__pm=true;}
;}
,_toggleInSelection:function(dE){var dF=this._selectableToHashCode(dE);if(this.__gx[dF]==null){this.__gx[dF]=dE;this._styleSelectable(dE,R,true);}
else {delete this.__gx[dF];this._styleSelectable(dE,R,false);}
;this.__pm=true;}
,_removeFromSelection:function(dG){var dH=this._selectableToHashCode(dG);if(this.__gx[dH]!=null){delete this.__gx[dH];this._styleSelectable(dG,R,false);this.__pm=true;}
;}
...
;}
;var dP=dK[0];var dI=dO;var dM=this.__gx;for(var dN in dM){if(dJ[dN]){delete dJ[dN];}
...
;if(!dL){return false;}
;this._scrollItemIntoView(dI);this._setLeadItem(dP);this._setAnchorItem(dP);this.__pm=true;this._fireChange();}
,_fireChange:function(dQ){if(this.__pm){this.__pn=dQ||null;this.fireDataEvent(bA,this.getSelection());delete this.__pm;}
;}
...
;return dS;}
;return null;}
},destruct:function(){this._disposeObjects(T);this.__gx=this.__pq=this.__pp=null;this.__po=null;}
});}
)();
...
;this.__gt.removeItem(I);}
,selectRange:function(J,K){this.__gt.selectItemRange(J,K);}
,resetSelection:function(){this.__gt.clearSelection();}
...
else {var M=this.getSelection();if(!qx.lang.Array.equals(M,L)){this.__gt.replaceSelection(L);}
;}
;}
,getSelection:function(){return this.__gt.getSelection();}
,getSortedSelection:function(){return this.__gt.getSortedSelection();}
,isSelectionEmpty:function(){return this.__gt.isSelectionEmpty();}
,getSelectionContext:function(){return this.__gt.getSelectionContext();}
,_getManager:function(){return this.__gt;}
,getSelectables:function(N){return this.__gt.getSelectables(N);}
,invertSelection:function(){this.__gt.invertSelection();}
,_getLeadItem:function(){var O=this.__gt.getMode();if(O===r||O===b){return this.__gt.getSelectedItem();}
else {return this.__gt.getLeadItem();}
;}
,_applySelectionMode:function(Q,P){this.__gt.setMode(Q);}
,_applyDragSelection:function(S,R){this.__gt.setDrag(S);}
,_applyQuickSelection:function(U,T){this.__gt.setQuick(U);}
,_onSelectionChange:function(e){this.fireDataEvent(l,e.getData());}
},destruct:function(){this._disposeObjects(h);}
});}
)();
(function(){var a="Could not set the model selection. Maybe your models are not unique? ",b="qx.ui.form.MModelSelection",c="__oO",d="Please use an array as parameter.",f="change",g="qx.debug",h="changeSelection",k="qx.event.type.Data";qx.Mixin.define(b,{construct:function(){this.__oO=new qx.data.Array();this.__oO.addListener(f,this.__oR,this);this.addListener(h,this.__oQ,this);}
,events:{changeModelSelection:k},members:{__oO:null,__oP:false,__oQ:function(){if(this.__oP){return;}
...
;}
;}
,__oR:function(){this.__oP=true;var r=this.getSelectables(true);var s=[];var q=this.__oO.toArray();for(var i=0;i<q.length;i++ ){var u=q[i];for(var j=0;j<r.length;j++ ){var v=r[j];var p=v.getModel?v.getModel():null;if(u===p){s.push(v);break;}
;}
;}
;this.setSelection(s);this.__oP=false;var t=this.getSelection();if(!qx.lang.Array.equals(t,s)){this.__oQ();}
;}
,getModelSelection:function(){return this.__oO;}
,setModelSelection:function(w){if(!w){this.__oO.removeAll();return;}
;if(qx.core.Environment.get(g)){this.assertArray(w,d);}
;w.unshift(this.__oO.getLength());w.unshift(0);var x=this.__oO.splice.apply(this.__oO,w);x.dispose();}
...
(function(){var a="mouse",b="scrollbar-y",c="qx.ui.core.scroll.MRoll",d="pointerdown",f="scrollbar-x",g="roll";qx.Mixin.define(c,{members:{__ol:null,_addRollHandling:function(){this.addListener(g,this._onRoll,this);this.addListener(d,this._onPointerDownForRoll,this);}
...
,_onPointerDownForRoll:function(e){this.__ol=e.getPointerId();}
,_onRoll:function(e){if(e.getPointerType()==a){return;}
;if(this.__ol&&e.getMomentum()){qx.event.Registration.getManager(e.getOriginalTarget()).getHandler(qx.event.handler.Gesture).gestureCancel(this.__ol);e.stopMomentum();this.__ol=null;return;}
;this.__ol=null;var k=this._isChildControlVisible(f);var l=this._isChildControlVisible(b);var q=l?this.getChildControl(b,true):null;var p=k?this.getChildControl(f,true):null;var m=e.getDelta().y;var h=e.getDelta().x;var j=!l;var o=!k;if(q){if(m!==0){q.scrollBy(parseInt(m,10));}
...
;r.addListener(k,this.__op,this);r.addListener(i,this.__oq,this);this.__om=[n,o];this.__on=[m,g];}
,properties:{dragScrollThresholdX:{check:p,init:30},dragScrollThresholdY:{check:p,init:30},dragScrollSlowDownFactor:{check:b,init:0.1}},members:{__oo:null,__om:null,__on:null,_findScrollableParent:function(s){var t=s;if(t===null){return null;}
...
;}
,_getAxis:function(A){if(this.__om.indexOf(A)!==-1){return a;}
else if(this.__on.indexOf(A)!==-1){return q;}
else {throw new Error(l+A+f);}
;}
,_getThresholdByEdgeType:function(B){if(this.__om.indexOf(B)!==-1){return this.getDragScrollThresholdX();}
else if(this.__on.indexOf(B)!==-1){return this.getDragScrollThresholdY();}
;}
...
;var P=S.getBounds(),O=T===a?P.width:P.height,R=this._calculateScrollAmount(O,Q);if(this._isScrollbarExceedingMaxPos(S,T,R)){this.__oo.stop();}
;S.scrollBy(R);}
,__op:function(e){if(this.__oo){this.__oo.stop();}
...
;bd=this._getAxis(X);if(this._isScrollbarVisible(U,bd)){bb=this._calculateThresholdExceedance(bc[X],this._getThresholdByEdgeType(X));if(this.__oo){this.__oo.dispose();}
;this.__oo=new qx.event.Timer(50);this.__oo.addListener(h,function(be,bg,bf){this._scrollBy(be,bg,bf);}
.bind(this,U,bd,bb));this.__oo.start();e.stopPropagation();return;}
else {U=this._findScrollableParent(U);}
;}
;}
,__oq:function(e){if(this.__oo){this.__oo.stop();}
;}
},destruct:function(){if(this.__oo){this.__oo.dispose();}
;}
});}
)();
...
,_getContentHint:function(){var L=qx.bom.element.Scroll.getScrollbarWidth();return {width:this.__mY?100:L,maxWidth:this.__mY?null:L,minWidth:this.__mY?null:L,height:this.__mY?L:100,maxHeight:this.__mY?L:null,minHeight:this.__mY?L:null};}
...
,_applyPosition:function(P){var content=this.getContentElement();if(this.__mY){content.scrollToX(P);}
else {content.scrollToY(P);}
;}
,_applyOrientation:function(S,Q){var R=this.__mY=S===x;this.set({allowGrowX:R,allowShrinkX:R,allowGrowY:!R,allowShrinkY:!R});if(R){this.replaceState(o,x);}
...
,_updateScrollBar:function(){var U=this.__mY;var V=this.getBounds();if(!V){return;}
...
,scrollTo:function(Y,X){this.stopScrollAnimation();if(X){var ba=this.getPosition();this.__os=new qx.bom.AnimationFrame();this.__os.on(d,function(bc){var bb=parseInt(bc/X*(Y-ba)+ba);this.updatePosition(bb);}
,this);this.__os.on(f,function(){this.setPosition(Math.max(0,Math.min(this.getMaximum(),Y)));this.__os=null;this.fireEvent(q);}
,this);this.__os.startSequence(X);}
...
,stopScrollAnimation:function(){if(this.__os){this.__os.cancelSequence();this.__os=null;}
;}
,_onScroll:function(e){var bj=this.getContentElement();var bk=this.__mY?bj.getScrollX():bj.getScrollY();this.setPosition(bk);}
...
,this);}
,events:{"scrollAnimationEnd":l},properties:{appearance:{refine:true,init:A},orientation:{check:[x,r],init:x,apply:s},maximum:{check:w,apply:B,init:100},position:{check:i,init:0,apply:z,event:F},singleStep:{check:f,init:20},pageStep:{check:f,init:10,apply:u},knobFactor:{check:c,apply:j,nullable:true}},members:{__wP:2,__wQ:0,_computeSizeHint:function(){var H=qx.ui.core.Widget.prototype._computeSizeHint.call(this);if(this.getOrientation()===x){this.__wQ=H.minWidth;H.minWidth=0;}
else {this.__wQ=H.minHeight;H.minHeight=0;}
;return H;}
,renderLayout:function(M,top,J,I){var L=qx.ui.core.Widget.prototype.renderLayout.call(this,M,top,J,I);var K=this.getOrientation()===x;if(this.__wQ>=(K?J:I)){this.getChildControl(n).setVisibility(o);this.getChildControl(D).setVisibility(o);}
...
,_onResizeSlider:function(e){var bg=this.getChildControl(a).getChildControl(m);var bj=bg.getSizeHint();var bh=false;var bi=this.getChildControl(a).getInnerSize();if(this.getOrientation()==r){if(bi.height<bj.minHeight+this.__wP){bh=true;}
;}
else {if(bi.width<bj.minWidth+this.__wP){bh=true;}
;}
;if(bh){bg.exclude();}
else {bg.show();}
;}
...
,_onPointerDown:function(e){if(this.__wU){return;}
;var bw=this.__mY;var bu=this.getChildControl(P);var bv=bw?G:L;var bA=bw?e.getDocumentLeft():e.getDocumentTop();var bC=this.getDecorator();bC=qx.theme.manager.Decoration.getInstance().resolve(bC);if(bw){var bz=bC?bC.getInsets().left:0;var by=(this.getPaddingLeft()||0)+bz;}
else {var bz=bC?bC.getInsets().top:0;var by=(this.getPaddingTop()||0)+bz;}
;var bB=this.__wR=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bv];bB+=by;var bx=this.__wS=qx.bom.element.Location.get(bu.getContentElement().getDomElement())[bv];if(e.getTarget()===bu){this.__wU=true;if(!this.__xa){this.__xa=new qx.event.Timer(100);this.__xa.addListener(r,this._fireValue,this);}
;this.__xa.start();this.__wV=bA+bB-bx;bu.addState(c);}
else {this.__wW=true;this.__wX=bA<=bx?-1:1;this.__xe(e);this._onInterval();if(!this.__de){this.__de=new qx.event.Timer(100);this.__de.addListener(r,this._onInterval,this);}
;this.__de.start();}
;this.addListener(w,this._onPointerMove);this.capture();e.stopPropagation();}
,_onPointerUp:function(e){if(this.__wU){this.releaseCapture();delete this.__wU;this.__xa.stop();this._fireValue();delete this.__wV;this.getChildControl(P).removeState(c);if(e.getType()===B){var bE;var bF;var bD;if(this.__mY){bE=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__wR);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[L];bF=e.getDocumentTop()-(bD+this.getChildControl(P).getBounds().top);}
else {bE=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__wR);bD=qx.bom.element.Location.get(this.getContentElement().getDomElement())[G];bF=e.getDocumentLeft()-(bD+this.getChildControl(P).getBounds().left);}
;if(bF<0||bF>this.__wT||bE<0||bE>this.__wT){this.getChildControl(P).removeState(b);}
;}
;}
else if(this.__wW){this.__de.stop();this.releaseCapture();delete this.__wW;delete this.__wX;delete this.__wY;}
;this.removeListener(w,this._onPointerMove);if(e.getType()===B){e.stopPropagation();}
;}
,_onPointerMove:function(e){if(this.__wU){var bH=this.__mY?e.getDocumentLeft():e.getDocumentTop();var bG=bH-this.__wV;this.slideTo(this._positionToValue(bG));}
else if(this.__wW){this.__xe(e);}
;e.stopPropagation();}
,_onInterval:function(e){var bI=this.getValue()+(this.__wX*this.getPageStep());if(bI<this.getMinimum()){bI=this.getMinimum();}
else if(bI>this.getMaximum()){bI=this.getMaximum();}
;var bJ=this.__wX==-1;if((bJ&&bI<=this.__wY)||(!bJ&&bI>=this.__wY)){bI=this.__wY;}
;this.slideTo(bI);}
,_onUpdate:function(e){var bL=this.getInnerSize();var bM=this.getChildControl(P).getBounds();var bK=this.__mY?E:x;this._updateKnobSize();this.__xd=bL[bK]-bM[bK];this.__wT=bM[bK];this._updateKnobPosition();}
,__mY:false,__xd:0,__xe:function(e){var bN=this.__mY;var bU=bN?e.getDocumentLeft():e.getDocumentTop();var bX=this.__wR;var bO=this.__wS;var bY=this.__wT;var bV=bU-bX;if(bU>=bO){bV-=bY;}
;var bR=this._positionToValue(bV);var bP=this.getMinimum();var bQ=this.getMaximum();if(bR<bP){bR=bP;}
else if(bR>bQ){bR=bQ;}
else {var bW=this.getValue();var bT=this.getPageStep();var bS=this.__wX<0?I:Q;bR=bW+(Math[bS]((bR-bW)/bT)*bT);}
;if(this.__wY==null||(this.__wX==-1&&bR<=this.__wY)||(this.__wX==1&&bR>=this.__wY)){this.__wY=bR;}
;}
,_positionToValue:function(cb){var ca=this.__xd;if(ca==null||ca==0){return 0;}
;var cd=cb/ca;if(cd<0){cd=0;}
else if(cd>1){cd=1;}
;var cc=this.getMaximum()-this.getMinimum();return this.getMinimum()+Math.round(cc*cd);}
,_valueToPosition:function(ch){var ce=this.__xd;if(ce==null){return 0;}
...
,_setKnobPosition:function(ck){var ci=this.getChildControl(P);var cj=this.getDecorator();cj=qx.theme.manager.Decoration.getInstance().resolve(cj);var content=ci.getContentElement();if(this.__mY){if(cj&&cj.getPadding()){ck+=cj.getPadding().left;}
...
;if(this.__mY){this.getChildControl(P).setWidth(Math.round(cm*cl.width));}
...
,slideTo:function(cu,ct){this.stopSlideAnimation();if(ct){this.__xg(cu,ct);}
else {this.updatePosition(cu);}
;}
,updatePosition:function(cv){this.setValue(this.__xf(cv));}
,stopSlideAnimation:function(){if(this.__os){this.__os.cancelSequence();this.__os=null;}
;}
,__xf:function(cw){if(cw<this.getMinimum()){cw=this.getMinimum();}
...
;return cw;}
,__xg:function(cy,cx){cy=this.__xf(cy);var cz=this.getValue();this.__os=new qx.bom.AnimationFrame();this.__os.on(i,function(cA){this.setValue(parseInt(cA/cx*(cy-cz)+cz));}
,this);this.__os.on(j,function(){this.setValue(cy);this.__os=null;this.fireEvent(O);}
,this);this.__os.startSequence(cx);}
,_applyOrientation:function(cD,cC){var cB=this.getChildControl(P);this.__mY=cD===N;if(this.__mY){this.removeState(C);cB.removeState(C);this.addState(N);cB.addState(N);cB.setLayoutProperties({top:0,right:null,bottom:0});}
...
else {if(this.__mY){this.getChildControl(P).resetWidth();}
else {this.getChildControl(P).resetHeight();}
;}
;}
,_applyValue:function(cH,cG){if(cH!=null){this._updateKnobPosition();if(this.__wU){this.__xc=[cH,cG];}
else {this.fireEvent(h,qx.event.type.Data,[cH,cG]);}
;}
else {this.resetValue();}
;}
,_fireValue:function(){if(!this.__xc){return;}
;var cI=this.__xc;this.__xc=null;this.fireEvent(h,qx.event.type.Data,cI);}
...
,events:{"execute":j,"press":j,"release":j},properties:{interval:{check:m,init:100},firstInterval:{check:m,init:500},minTimer:{check:m,init:20},timerDecrease:{check:m,init:2}},members:{__to:null,__de:null,press:function(){if(this.isEnabled()){if(!this.hasState(g)){this.__tp();}
;this.removeState(l);this.addState(g);}
;}
,release:function(q){if(!this.isEnabled()){return;}
;if(this.hasState(g)){if(!this.__to){this.execute();}
;}
;this.removeState(g);this.removeState(l);this.__tq();}
...
;}
,_onPointerDown:function(e){if(!e.isLeftPressed()){return;}
;this.capture();this.__tp();e.stopPropagation();}
,_onPointerUp:function(e){this.releaseCapture();if(!this.hasState(l)){this.addState(b);if(this.hasState(g)&&!this.__to){this.execute();}
;}
;this.__tq();e.stopPropagation();}
,_onTap:function(e){}
,_onKeyUp:function(e){switch(e.getKeyIdentifier()){case f:case k:if(this.hasState(g)){if(!this.__to){this.execute();}
;this.removeState(g);this.removeState(l);e.stopPropagation();this.__tq();}
;};}
,_onKeyDown:function(e){switch(e.getKeyIdentifier()){case f:case k:this.removeState(l);this.addState(g);e.stopPropagation();this.__tp();};}
,_onInterval:function(e){this.__to=true;this.fireEvent(n);}
,__tp:function(){this.fireEvent(a);this.__to=false;this.__de.set({interval:this.getInterval(),firstInterval:this.getFirstInterval(),minimum:this.getMinTimer(),decrease:this.getTimerDecrease()}).start();this.removeState(l);this.addState(g);}
,__tq:function(){this.fireEvent(d);this.__de.stop();this.removeState(l);this.removeState(g);}
},destruct:function(){this._disposeObjects(i);}
});}
)();
(function(){var a="Integer",b="__de",c="interval",d="qx.event.type.Event",e="qx.event.AcceleratingTimer";qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);this.__de=new qx.event.Timer(this.getInterval());this.__de.addListener(c,this._onInterval,this);}
,events:{"interval":d},properties:{interval:{check:a,init:100},firstInterval:{check:a,init:500},minimum:{check:a,init:20},decrease:{check:a,init:2}},members:{__de:null,__tr:null,start:function(){this.__de.setInterval(this.getFirstInterval());this.__de.start();}
,stop:function(){this.__de.stop();this.__tr=null;}
,_onInterval:function(){this.__de.stop();if(this.__tr==null){this.__tr=this.getInterval();}
;this.__tr=Math.max(this.getMinimum(),this.__tr-this.getDecrease());this.__de.setInterval(this.__tr);this.__de.start();this.fireEvent(c);}
},destruct:function(){this._disposeObjects(b);}
});}
)();
...
;if(v){this.setSeparator(v);}
;}
,properties:{alignY:{check:[k,o,f],init:k,apply:b},alignX:{check:[m,c,r],init:m,apply:b},spacing:{check:p,init:0,apply:b},separator:{check:a,nullable:true,apply:b},reversed:{check:s,init:false,apply:d}},members:{__mT:null,__lx:null,__ly:null,__gP:null,_applyReversed:function(){this._invalidChildrenCache=true;this._applyLayoutChange();}
,__lz:function(){var B=this._getLayoutChildren();var length=B.length;var x=false;var w=this.__mT&&this.__mT.length!=length&&this.__lx&&this.__mT;var z;var y=w?this.__mT:new Array(length);var A=w?this.__lx:new Array(length);if(this.getReversed()){B=B.concat().reverse();}
...
;}
;if(!w){this.__mT=y;this.__lx=A;}
;this.__ly=x;this.__gP=B;delete this._invalidChildrenCache;}
...
;}
,"false":null}),renderLayout:function(V,N,Y){if(this._invalidChildrenCache){this.__lz();}
;var K=this.__gP;var length=K.length;var U=qx.ui.layout.Util;var T=this.getSpacing();var bb=this.getSeparator();if(bb){var H=U.computeVerticalSeparatorGaps(K,T,bb);}
else {var H=U.computeVerticalGaps(K,T,true);}
;var i,ba,G,O;var P=[];var W=H;for(i=0;i<length;i+=1){O=this.__mT[i];G=O!=null?Math.floor((N-H)*O):K[i].getSizeHint().height;P.push(G);W+=G;}
...
;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__lz();}
;var be=qx.ui.layout.Util;var bs=this.__gP;var bh=0,bi=0,bq=0;var bj=0,bk=0;var bo,bf,br;for(var i=0,l=bs.length;i<l;i+=1){bo=bs[i];bf=bo.getSizeHint();bi+=bf.height;var bn=this.__lx[i];var bg=this.__mT[i];if(bn){bh+=bf.minHeight;}
...
},destruct:function(){this.__mT=this.__lx=this.__gP=null;}
});}
)();
...
(function(){var a="' must be defined!",b="height",c="vAlign",d="hAlign",e="' is not supported by the Grid layout!",f="bottom",g="Invalid parameter 'column'",h="Integer",m="The property '",n="'",o="_applyLayoutChange",p="qx.debug",q="Value must be positive",r="center",s="qx.ui.layout.Grid",t="middle",u="maxHeight",v="Cannot add widget '",w="width",z=") for '",A="'!. ",B="top",C="minHeight",D="right",E="' in this cell (",F=", ",G="The layout properties 'row' and 'column' of the child widget '",H="minWidth",I="flex",J="left",K="maxWidth",L="Invalid parameter 'row'",M="There is already a widget '";qx.Class.define(s,{extend:qx.ui.layout.Abstract,construct:function(O,N){qx.ui.layout.Abstract.call(this);this.__ot=[];this.__ou=[];if(O){this.setSpacingX(O);}
;if(N){this.setSpacingY(N);}
;}
,properties:{spacingX:{check:h,init:0,apply:o},spacingY:{check:h,init:0,apply:o}},members:{__ov:null,__ot:null,__ou:null,__ow:null,__ox:null,__oy:null,__oz:null,__oA:null,__oB:null,verifyLayoutProperty:qx.core.Environment.select(p,{"true":function(P,name,Q){var R={"row":1,"column":1,"rowSpan":1,"colSpan":1};this.assert(R[name]==1,m+name+e);this.assertInteger(Q);this.assert(Q>=0,q);}
,"false":null}),__oC:function(){var Y=[];var T=[];var W=[];var V=-1;var S=-1;var X=this._getLayoutChildren();for(var i=0,l=X.length;i<l;i++ ){var ba=X[i];var bb=ba.getLayoutProperties();var bc=bb.row;var U=bb.column;bb.colSpan=bb.colSpan||1;bb.rowSpan=bb.rowSpan||1;if(bc==null||U==null){throw new Error(G+ba+a);}
...
;}
;this.__ov=Y;this.__ow=T;this.__ox=W;this.__oy=V;this.__oz=S;this.__oA=null;this.__oB=null;delete this._invalidChildrenCache;}
,_setRowData:function(bg,be,bf){var bd=this.__ot[bg];if(!bd){this.__ot[bg]={};this.__ot[bg][be]=bf;}
else {bd[be]=bf;}
;}
,_setColumnData:function(bh,bj,bk){var bi=this.__ou[bh];if(!bi){this.__ou[bh]={};this.__ou[bh][bj]=bk;}
else {bi[bj]=bk;}
;}
...
,getColumnAlign:function(bp){var bq=this.__ou[bp]||{};return {vAlign:bq.vAlign||B,hAlign:bq.hAlign||J};}
...
,getRowAlign:function(bv){var bu=this.__ot[bv]||{};return {vAlign:bu.vAlign||B,hAlign:bu.hAlign||J};}
,getCellWidget:function(bx,bw){if(this._invalidChildrenCache){this.__oC();}
;var bx=this.__ov[bx]||{};return bx[bw]||null;}
,getRowCount:function(){if(this._invalidChildrenCache){this.__oC();}
;return this.__oy+1;}
,getColumnCount:function(){if(this._invalidChildrenCache){this.__oC();}
;return this.__oz+1;}
,getCellAlign:function(bF,bz){var bE=B;var bC=J;var bD=this.__ot[bF];var bA=this.__ou[bz];var by=this.__ov[bF][bz];if(by){var bB={vAlign:by.getAlignY(),hAlign:by.getAlignX()};}
else {bB={};}
...
,getColumnFlex:function(bI){var bJ=this.__ou[bI]||{};return bJ.flex!==undefined?bJ.flex:0;}
,setRowFlex:function(bL,bK){this._setRowData(bL,I,bK);this._applyLayoutChange();return this;}
,getRowFlex:function(bO){var bM=this.__ot[bO]||{};var bN=bM.flex!==undefined?bM.flex:0;return bN;}
,setColumnMaxWidth:function(bP,bQ){this._setColumnData(bP,K,bQ);this._applyLayoutChange();return this;}
,getColumnMaxWidth:function(bR){var bS=this.__ou[bR]||{};return bS.maxWidth!==undefined?bS.maxWidth:Infinity;}
,setColumnWidth:function(bT,bU){this._setColumnData(bT,w,bU);this._applyLayoutChange();return this;}
,getColumnWidth:function(bV){var bW=this.__ou[bV]||{};return bW.width!==undefined?bW.width:null;}
,setColumnMinWidth:function(bX,bY){this._setColumnData(bX,H,bY);this._applyLayoutChange();return this;}
,getColumnMinWidth:function(ca){var cb=this.__ou[ca]||{};return cb.minWidth||0;}
,setRowMaxHeight:function(cd,cc){this._setRowData(cd,u,cc);this._applyLayoutChange();return this;}
,getRowMaxHeight:function(cf){var ce=this.__ot[cf]||{};return ce.maxHeight||Infinity;}
,setRowHeight:function(cg,ch){this._setRowData(cg,b,ch);this._applyLayoutChange();return this;}
,getRowHeight:function(cj){var ci=this.__ot[cj]||{};return ci.height!==undefined?ci.height:null;}
,setRowMinHeight:function(cl,ck){this._setRowData(cl,C,ck);this._applyLayoutChange();return this;}
,getRowMinHeight:function(cn){var cm=this.__ot[cn]||{};return cm.minHeight||0;}
,__oD:function(cp){var co=cp.getSizeHint();var cr=cp.getMarginLeft()+cp.getMarginRight();var cq=cp.getMarginTop()+cp.getMarginBottom();var cs={height:co.height+cq,width:co.width+cr,minHeight:co.minHeight+cq,minWidth:co.minWidth+cr,maxHeight:co.maxHeight+cq,maxWidth:co.maxWidth+cr};return cs;}
,_fixHeightsRowSpan:function(cN){var cz=this.getSpacingY();for(var i=0,l=this.__ox.length;i<l;i++ ){var cF=this.__ox[i];var cC=this.__oD(cF);var cv=cF.getLayoutProperties();var cB=cv.row;var cK=cz*(cv.rowSpan-1);var ct=cK;var cw={};for(var j=0;j<cv.rowSpan;j++ ){var cA=cv.row+j;var cJ=cN[cA];var cL=this.getRowFlex(cA);if(cL>0){cw[cA]={min:cJ.minHeight,value:cJ.height,max:cJ.maxHeight,flex:cL};}
...
;}
;}
,_fixWidthsColSpan:function(cR){var cS=this.getSpacingX();for(var i=0,l=this.__ow.length;i<l;i++ ){var cO=this.__ow[i];var cQ=this.__oD(cO);var cU=cO.getLayoutProperties();var cP=cU.column;var db=cS*(cU.colSpan-1);var cT=db;var cV={};var cX;for(var j=0;j<cU.colSpan;j++ ){var cW=cU.column+j;var da=cR[cW];var cY=this.getColumnFlex(cW);if(cY>0){cV[cW]={min:da.minWidth,value:da.width,max:da.maxWidth,flex:cY};}
...
;}
;}
,_getRowHeights:function(){if(this.__oA!=null){return this.__oA;}
;var dm=[];var df=this.__oy;var de=this.__oz;for(var dn=0;dn<=df;dn++ ){var dg=0;var di=0;var dh=0;for(var dl=0;dl<=de;dl++ ){var dd=this.__ov[dn][dl];if(!dd){continue;}
;var dj=dd.getLayoutProperties().rowSpan||0;if(dj>1){continue;}
;var dk=this.__oD(dd);if(this.getRowFlex(dn)>0){dg=Math.max(dg,dk.minHeight);}
...
;if(this.__ox.length>0){this._fixHeightsRowSpan(dm);}
;this.__oA=dm;return dm;}
,_getColWidths:function(){if(this.__oB!=null){return this.__oB;}
;var dt=[];var dq=this.__oz;var ds=this.__oy;for(var dy=0;dy<=dq;dy++ ){var dw=0;var dv=0;var dr=Infinity;for(var dz=0;dz<=ds;dz++ ){var dp=this.__ov[dz][dy];if(!dp){continue;}
;var du=dp.getLayoutProperties().colSpan||0;if(du>1){continue;}
;var dx=this.__oD(dp);if(this.getColumnFlex(dy)>0){dv=Math.max(dv,dx.minWidth);}
...
;if(this.__ow.length>0){this._fixWidthsColSpan(dt);}
;this.__oB=dt;return dt;}
...
,renderLayout:function(ej,dO,ei){if(this._invalidChildrenCache){this.__oC();}
;var ed=qx.ui.layout.Util;var dQ=this.getSpacingX();var dW=this.getSpacingY();var eh=this._getColWidths();var ek=this._getColumnFlexOffsets(ej);var dR=[];var em=this.__oz;var dP=this.__oy;var el;for(var en=0;en<=em;en++ ){el=ek[en]?ek[en].offset:0;dR[en]=eh[en].width+el;}
...
;var er=0;for(var en=0;en<=em;en++ ){var top=0;for(var dX=0;dX<=dP;dX++ ){var ef=this.__ov[dX][en];if(!ef){top+=et[dX]+dW;continue;}
...
;er+=dR[en]+dQ;}
;}
,invalidateLayoutCache:function(){qx.ui.layout.Abstract.prototype.invalidateLayoutCache.call(this);this.__oB=null;this.__oA=null;}
,_computeSizeHint:function(){if(this._invalidChildrenCache){this.__oC();}
...
},destruct:function(){this.__ov=this.__ot=this.__ou=this.__ow=this.__ox=this.__oB=this.__oA=null;}
});}
)();
...
,events:{update:m,scrollAnimationEnd:m},properties:{scrollX:{check:j,apply:g,event:f,init:0},scrollY:{check:l,apply:h,event:b,init:0}},members:{__oE:null,add:function(r){var s=this._getChildren()[0];if(s){this._remove(s);s.removeListener(a,this._onUpdate,this);}
...
;this.stopScrollAnimation();if(L){var N=this.getScrollX();this.__oE=new qx.bom.AnimationFrame();this.__oE.on(o,function(){this.setScrollX(O);this.__oE=null;this.fireEvent(c);}
,this);this.__oE.on(i,function(Q){var P=parseInt(Q/L*(O-N)+N);this.setScrollX(P);}
,this);this.__oE.startSequence(L);}
else {this.setScrollX(O);}
;}
,scrollToY:function(U,R){var S=this.getScrollMaxY();if(U<0){U=0;}
else if(U>S){U=S;}
;this.stopScrollAnimation();if(R){var T=this.getScrollY();this.__oE=new qx.bom.AnimationFrame();this.__oE.on(o,function(){this.setScrollY(U);this.__oE=null;this.fireEvent(c);}
,this);this.__oE.on(i,function(W){var V=parseInt(W/R*(U-T)+T);this.setScrollY(V);}
,this);this.__oE.startSequence(R);}
...
(function(){var a="qx.ui.form.List",b="text",c="Boolean",d="one",f="addChildWidget",g="_applySpacing",h="Enter",j="Integer",k="action",m="keyinput",n="qx.event.type.Data",o="addItem",p="vertical",q="removeChildWidget",r="",s="_applyOrientation",t="__pu",u="single",v="keypress",w="list",x="horizontal",y="label",z="pane",A="removeItem";qx.Class.define(a,{extend:qx.ui.core.scroll.AbstractScrollArea,implement:[qx.ui.core.IMultiSelection,qx.ui.form.IForm,qx.ui.form.IModelSelection],include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MMultiSelectionHandling,qx.ui.form.MForm,qx.ui.form.MModelSelection],construct:function(B){qx.ui.core.scroll.AbstractScrollArea.call(this);this.__pu=this._createListItemContainer();this.__pu.addListener(f,this._onAddChild,this);this.__pu.addListener(q,this._onRemoveChild,this);this.getChildControl(z).add(this.__pu);if(B){this.setOrientation(x);}
else {this.initOrientation();}
;this.addListener(v,this._onKeyPress);this.addListener(m,this._onKeyInput);this.__pv=r;}
,events:{addItem:n,removeItem:n},properties:{appearance:{refine:true,init:w},focusable:{refine:true,init:true},orientation:{check:[x,p],init:p,apply:s},spacing:{check:j,init:0,apply:g,themeable:true},enableInlineFind:{check:c,init:true}},members:{__pv:null,__pw:null,__pu:null,SELECTION_MANAGER:qx.ui.core.selection.ScrollArea,getChildrenContainer:function(){return this.__pu;}
...
,_applyOrientation:function(F,C){var D=F===x;var E=D?new qx.ui.layout.HBox():new qx.ui.layout.VBox();var content=this.__pu;content.setLayout(E);content.setAllowGrowX(!D);content.setAllowGrowY(D);this._applySpacing(this.getSpacing());}
,_applySpacing:function(H,G){this.__pu.getLayout().setSpacing(H);}
...
;if(((new Date).valueOf()-this.__pw)>1000){this.__pv=r;}
;this.__pv+=e.getChar();var K=this.findItemByLabelFuzzy(this.__pv);if(K){this.setSelection([K]);}
;this.__pw=(new Date).valueOf();}
...
;if(this.getSelection()==null){this.__Bu=new qx.data.Array();this.setSelection(this.__Bu);}
;}
,properties:{selection:{check:b,event:f,apply:d,init:null}},events:{"changeSelection":h},members:{_modifingSelection:0,__Bv:null,__Bw:null,__Bu:null,_applySelection:function(k,j){if(this.__Bw!=undefined&&j!=undefined){j.removeListenerById(this.__Bw);}
;this.__Bw=k.addListener(a,this.__Bx,this);this._updateSelection();}
,__Bx:function(){this._updateSelection();}
,_changeTargetSelection:function(){if(this.getTarget()==null){return;}
;if(!this.__By()&&!this.__Bz()){return;}
;if(this._inSelectionModification()){return;}
;var m=this.getTarget().getSelection();var l=this.getSelection();if(l==null){l=new qx.data.Array();this.__Bu=l;this.setSelection(l);}
...
,_addChangeTargetListener:function(p,o){if(this.__Bv!=undefined&&o!=undefined){o.removeListenerById(this.__Bv);}
;if(p!=null){if(this.__By()||this.__Bz()){this.__Bv=p.addListener(f,this._changeTargetSelection,this);}
;}
;}
,_updateSelection:function(){if(!this.getTarget()){return;}
;this._startSelectionModification();if(this.__By()){var s=[];for(var i=0;i<this.getSelection().length;i++ ){var t=this.getSelection().getItem(i);var u=this.__BB(t);if(u!=null){s.push(u);}
;}
...
;}
;}
else if(this.__Bz()){var r=this.getSelection().getItem(this.getSelection().length-1);if(r!==undefined){this.__BA(r);this.getSelection().splice(0,this.getSelection().getLength()-1).dispose();}
else {this.getTarget().resetSelection();}
;}
;this._endSelectionModification();}
,__By:function(){var v=this.getTarget().constructor;return qx.Class.implementsInterface(v,qx.ui.core.IMultiSelection);}
,__Bz:function(){var w=this.getTarget().constructor;return qx.Class.implementsInterface(w,qx.ui.core.ISingleSelection);}
,__BA:function(x){var y=this.__BB(x);if(y==null){return;}
;if(this.__By()){this.getTarget().addToSelection(y);}
else if(this.__Bz()){this.getTarget().setSelection([y]);}
;}
,__BB:function(z){var A=this.getTarget().getSelectables(true);for(var i=0;i<A.length;i++ ){if(A[i].getModel()==z){return A[i];}
;}
;return null;}
...
},destruct:function(){if(this.__Bu){this.__Bu.dispose();}
;}
});}
)();
...
(function(){var a="changeModel",b="_applyLabelOptions",c="_applyLabelPath",d="changeTarget",e="_applyModel",f="change",g="icon",h="ReverseBindingId",k="qx.data.controller.List",m="_applyIconPath",n="BindingId",o="_applyDelegate",p="",q="changeDelegate",r="_applyTarget",s="]",t="qx.data.IListData",u="model[",v="model",w="label",x="String",y="_applyIconOptions",z=".";qx.Class.define(k,{extend:qx.core.Object,include:qx.data.controller.MSelection,implement:qx.data.controller.ISelection,construct:function(A,B,C){qx.core.Object.call(this);this.__ym=[];this.__BC=[];this.__BD=[];this.__BE={};if(C!=null){this.setLabelPath(C);}
;if(A!=null){this.setModel(A);}
;if(B!=null){this.setTarget(B);}
;}
,properties:{model:{check:t,apply:e,event:a,nullable:true,dereference:true},target:{apply:r,event:d,nullable:true,init:null,dereference:true},labelPath:{check:x,apply:c,nullable:true},iconPath:{check:x,apply:m,nullable:true},labelOptions:{apply:b,nullable:true},iconOptions:{apply:y,nullable:true},delegate:{apply:o,event:q,init:null,nullable:true}},members:{__yI:null,__ym:null,__BE:null,__BC:null,__BD:null,__BF:null,__BG:null,update:function(){this.__BI();this.__BM();this._updateSelection();}
,_applyDelegate:function(E,D){this._setConfigureItem(E,D);this._setFilter(E,D);this._setCreateItem(E,D);this._setBindItem(E,D);}
,_applyIconOptions:function(G,F){this.__BM();}
,_applyLabelOptions:function(I,H){this.__BM();}
,_applyIconPath:function(K,J){this.__BM();}
,_applyLabelPath:function(M,L){this.__BM();}
,_applyModel:function(P,N){if(N!=undefined){if(this.__yI!=undefined){N.removeListenerById(this.__yI);}
;}
;if(this.getSelection()!=undefined&&this.getSelection().length>0){this.getSelection().splice(0,this.getSelection().length).dispose();}
;if(P!=null){this.__yI=P.addListener(f,this.__BH,this);this.__yu();this.__BI();if(N==null){this._changeTargetSelection();}
else {this.__BF=true;qx.ui.core.queue.Widget.add(this);}
;}
else {var O=this.getTarget();if(O!=null){var length=O.getChildren().length;for(var i=0;i<length;i++ ){this.__BL();}
;}
;}
;}
...
;if(S!=null){if(this.getModel()!=null){for(var i=0;i<this.__ym.length;i++ ){this.__BK(this.__BO(i));}
;}
;}
;}
,__BH:function(){this.__BG=true;qx.ui.core.queue.Widget.add(this);if(this.__ym.length!=this.getModel().getLength()){this.update();}
;}
,syncWidget:function(){if(this.__BF){this._changeTargetSelection();}
;if(this.__BG){this._updateSelection();}
;this.__BG=this.__BF=null;}
,__BI:function(){if(this.getTarget()==null){return;}
;this.__yu();var U=this.__ym.length;var T=this.getTarget().getChildren().length;if(U>T){for(var j=T;j<U;j++ ){this.__BK(this.__BO(j));}
;}
else if(U<T){for(var j=T;j>U;j-- ){this.__BL();}
;}
;this.__BF=true;qx.ui.core.queue.Widget.add(this);}
,__BJ:function(){var V=this.getModel();if(V!=null){V.removeListenerById(this.__yI);this.__yI=V.addListener(f,this.__BH,this);}
;}
...
;return W;}
,__BK:function(Y){var ba=this._createItem();this._bindListItem(ba,Y);this.getTarget().add(ba);}
,__BL:function(){this._startSelectionModification();var bc=this.getTarget().getChildren();var bb=bc.length-1;var bd=bc[bb];this._removeBindingsFrom(bd);this.getTarget().removeAt(bb);bd.destroy();this._endSelectionModification();}
...
;}
,bindProperty:function(bo,br,bp,bn,bm){if(bp!=null){var bp=qx.lang.Object.clone(bp);this.__BE[br]=bp.onUpdate;delete bp.onUpdate;}
else {bp={};this.__BE[br]=null;}
;bp.onUpdate=qx.lang.Function.bind(this._onBindingSet,this,bm);bp.ignoreConverter=v;var bq=u+bm+s;if(bo!=null&&bo!=p){bq+=z+bo;}
;var bs=this.bind(bq,bn,br,bp);bn.setUserData(br+n,bs);if(!qx.lang.Array.contains(this.__BC,br)){this.__BC.push(br);}
;}
,bindPropertyReverse:function(by,bv,bw,bx,bt){var bu=u+bt+s;if(by!=null&&by!=p){bu+=z+by;}
;var bz=bx.bind(bv,this,bu,bw);bx.setUserData(by+h,bz);if(!qx.lang.Array.contains(this.__BD,by)){this.__BD.push(by);}
;}
,_onBindingSet:function(bA,bB,bC){if(this.getModel()==null||this._inSelectionModification()){return;}
;for(var i=0;i<this.__BC.length;i++ ){if(this.__BE[this.__BC[i]]!=null){this.__BE[this.__BC[i]]();}
;}
;}
,_removeBindingsFrom:function(bD){for(var i=0;i<this.__BC.length;i++ ){var bE=bD.getUserData(this.__BC[i]+n);if(bE!=null){this.removeBinding(bE);}
;}
;for(var i=0;i<this.__BD.length;i++ ){var bE=bD.getUserData(this.__BD[i]+h);if(bE!=null){bD.removeBinding(bE);}
;}
;}
,__BM:function(){if(this.getTarget()==null||this.getModel()==null){return;}
;var bF=this.getTarget().getChildren();for(var i=0;i<bF.length;i++ ){this._removeBindingsFrom(bF[i]);this._bindListItem(bF[i],this.__BO(i));}
;this.__BJ();}
...
,_setFilter:function(bR,bP){if((bR==null||bR.filter==null)&&(bP!=null&&bP.filter!=null)){this.__BN();}
...
;var bQ=this.__ym;this.__yu();if(bQ.length>this.__ym.length){for(var j=bQ.length;j>this.__ym.length;j-- ){this.getTarget().removeAt(j-1).destroy();}
;}
else if(bQ.length<this.__ym.length){for(var j=bQ.length;j<this.__ym.length;j++ ){var bT=this._createItem();this.getTarget().add(bT);}
;}
;var bS=this.getTarget().getChildren();for(var i=0;i<bS.length;i++ ){this._bindListItem(bS[i],this.__BO(i));}
;this.__BJ();this._endSelectionModification();this._updateSelection();}
,__BN:function(){this.__yu();this.__BI();this.__BM();this.__BG=true;qx.ui.core.queue.Widget.add(this);}
,__yu:function(){var bW=this.getModel();if(bW==null){return;}
;var bX=this.getDelegate();if(bX!=null){var bV=bX.filter;}
;this.__ym=[];for(var i=0;i<bW.getLength();i++ ){if(bV==null||bV(bW.getItem(i))){this.__ym.push(i);}
;}
;}
,__BO:function(bY){return this.__ym[bY];}
},destruct:function(){this.__ym=this.__BE=this.__BC=null;this.__BD=null;qx.ui.core.queue.Widget.remove(this);}
});}
)();
...
,__zK:function(bf){var be={"<":m,">":e,"&":s,"'":t,'"':D};return be[bf]||L;}
...
,process:function(bd){this.__co.appendChild(qx.log.appender.Util.toHtml(bd));this.__zH();}
,__zH:function(){this.__co.scrollTop=this.__co.scrollHeight;}
,__gF:true,toggle:function(){if(!this.__zE){this.init();}
else if(this.__zE.style.display==p){this.show();}
else {this.__zE.style.display=p;}
;}
,show:function(){if(!this.__zE){this.init();}
else {this.__zE.style.display=P;this.__co.scrollTop=this.__co.scrollHeight;}
;}
,__zI:[],execute:function(){var bf=this.__zF.value;if(bf==x){return;}
;if(bf==F){this.clear();return;}
;var be=document.createElement(j);be.innerHTML=qx.log.appender.Util.escapeHTML(V+bf);be.className=k;this.__zI.push(bf);this.__zG=this.__zI.length;this.__co.appendChild(be);this.__zH();try{var bg=window.eval(bf);}
catch(bh){qx.log.Logger.error(bh);}
;if(bg!==undefined){qx.log.Logger.debug(bg);}
;}
,__zJ:function(e){this.__co.style.height=(this.__zE.clientHeight-this.__zE.firstChild.offsetHeight-this.__zE.lastChild.offsetHeight)+c;}
...
;if(!this.__zE){return;}
;if(!qx.dom.Hierarchy.contains(this.__zE,e.getTarget())){return;}
;if(bj==b&&this.__zF.value!=x){this.execute();this.__zF.value=x;}
;if(bj==m||bj==W){this.__zG+=bj==m?-1:1;this.__zG=Math.min(Math.max(0,this.__zG),this.__zI.length);var bi=this.__zI[this.__zG];this.__zF.value=bi||x;this.__zF.select();}
;}
