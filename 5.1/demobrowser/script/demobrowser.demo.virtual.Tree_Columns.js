...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.virtual.Tree_Columns.979cb893f35a.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
,openNodeAndParents:function(bk){this.__zu(this.getModel(),bk);this.buildLookupTable();}
,closeNode:function(bl){if(qx.lang.Array.contains(this.__zo,bl)){qx.lang.Array.remove(this.__zo,bl);this.fireDataEvent(G,bl);this.buildLookupTable();}
;}
...
,isNodeOpen:function(bo){return qx.lang.Array.contains(this.__zo,bo);}
,_init:function(){this.__ym=new qx.data.Array();this.__zo=[];this.__zp=[];this._initLayer();}
...
,this);}
,getLookupTable:function(){return this.__ym;}
,_reverseLookup:function(bp){return bp;}
,_getDataFromRow:function(bq){return this.__ym.getItem(bq);}
,_getSelectables:function(){return this.__ym;}
,getOpenNodes:function(){return this.__zo;}
,isNode:function(br){return qx.ui.tree.core.Util.isNode(br,this.getChildProperty());}
,getLevel:function(bs){return this.__zp[bs];}
...
,_applyModel:function(bQ,bP){this.__zo=[];if(bQ!=null){if(qx.core.Environment.get(w)){if(!qx.Class.hasMixin(bQ.constructor,qx.data.marshal.MEventBubbling)){this.warn(H+h+k);}
;}
;bQ.addListener(J,this._onChangeBubble,this);this.__zt(bQ);}
...
;}
;if(this.__ym.indexOf(bV)!=-1){this.__zr();}
;}
;}
,_onUpdated:function(event){if(this.__hb==null){this.__hb=new qx.util.DeferredCall(function(){qx.ui.core.queue.Widget.add(this);}
,this);}
;this.__hb.schedule();}
,_onOpen:function(event){var ca=event.getRow();var bY=this.__ym.getItem(ca);if(this.isNode(bY)){if(this.isNodeOpen(bY)){this.closeNode(bY);}
...
;break;};}
;}
,_beforeApplySelection:function(cg){if(cg.length===0&&this.getSelectionMode()===g){var cf=this.__zw();var ch=this.getLookupTable().indexOf(cf);if(ch>=0){cg.push(ch);}
;}
;}
,_afterApplySelection:function(){var ci=this.getSelection();if(ci.getLength()>0&&this.getSelectionMode()===g){this.__zv(ci.getItem(0));}
else {this.__zq=[];}
;}
,__zr:function(){this.buildLookupTable();this._applyDefaultSelection();}
,buildLookupTable:function(){if(this.getModel()!=null&&(this.getChildProperty()==null||this.getLabelPath()==null)){throw new Error(Q+f);}
;this._itemWidth=0;var cm=[];this.__zp=[];var ck=-1;var cl=this.getModel();if(cl!=null){if(!this.isHideRoot()){ck++ ;cm.push(cl);this.__zp.push(ck);}
;if(this.isNodeOpen(cl)){var cj=this.__zs(cl,ck);cm=cm.concat(cj);}
;}
;if(!qx.lang.Array.equals(this.__ym.toArray(),cm)){this._provider.removeBindings();this.__ym.removeAll();this.__ym.append(cm);this.__ys();this._updateSelection();}
;}
,__zs:function(cn,cs){var cr=[];cs++ ;if(!this.isNode(cn)){return cr;}
...
;if(this.isNode(cu)){this.__zp.push(cs);cr.push(cu);if(this.isNodeOpen(cu)){var co=this.__zs(cu,cs);cr=cr.concat(co);}
;}
else {if(this.isShowLeafs()){this.__zp.push(cs);cr.push(cu);}
;}
;}
;cv.dispose();return cr;}
,__zt:function(cw){if(!qx.lang.Array.contains(this.__zo,cw)){this.__zo.push(cw);this.fireDataEvent(t,cw);}
;}
,__zu:function(cx,cy){if(cx===cy){this.__zt(cy);return true;}
;if(!this.isNode(cx)){return false;}
;var cB=cx.get(this.getChildProperty());if(cB==null){return false;}
;for(var i=0;i<cB.getLength();i++ ){var cA=cB.getItem(i);var cz=this.__zu(cA,cy);if(cz===true){this.__zt(cA);return true;}
;}
;return false;}
,__ys:function(){this.getPane().getRowConfig().setItemCount(this.__ym.getLength());this.getPane().fullUpdate();}
,getParent:function(cD){var cE=this.__ym.indexOf(cD);if(cE<0){return null;}
;var cF=this.__zp[cE];while(cE>0){cE-- ;var cC=this.__zp[cE];if(cC<cF){return this.__ym.getItem(cE);}
;}
;return null;}
,__zv:function(cG){this.__zq=[];var parent=this.getParent(cG);while(parent!=null){this.__zq.unshift(parent);parent=this.getParent(parent);}
;}
,__zw:function(){if(this.__zq==null){return this.getModel();}
;var cH=this.getLookupTable();var parent=this.__zq.pop();while(parent!=null){if(cH.contains(parent)){return parent;}
;parent=this.__zq.pop();}
...
;}
;if(!qx.core.ObjectRegistry.inShutDown&&this.__hb!=null){this.__hb.cancel();this.__hb.dispose();}
;var cI=this.getModel();if(cI!=null){cI.removeListener(J,this._onChangeBubble,this);}
;this._layer.removeListener(R,this._onUpdated,this);this._layer.destroy();this._provider.dispose();this.__ym.dispose();this._layer=this._provider=this.__ym=this.__zo=this.__hb=null;}
});}
)();
...
(function(){var a="BindingIds",b="model",c="",d="changeDelegate",f="virtual-tree-file",g="label",h="appearance",i="icon",j="bindItem",k=".length",l="]",m="virtual-tree-folder",n="[",o=".",p="String",q="qx.ui.tree.core.MWidgetController";qx.Mixin.define(q,{construct:function(){this.__xx=[];}
,properties:{labelPath:{check:p,nullable:true},iconPath:{check:p,nullable:true},labelOptions:{nullable:true},iconOptions:{nullable:true},childProperty:{check:p,nullable:true},delegate:{event:d,init:null,nullable:true}},members:{__xx:null,bindDefaultProperties:function(u,s){this.bindProperty(c,b,null,u,s);this.bindProperty(this.getLabelPath(),g,this.getLabelOptions(),u,s);var r=this.__xy(s);var t=this._tree.getLookupTable();t=qx.data.SingleValueBinding.resolvePropertyChain(t,r);if(qx.util.OOUtil.hasProperty(t.constructor,this.getChildProperty())){this.bindProperty(this.getChildProperty()+k,h,{converter:function(){return m;}
},u,s);}
else {u.setAppearance(f);}
;if(this.getIconPath()!=null){this.bindProperty(this.getIconPath(),i,this.getIconOptions(),u,s);}
;}
,bindProperty:function(y,B,z,x,v){var A=this.__xy(v,y);var w=this._tree.getLookupTable();var C=w.bind(A,x,B,z);this.__xz(x,C);}
,bindPropertyReverse:function(J,F,H,I,D){var G=this.__xy(D,J);var E=this._tree.getLookupTable();var K=I.bind(F,E,G,H);this.__xz(I,K);}
,removeBindings:function(){while(this.__xx.length>0){var L=this.__xx.pop();this._removeBindingsFrom(L);}
;}
...
;}
,_removeBindingsFrom:function(P){var Q=this.__xA(P);while(Q.length>0){var R=Q.pop();try{this._tree.getLookupTable().removeBinding(R);}
catch(e){P.removeBinding(R);}
;}
;if(qx.lang.Array.contains(this.__xx,P)){qx.lang.Array.remove(this.__xx,P);}
;}
,__xy:function(T,U){var S=n+T+l;if(U!=null&&U!=c){S+=o+U;}
;return S;}
,__xz:function(W,X){var V=this.__xA(W);if(!qx.lang.Array.contains(V,X)){V.push(X);}
;if(!qx.lang.Array.contains(this.__xx,W)){this.__xx.push(W);}
;}
,__xA:function(Y){var ba=Y.getUserData(a);if(ba==null){ba=[];Y.setUserData(a,ba);}
;return ba;}
...
;var o=this._renderer.getCellWidget();o.setOpen(p&&this._tree.isNodeOpen(s));o.addListener(a,this.__zx,this);o.setUserData(b,this.getChildProperty());o.setUserData(g,this._tree.isShowLeafs());if(this._tree.getSelection().contains(s)){this._styleSelectabled(o);}
...
,poolCellWidget:function(u){u.removeListener(a,this.__zx,this);this._removeBindingsFrom(u);this._renderer.pool(u);this._onPool(u);}
...
,__zx:function(event){var K=event.getTarget();var L=K.getUserData(k);var J=this._tree.getLookupTable().getItem(L);if(event.getData()){this._tree.openNodeWithoutScrolling(J);}
...
,addSpacer:function(){if(!this.__zA){this.__zA=new qx.ui.core.Spacer();}
else {this._remove(this.__zA);}
;this._add(this.__zA);}
,addOpenButton:function(){this._add(this.getChildControl(l));}
,_onChangeOpen:function(e){if(this.isOpenable()){this.setOpen(e.getData());}
;}
,addIcon:function(){var L=this.getChildControl(t);if(this.__zz){this._remove(L);}
;this._add(L);this.__zz=true;}
,addLabel:function(M){var N=this.getChildControl(D);if(this.__zy){this._remove(N);}
;if(M){this.setLabel(M);}
else {N.setValue(this.getLabel());}
;this._add(N);this.__zy=true;}
,_applyIcon:function(P,O){if(!this.__zC()){this.__zD(P);}
else if(!this.isOpen()){this.__zD(P);}
;}
,_applyIconOpened:function(R,Q){if(this.isOpen()){if(this.__zB()&&this.__zC()){this.__zD(R);}
else if(!this.__zB()&&this.__zC()){this.__zD(R);}
;}
;}
...
;var X;if(W){X=this.__zC()?this.getIconOpened():null;}
else {X=this.getIcon();}
;if(X){this.__zD(X);}
;W?this.addState(u):this.removeState(u);}
,__zB:function(){return qx.util.PropertyUtil.getUserValue(this,t);}
,__zC:function(){return qx.util.PropertyUtil.getUserValue(this,v);}
,__zD:function(Y){var ba=this.getChildControl(t,true);if(ba){ba.setSource(Y);}
;}
...
;}
;if(this.__zA){this.__zA.setWidth((this.getLevel()+1)*this.getIndent()-bf);}
;}
...
(function(){var a="source",b="changeLeadIcon",c="changeSize",d="changeMode",e="changeDate",f="checked",g="changeChecked",h="icon/16/places/user-desktop.png",i="value",j="size",k="leadIcon",l="date",m="mode",n="demobrowser.demo.virtual.tree.TreeItem",o="String",p="Boolean";qx.Class.define(n,{extend:qx.ui.tree.VirtualTreeItem,properties:{leadIcon:{check:o,event:b,nullable:true},checked:{check:p,event:g,nullable:true},size:{check:o,event:c,nullable:true},date:{check:o,event:e,nullable:true},mode:{check:o,event:d,nullable:true}},members:{__OK:null,__OL:null,__OM:null,__ON:null,__iP:null,_addWidgets:function(){var q=this.__OK=new qx.ui.basic.Image();this.bind(k,q,a);q.setWidth(16);this.addWidget(q);this.addSpacer();this.addOpenButton();this.addIcon();this.setIcon(h);var r=this.__OL=new qx.ui.form.CheckBox();this.bind(f,r,i);r.bind(i,this,f);r.setFocusable(false);r.setTriState(true);r.setMarginRight(4);this.addWidget(r);this.addLabel();this.addWidget(new qx.ui.core.Spacer(),{flex:1});var s=this.__OM=new qx.ui.basic.Label();this.bind(j,s,i);s.setWidth(50);this.addWidget(s);s=this.__ON=new qx.ui.basic.Label();this.bind(l,s,i);s.setWidth(150);this.addWidget(s);s=this.__iP=new qx.ui.basic.Label();this.bind(m,s,i);s.setWidth(80);this.addWidget(s);}
}});}
)();
...
(function(){var a=" array contains ",b="__gt",c="qx.ui.core.MSingleSelectionHandling",d=" items!",f="changeSelection",g="changeSelected",h="Could only select one item, but the selection",i="qx.event.type.Data";qx.Mixin.define(c,{events:{"changeSelection":i},members:{__gt:null,getSelection:function(){var j=this.__oa().getSelected();if(j){return [j];}
else {return [];}
;}
,setSelection:function(k){switch(k.length){case 0:this.resetSelection();break;case 1:this.__oa().setSelected(k[0]);break;default:throw new Error(h+a+k.length+d);};}
,resetSelection:function(){this.__oa().resetSelected();}
,isSelected:function(l){return this.__oa().isSelected(l);}
,isSelectionEmpty:function(){return this.__oa().isSelectionEmpty();}
,getSelectables:function(m){return this.__oa().getSelectables(m);}
,_onChangeSelected:function(e){var o=e.getData();var n=e.getOldData();o==null?o=[]:o=[o];n==null?n=[]:n=[n];this.fireDataEvent(f,o,n);}
,__oa:function(){if(this.__gt==null){var p=this;this.__gt=new qx.ui.core.SingleSelectionManager({getItems:function(){return p._getItems();}
,isItemSelectable:function(q){if(p._isItemSelectable){return p._isItemSelectable(q);}
else {return q.isVisible();}
;}
});this.__gt.addListener(g,this._onChangeSelected,this);}
;this.__gt.setAllowEmptySelection(this._isAllowEmptySelection());return this.__gt;}
},destruct:function(){this._disposeObjects(b);}
});}
)();
(function(){var a=", because it is not a child element!",b=" because it is not a child element!",c="Boolean",d="qx.ui.core.SingleSelectionManager",e="qx.debug",f="Invalid selectionProvider!",g="Could not check if ",h=" is selected,",j="__od",k="__ob",l="changeSelected",m="Could not select ",n="__oc",o="qx.event.type.Data";qx.Class.define(d,{extend:qx.core.Object,construct:function(p){qx.core.Object.call(this);if(qx.core.Environment.get(e)){qx.core.Assert.assertInterface(p,qx.ui.core.ISingleSelectionProvider,f);}
;this.__ob=p;}
,events:{"changeSelected":o},properties:{allowEmptySelection:{check:c,init:true,apply:j}},members:{__oc:null,__ob:null,getSelected:function(){return this.__oc;}
,setSelected:function(q){if(!this.__of(q)){throw new Error(m+q+a);}
;this.__oe(q);}
,resetSelected:function(){this.__oe(null);}
,isSelected:function(r){if(!this.__of(r)){throw new Error(g+r+h+b);}
;return this.__oc===r;}
,isSelectionEmpty:function(){return this.__oc==null;}
,getSelectables:function(t){var s=this.__ob.getItems();var u=[];for(var i=0;i<s.length;i++ ){if(this.__ob.isItemSelectable(s[i])){u.push(s[i]);}
;}
;if(!t){for(var i=u.length-1;i>=0;i-- ){if(!u[i].getEnabled()){u.splice(i,1);}
;}
;}
;return u;}
,__od:function(w,v){if(!w){this.__oe(this.__oc);}
;}
,__oe:function(x){var A=this.__oc;var y=x;if(y!=null&&A===y){return;}
;if(!this.isAllowEmptySelection()&&y==null){var z=this.getSelectables(true)[0];if(z){y=z;}
;}
;this.__oc=y;this.fireDataEvent(l,y,A);}
,__of:function(B){var C=this.__ob.getItems();for(var i=0;i<C.length;i++ ){if(C[i]===B){return true;}
;}
;return false;}
},destruct:function(){if(this.__ob.toHashCode){this._disposeObjects(k);}
else {this.__ob=null;}
...
;}
,remove:function(w){var x=this.__rV;if(qx.lang.Array.contains(x,w)){qx.lang.Array.remove(x,w);if(w.getGroup()===this){w.resetGroup();}
...
;}
,_applyInvalidMessage:function(A,z){for(var i=0;i<this.__rV.length;i++ ){this.__rV[i].setInvalidMessage(A);}
;}
,_applyValid:function(C,B){for(var i=0;i<this.__rV.length;i++ ){this.__rV[i].setValid(C);}
;}
,_applyEnabled:function(F,E){var D=this.__rV;if(F==null){for(var i=0,l=D.length;i<l;i++ ){D[i].resetEnabled();}
;}
...
;}
,selectNext:function(){var J=this.getSelection()[0];var K=this.__rV;var I=K.indexOf(J);if(I==-1){return;}
...
,selectPrevious:function(){var M=this.getSelection()[0];var N=this.__rV;var L=N.indexOf(M);if(L==-1){return;}
...
,__og:function(e){var Q=e.getData()[0];var P=e.getOldData()[0];if(P){P.setValue(false);}
