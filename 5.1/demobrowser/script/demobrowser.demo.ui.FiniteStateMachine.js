...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:demobrowser.demo.ui.FiniteStateMachine.643f45bfeadb.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var b="TERMINATED",c=" => ",d="#autoActionsBeforeOnexit",f=": Could not find friendly name for '",g=": event '",h=" blocked.  Re-queuing.",j="string",k="qx.event.type.Data",l="'",m="Attempt to transition to nonexistent state ",n="Saved-state stack is empty",o="Internal error: invalid nextState",p=" not handled.  Ignoring.",q="State ",r="#autoActionsBeforeOntransition",s=" returned a value other than ",u="#",v=" already exists",w="Attempt to transition to POP_STATE_STACK ",x=":",y="Transition ",z="__ET",A=" does not exist",B="' on '",C="qx.util.fsm.FiniteStateMachine",D="String",E="terminated",F="#autoActionsAfterOntransition",G="",H="#actionsAfterOnentry",I="Saved-state stack is full",J="#autoActionsBeforeOnentry",K=": no transition found.  No state change.",L="#autoActionsAfterOnentry",M="#actionsBeforeOnentry",N="', because the finite state machine is not running.",O=": Pushed event: ",P=": Cannot listen to event '",Q="Machine started with no available states",R=": Process event: ",S="#entry",T="#ontransition",U=" on ",V="Invalid state: not an instance of ",W="Number",X="#exit",Y="true, false, or null.",bp="Explicit transition ",bq="  ",br="qx.util.fsm.State",bl="while state stack is empty.",bm=".  Ignoring.",bn="#autoActionsAfterOnexit",bo=": Event '",bs="object",bt=" not handled for target ",bu="__EU",bv=": Queued event: ";qx.Class.define(C,{extend:qx.core.Object,construct:function(bw){qx.core.Object.call(this);this.setName(bw);this.__jB={};this.__ER=null;this.__ES=[];this.__ET=[];this.__EU=[];this.__EV={};this.__EW={};this.__EX={};this.__EY={};this.__Fa={};}
,statics:{StateChange:{CURRENT_STATE:1,POP_STATE_STACK:2,TERMINATE:3},EventHandling:{PREDICATE:1,BLOCKED:2},DebugFlags:{EVENTS:1,TRANSITIONS:2,FUNCTION_DETAIL:4,OBJECT_NOT_FOUND:8}},events:{"terminated":k},properties:{name:{check:D,nullable:true},state:{check:D,nullable:true},previousState:{check:D,nullable:true},nextState:{check:D,nullable:true},maxSavedStates:{check:W,init:2},debugFlags:{check:W,init:7}},members:{__jB:null,__ER:null,__ET:null,__EU:null,__ES:null,__EV:null,__EW:null,__EX:null,__EY:null,__Fa:null,__Fb:false,__Fc:true,isTerminated:function(){return this.__Fc;}
,addState:function(bx){if(!bx instanceof qx.util.fsm.State){throw new Error(V+br);}
;var by=bx.getName();if(by in this.__jB){throw new Error(q+by+v);}
;if(this.__ER==null){this.__ER=by;}
;this.__jB[by]=bx;}
,replaceState:function(bC,bB){if(!bC instanceof qx.util.fsm.State){throw new Error(V+br);}
;var bA=bC.getName();var bz=this.__jB[bA];this.__jB[bA]=bC;if(bB){bz._bNeedDispose=true;}
;return bz;}
,addObject:function(bG,bD,bE){var bH=bD.toHashCode();this.__EW[bG]=bH;this.__EX[bH]=bG;this.__EV[bG]=bD;if(!bE){return;}
;if(typeof (bE)==j){bE=[bE];}
;for(var i=0;i<bE.length;i++ ){var bF=bE[i];if(!this.__EY[bF]){this.__EY[bF]={};}
;this.__EY[bF][bG]=true;if(!this.__Fa[bG]){this.__Fa[bG]=[];}
;this.__Fa[bG].push(bF);}
;}
,removeObject:function(bL){var bI;var bJ;var bK;var bM;bI=this.__EW[bL];if(this.__Fa[bL]){for(var i=0;i<this.__Fa[bL].length;i++ ){bJ=this.__Fa[bL][i];delete this.__EY[bJ][bL];bM=true;for(bK in this.__EY[bJ]){bM=false;break;}
;if(bM){delete this.__EY[bJ];}
;}
;delete this.__Fa[bL];}
;delete this.__EX[bI];delete this.__EW[bL];delete this.__EV[bL];}
,getObject:function(bN){return this.__EV[bN];}
,getFriendlyName:function(bO){var bP=bO?bO.toHashCode():null;return bP?this.__EX[bP]:null;}
,getGroupObjects:function(bQ){var a=[];for(var name in this.__EY[bQ]){a.push(name);}
;return a;}
,displayAllObjects:function(){for(var bR in this.__EW){var bT=this.__EW[bR];var bS=this.getObject(bR);this.debug(bR+c+bT);this.debug(bq+bT+c+this.__EX[bT]);this.debug(bq+bR+c+this.getObject(bR));this.debug(bq+this.getObject(bR)+c+this.getFriendlyName(bS));}
;}
,_getInternalData:function(){return ({"states":this.__jB,"startState":this.__ER,"eventQueue":this.__ET,"blockedEvents":this.__EU,"savedStates":this.__ES,"friendlyToObject":this.__EV,"friendlyToHash":this.__EW,"hashToFriendly":this.__EX,"groupToFriendly":this.__EY,"friendlyToGroups":this.__Fa});}
,start:function(){this.__Fc=false;var bV=this.__ER;if(bV==null){throw new Error(Q);}
;this.setState(bV);this.setPreviousState(null);this.setNextState(null);var bU=(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL);if(bU){this.debug(this.getName()+u+bV+M);}
;this.__jB[bV].getAutoActionsBeforeOnentry()(this);if(bU){this.debug(this.getName()+u+bV+S);}
;this.__jB[bV].getOnentry()(this,null);if(bU){this.debug(this.getName()+u+bV+H);}
;this.__jB[bV].getAutoActionsAfterOnentry()(this);}
,pushState:function(bW){if(this.__ES.length>=this.getMaxSavedStates()){throw new Error(I);}
;if(bW===true){this.__ES.push(this.getState());}
else if(bW){this.__ES.push(bW);}
else {this.__ES.push(this.getPreviousState());}
;}
,popState:function(){if(this.__ES.length==0){throw new Error(n);}
;return this.__ES.pop();}
,postponeEvent:function(event){this.__EU.unshift(event);}
,enqueueEvent:function(event,bX){if(bX){this.__ET.push(event);}
else {this.__ET.unshift(event);}
;if(this.getDebugFlags()&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS){var bY=this.getFriendlyName(event.getTarget());if(bX){this.debug(this.getName()+O+event.getType()+(bY?U+bY:G));}
else {this.debug(this.getName()+bv+event.getType()+(bY?U+bY:G));}
;}
;}
,eventListener:function(event){if(this.__Fc){this.debug(this.getName()+P+event.getType()+N);return;}
;var e=event.clone();this.enqueueEvent(e,false);this.__Fd();}
,fireImmediateEvent:function(cb,cc,ca){if(this.__Fc){this.debug(this.getName()+P+cb+N);return;}
...
,this,cg||20);}
,__Fd:function(){if(this.__Fb){return;}
;this.__Fb=true;while(this.__ET.length>0){var event=this.__ET.pop();var ch=this.__Fe(event);if(ch){event.dispose();}
;}
;this.__Fb=false;}
,__Fe:function(event){var ck;var cl;var cj;var cn;var cq;var e;var cp;var ci=this.getDebugFlags();var cu=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.EVENTS;var cs=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.TRANSITIONS;var cr=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.FUNCTION_DETAIL;var co=ci&qx.util.fsm.FiniteStateMachine.DebugFlags.OBJECT_NOT_FOUND;var ct=this.getFriendlyName(event.getTarget());if(cu){this.debug(this.getName()+R+event.getType()+(ct?U+ct:G));}
;ck=this.getState();cn=this.__jB[ck];cq=cn.transitions;e=cn.getEvents()[event.getType()];if(!e){if(cu){this.debug(this.getName()+bo+event.getType()+l+p);}
;return true;}
;if(typeof (e)==bs){if(!ct){if(co){this.debug(this.getName()+f+event.getType()+B+event.getTarget()+l);}
;return true;}
;cp=e[ct];if(!cp){if(cu){this.debug(this.getName()+bo+event.getType()+l+bt+ct+bm);}
;return true;}
;}
else {cp=e;}
;switch(cp){case qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE:break;case qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED:if(cu){this.debug(this.getName()+bo+event.getType()+l+h);}
;this.__EU.unshift(event);return false;default:if(typeof (cp)==j){if(cq[cp]){var cm=cq[cp];cq={};cq[cp]=cm;}
else {throw new Error(bp+cp+A);}
;break;}
;};for(var t in cq){var cm=cq[t];switch(cm.getPredicate()(this,event)){case true:break;case false:continue;case null:return true;default:throw new Error(y+ck+x+t+s+Y);};cl=cm.getNextState();if(typeof (cl)==j){if(!cl in this.__jB){throw new Error(m+cl);}
;this.setNextState(cl);}
else {switch(cl){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:cl=ck;this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:if(this.__ES.length==0){throw new Error(w+bl);}
;cl=this.__ES.pop();this.setNextState(cl);break;case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:this.__Fc=true;this.setNextState(null);break;default:throw new Error(o);};}
;if(cr){this.debug(this.getName()+u+ck+u+t+r);}
;cm.getAutoActionsBeforeOntransition()(this);if(cr){this.debug(this.getName()+u+ck+u+t+T);}
;cm.getOntransition()(this,event);if(cr){this.debug(this.getName()+u+ck+u+t+F);}
;cm.getAutoActionsAfterOntransition()(this);if(cr){this.debug(this.getName()+u+ck+d);}
;cn.getAutoActionsBeforeOnexit()(this);if(cr){this.debug(this.getName()+u+ck+X);}
;cn.getOnexit()(this,event);if(cr){this.debug(this.getName()+u+ck+bn);}
;cn.getAutoActionsAfterOnexit()(this);if(cn._bNeedDispose){cn.dispose();}
;if(this.__Fc){if(cr){this.debug(this.getName()+u+b);}
;this.fireDataEvent(E,ck);return true;}
;cn=this.__jB[this.getNextState()];this.setPreviousState(ck);this.setState(this.getNextState());this.setNextState(null);cj=ck;ck=cl;cl=undefined;if(cr){this.debug(this.getName()+u+ck+J);}
;cn.getAutoActionsBeforeOnentry()(this);if(cr){this.debug(this.getName()+u+ck+S);}
;cn.getOnentry()(this,event);if(cr){this.debug(this.getName()+u+ck+L);}
;cn.getAutoActionsAfterOnentry()(this);for(var i=0;i<this.__EU.length;i++ ){e=this.__EU.pop();this.__ET.unshift(e);}
;if(cs){this.debug(this.getName()+u+cj+c+this.getName()+u+ck);}
;return true;}
;if(cs){this.debug(this.getName()+u+ck+g+event.getType()+l+K);}
;return true;}
},destruct:function(){this._disposeArray(z);this._disposeArray(bu);this.__ES=this.__jB=null;}
});}
)();
(function(){var b="State info must be an object",c="function",d="  var groupObjects = ",h="parameters",k=")",l=" fsm.getObject('",m="groups",n="Invalid function parameters: ",o="Invalid onexit type: ",p="  for (var i = 0; i < groupObjects.length; i++)",q="context",r="string",s="autoActionsAfterOnentry",t="Invalid numeric value in events object: ",u='"',v="__EI",w="__EK",x="Invalid state name",y=" value: ",z="');",A="State ",B="autoActionsBeforeOnexit",C="autoActionsBeforeOnentry",D="}",E="): ",F="events must be an object",G="onexit",H="{",I="Invalid function request type: ",J=": ",K="__EO",L="expected array, found ",M="Invalid friendly name in 'objects' list: ",N="autoActionsAfterOnexit",O="  fsm.debug(ex);",P="(",Q="__EL",R="__EM",S="Invalid onentry type: ",T="    fsm.getObject(objName).",U=")(fsm)",V="events",W="undefined",X="Invalid ",Y="try",ba="    var objName = groupObjects[i];",bb="Invalid value in events object: ",bc="').",bd="objects",be="expected object, found ",bf="qx.util.fsm.Transition",bg="The events object must be provided in new state info",bh="catch(ex)",bi="  {",bj="__EP",bk="    fsm.getGroupObjects('",bl="Invalid value in events object ",bm="Invalid transition: not an instance of ",bn="  }",bo="__EJ",bp="__EN",bq="Invalid 'objects' list: expected array, got ",br="Invalid numeric value in events object ",bs="qx.util.fsm.State",bt="onentry",bu=";",bv="Invalid function request parameter type: ",bw="Invalid 'groups' list: expected array, got ",bx="fsm",by=",",bz="Adding user-provided field to state: ",bA="number",bB="object";qx.Class.define(bs,{extend:qx.core.Object,construct:function(bD,bF){var bE;qx.core.Object.call(this);this.setName(bD);if(typeof (bF)!=bB){throw new Error(b);}
;bE=bF.context||window;this.setUserData(q,bE);for(var bC in bF){switch(bC){case bt:this.setOnentry(this.__EQ(bF[bC],bE));break;case G:this.setOnexit(this.__EQ(bF[bC],bE));break;case C:this.setAutoActionsBeforeOnentry(bF[bC]);break;case s:this.setAutoActionsAfterOnentry(bF[bC]);break;case B:this.setAutoActionsBeforeOnexit(bF[bC]);break;case N:this.setAutoActionsAfterOnexit(bF[bC]);break;case V:this.setEvents(bF[bC]);break;case q:break;default:this.setUserData(bC,bF[bC]);this.debug(A+bD+J+bz+bC);break;};}
;if(!this.getEvents()){throw new Error(bg);}
;this.transitions={};}
,statics:{_commonTransformAutoActions:function(actionType,value,context){if(typeof (value)!=bB){throw new Error(X+actionType+y+typeof (value));}
;var funcFragment;var func=Y+H;var param;var objectAndGroupList;for(var f in value){var functionRequest=value[f];if(!functionRequest instanceof Array){throw new Error(I+L+typeof (functionRequest));}
...
else {if(!params instanceof Array){throw new Error(n+L+typeof (params));}
;}
;funcFragment=f+P;for(var j=0;j<params.length;j++ ){if(j!=0){funcFragment+=by;}
;if(typeof (params[j])==c){funcFragment+=P+params[j]+U;}
else if(typeof (params[j])==r){funcFragment+=u+params[j]+u;}
...
;for(var j=0;j<a.length;j++ ){if(typeof (a[j])!=r){throw new Error(M+a[j]);}
;func+=l+a[j]+bc+funcFragment+bu;}
;var g=objectAndGroupList[m];if(g){if(!g instanceof Array){throw new Error(bw+typeof (g));}
;for(j=0;j<g.length;j++ ){func+=d+bk+g[j]+z+p+bi+ba+T+funcFragment+bu+bn;}
;}
;}
;}
;func+=D+bh+H+O+D;return qx.lang.Function.bind(new Function(bx,func),context);}
},properties:{name:{transform:v,nullable:true},onentry:{transform:bo,nullable:true,init:function(bG,event){}
},onexit:{transform:w,nullable:true,init:function(bH,event){}
},autoActionsBeforeOnentry:{transform:R,nullable:true,init:function(bI,event){}
},autoActionsAfterOnentry:{transform:bp,nullable:true,init:function(bJ,event){}
},autoActionsBeforeOnexit:{transform:K,nullable:true,init:function(bK,event){}
},autoActionsAfterOnexit:{transform:bj,nullable:true,init:function(bL,event){}
},events:{transform:Q,nullable:true}},members:{__EI:function(bM){if(typeof (bM)!=r||bM.length<1){throw new Error(x);}
;return bM;}
,__EJ:function(bN){switch(typeof (bN)){case W:return function(bO,event){}
;case c:return qx.lang.Function.bind(bN,this.getUserData(q));default:throw new Error(S+typeof (bN));};}
,__EK:function(bP){switch(typeof (bP)){case W:return function(bQ,event){}
;case c:return qx.lang.Function.bind(bP,this.getUserData(q));default:throw new Error(o+typeof (bP));};}
,__EL:function(bT){if(typeof (bT)!=bB){throw new Error(F);}
;for(var e in bT){var bR=bT[e];if(typeof (bR)==bA&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(t+e+J+bR);}
else if(typeof (bR)==bB){for(var bS in bR){if(typeof (bR[bS])==bA&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.PREDICATE&&bR[bS]!=qx.util.fsm.FiniteStateMachine.EventHandling.BLOCKED){throw new Error(br+P+e+E+bS+J+bR[bS]);}
else if(typeof (bR[bS])!=r&&typeof (bR[bS])!=bA){throw new Error(bl+P+e+E+bS+J+bR[bS]);}
;}
;}
else if(typeof (bR)!=r&&typeof (bR)!=bA){throw new Error(bb+e+J+bT[e]);}
;}
;return bT;}
,__EM:function(bU){return qx.util.fsm.State._commonTransformAutoActions(C,bU,this.getUserData(q));}
,__EN:function(bV){return qx.util.fsm.State._commonTransformAutoActions(s,bV,this.getUserData(q));}
,__EO:function(bW){return qx.util.fsm.State._commonTransformAutoActions(B,bW,this.getUserData(q));}
,__EP:function(bX){return qx.util.fsm.State._commonTransformAutoActions(N,bX,this.getUserData(q));}
,__EQ:function(f,bY){if(typeof (f)==c){f=qx.lang.Function.bind(f,bY);}
;return f;}
,addTransition:function(ca){if(!ca instanceof qx.util.fsm.Transition){throw new Error(bm+bf);}
;this.transitions[ca.getName()]=ca;}
}});}
)();
(function(){var a="function",b="autoActionsBeforeOntransition",c="or one of the Fsm.StateChange constants",d="__Fh",e="autoActionsAfterOntransition",g="context",h="__Ff",i="string",j="predicate",k="__Fj",l="nextState must be an explicit state name, ",m="Transition ",n=": ",o="__Fg",p="String",q="number",r="Invalid transition nextState type: ",s="__Fi",t="Adding user-provided field to transition: ",u="ontransition",v="Invalid transition predicate type: ",w="boolean",x="nextState",y="Invalid transition nextState value: ",z="Invalid ontransition type: ",A="qx.util.fsm.Transition",B="undefined";qx.Class.define(A,{extend:qx.core.Object,construct:function(F,D){var E;qx.core.Object.call(this);this.setName(F);E=D.context||window;this.setUserData(g,E);for(var C in D){switch(C){case j:this.setPredicate(this.__EQ(D[C],E));break;case x:this.setNextState(D[C]);break;case b:this.setAutoActionsBeforeOntransition(this.__EQ(D[C],E));break;case e:this.setAutoActionsAfterOntransition(this.__EQ(D[C],E));break;case u:this.setOntransition(this.__EQ(D[C],E));break;case g:break;default:this.setUserData(C,D[C]);this.debug(m+F+n+t+C);break;};}
;}
,properties:{name:{check:p,nullable:true},predicate:{init:function(G,event){return true;}
,transform:h},nextState:{init:qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE,transform:o},autoActionsBeforeOntransition:{init:function(H,event){}
,transform:d},autoActionsAfterOntransition:{init:function(I,event){}
,transform:s},ontransition:{init:function(J,event){}
,transform:k}},members:{__Ff:function(K){switch(typeof (K)){case B:return function(L,event){return true;}
...
,__Fg:function(N){switch(typeof (N)){case i:return N;case q:switch(N){case qx.util.fsm.FiniteStateMachine.StateChange.CURRENT_STATE:case qx.util.fsm.FiniteStateMachine.StateChange.POP_STATE_STACK:case qx.util.fsm.FiniteStateMachine.StateChange.TERMINATE:return N;default:throw new Error(y+N+n+l+c);};break;default:throw new Error(r+typeof (N));};}
,__Fh:function(O){return qx.util.fsm.State._commonTransformAutoActions(b,O,this.getUserData(g));}
,__Fi:function(P){return qx.util.fsm.State._commonTransformAutoActions(e,P,this.getUserData(g));}
,__Fj:function(Q){switch(typeof (Q)){case B:return function(R,event){}
;case a:return qx.lang.Function.bind(Q,this.getUserData(g));default:throw new Error(z+typeof (Q));};}
,__EQ:function(f,S){if(typeof (f)==a){f=qx.lang.Function.bind(f,S);}
;return f;}
}});}
)();
